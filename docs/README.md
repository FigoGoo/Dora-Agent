...
用户要求以外部设计文档 `AIGC_Creation_Agent v1_1.md` 为准，对 Dora-Agent 进行系统重构，并要求分阶段产出设计文档。旧产品、技术、契约和测试文档已从仓库清理，新一轮事实源从该外部设计文档重新整理。
...
2. `api/schemas/**`、`api/agui/**`、`api/openapi/**`、`api/thrift/**`、`db/migrations/**` 和 `tests/fixtures/**` 承接字段级事实源。
4. 本目录不直接作为研发编码依据。
