...
SKILL-2 已完成实现并被当前字段级事实源承接：`api/thrift/business_agent_service.thrift` 已包含 `SkillOutputElementDTO`、`SkillSpecResponse.output_elements` 和 `ReviewCandidateSkillSpecResponse.output_elements`；`db/migrations/iterations/2026-06-27-business-core/business/0021_skill_output_element_structure.up.sql` 已落 per-skill 输出元素结构字段；业务侧 `services/business/internal/application/skillcatalog/**` 已完成写入、校验和装配；Agent 侧 `services/agent/internal/application/workbench/**` 已消费输出元素结构组织草稿和最终产物。
...
`GetPublishedSkillSpec` 按 `version_id` 批量装配（一次 `IN` 查询，避免 N+1）。`ReviewCandidateSkillSpecResponse` 也已补 `output_elements` 供审核态校验，与既有 `expected_elements_json` 并存（前者是结构声明，后者是测试期望）。
...
4. **审核态范围**：已定并实现 — `ReviewCandidateSkillSpecResponse` 同步补 `output_elements`，便于发布前校验。
