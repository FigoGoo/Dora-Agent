...
当前成熟度：active。
使用方式：作为 RPC 服务边界、通用 DTO、业务规则和错误码当前事实源；字段级 request/response 以 `api/thrift/business_agent_service.thrift` 为准，索引见 `docs/contracts/字段级契约索引.md`。

已补齐项：当前 Thrift 方法索引、方法级 timeout/retry、幂等冲突语义、错误码覆盖和 contract fixture 对应关系已在本文冻结。

运行证据：`tests/reports/m6-service-acceptance-report.md` 已记录 18 个 RPC 服务账本、Thrift 方法集合、Kitex 生成目录、`RegisterAll` 注册、handler 方法和 per-method fixture 覆盖通过；`tests/contract/validate_fixtures.py` 由 `scripts/validate-m6.sh` 串行执行。
...
- 需要幂等的写操作必须形成内部业务幂等键；调用方可传 `request_meta.idempotency_key`，未传时由业务应用层按业务字段、目标资源和操作者生成。
...
| idempotency_key | string | 可选 | 调用方显式业务幂等键；为空时业务应用层可按业务字段生成内部幂等键 | 读操作可为空 |
...
- RPC 服务级验收：见 `tests/reports/m6-service-acceptance-report.md`。
- Contract fixture 验证入口：`tests/contract/validate_fixtures.py`，由 `scripts/validate-m6.sh` 串行执行。
- 新增或调整 RPC 方法时必须同步 Thrift、Kitex 生成代码、fixture、handler 和本文档。
