...
当前成熟度：active。
使用方式：作为 Agent DB 边界、表清单、字段类型、状态机、索引、保留周期和测试策略当前事实源；实际 SQL 以 `db/migrations/iterations/20260627_agent_runtime/agent/001_create_agent_runtime.up.sql` 为当前落脚。

已补齐项：字段类型、唯一约束、状态迁移约束、数据保留周期、`agent_memories` 启用策略、run 并发策略和 session 归档策略已在本文冻结。

运行证据：`tests/reports/m1-technical-baseline-report.md` 已记录 Agent DB Testcontainers PostgreSQL migration up/down、核心表、无外键、repository 场景和边界检查通过；`tests/reports/m6-service-acceptance-report.md` 已记录 DB 验收覆盖 Agent/Business migration up/down 成对、seed 可执行、无数据库级外键和双库边界。
...
- Migration up/down 与 repository 场景：见 `tests/reports/m1-technical-baseline-report.md`。
- 服务级 DB 边界：见 `tests/reports/m6-service-acceptance-report.md`。
- 关键查询路径已通过索引设计和 repository / service 测试覆盖；后续新增查询或清理任务时必须补 query count 或性能证据。
