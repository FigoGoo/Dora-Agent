...
PRD 套件已确认 `product_status：Done`，第一阶段服务端开发已经完成。本目录用于沉淀智能体微服务、业务微服务、前端和测试之间的契约边界、状态和冻结条件。
...
| 业务微服务 RPC 契约 | `docs/contracts/rpc/业务微服务RPC契约草案.md` | 文档与契约责任域汇总；业务服务确认；Agent 服务提出需求 | active | 智能体微服务调用业务能力 |
| 统一 Agent 工作台 AG-UI 事件协议 | `docs/contracts/ag-ui/统一Agent工作台AGUI事件协议草案.md` | 文档与契约责任域汇总；Agent 服务生产；前端消费 | active | SSE、事件类型、payload、断线重连 |
| Agent 工作台 API 契约 | `docs/contracts/api/Agent工作台API契约草案.md` | 文档与契约责任域汇总；Agent 服务与前端确认 | active | session、run、stream、confirm、snapshot |
| C 端与后台业务 API 契约 | `docs/contracts/api/C端与后台业务API契约草案.md` | 文档与契约责任域汇总；业务服务与前端确认 | active | Auth、Public、Project、Asset、Works、Enterprise、Admin |
| Agent 领域数据模型 | `docs/contracts/data/Agent领域数据模型草案.md` | Agent 服务责任域 | active | Agent Runtime 表和状态机 |
| 第一版迭代 SQL 清单 | `docs/contracts/sql/第一版迭代SQL清单.md` | 文档与契约责任域汇总；业务服务与 Agent 服务按数据库边界维护 | active | 业务库和 Agent 库 SQL 计划 |

## 历史补充契约

| 文档 | 路径 | 状态 | 说明 |
| --- | --- | --- | --- |
| SKILL-2 输出元素结构契约设计 | `docs/contracts/rpc/SKILL-2-输出元素结构契约设计.md` | archived | P0 缺口设计已完成，当前事实源由 Thrift、migration、实现和测试报告承接。 |
...
具体状态以 [`契约成熟度复核.md`](./契约成熟度复核.md) 为准。字段级事实源以 [`字段级契约索引.md`](./字段级契约索引.md) 为准。当前 P0 设计缺口和 M6 服务级验收已关闭；后续新增或调整 RPC、API、AG-UI、数据模型或 SQL 时，必须同步字段级事实源、fixture、服务级验证和本文档状态。

## Done 条件

- [x] `契约成熟度复核.md` 中 P0 缺口关闭或明确延期且不影响本次实现。
- [x] RPC 契约完成字段级 request/response、错误码、幂等、timeout/retry 和 fixture 映射。
- [x] AG-UI 事件协议完成 canonical 事件、payload、补偿、snapshot fallback 和兼容映射。
- [x] API 契约完成 route 级 request/response、鉴权、幂等、分页和 OpenAPI 映射。
- [x] Agent 数据模型完成表字段、索引、状态机、保留周期和测试边界检查。
- [x] SQL 清单完成业务库与 Agent 库落脚、up/down、无外键检查和执行证据。
