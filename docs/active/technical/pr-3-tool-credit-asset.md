...
相关代码路径：`internal/contracts/toolasset/**`、`services/agent/internal/infra/repository/**`、`services/business/internal/infra/repository/businesscore/**`
...
| 类型 | 文件 | 说明 |
| --- | --- | --- |
| Tool | `internal/contracts/toolasset/tool.go` | ToolPlan、ToolTask、ToolResult 类型和状态校验 |
| Credit | `internal/contracts/toolasset/credit.go` | 冻结、扣减、释放状态机校验 |
| Asset | `internal/contracts/toolasset/asset.go` | GeneratedAsset、部分提交和失败资产不扣费守卫 |
| Worker | `internal/contracts/toolasset/worker.go` | provider async resume 和 Redis stream 事件校验 |
| AG-UI | `internal/contracts/toolasset/agui.go` | Tool 生成费、ToolTask、AssetCommit payload 类型 |
| Tests | `internal/contracts/toolasset/*_test.go` | 读取 PR-3 active fixture 做漂移防护 |
...
| Agent Workbench | `services/agent/internal/application/workbench/app_tool_generation.go`、`services/agent/internal/application/workbench/app.go` | Board approve 后创建 ToolPlan、Tool 生成费 CostDisclosure、tool_generation_confirmation、ToolTask 和资产提交 AG-UI 闭环 |
| Business Repository | `services/business/internal/infra/repository/businesscore/tool_credit_asset.go` | Credit freeze / commit / release、Asset partial commit 事务与幂等 |
| Migration Tests | `services/agent/internal/infra/repository/tool_credit_asset_integration_test.go`、`services/business/internal/infra/repository/businesscore/tool_credit_asset_integration_test.go` | PR-3 agent / business migration up/down 和无外键边界验证 |
...
- [x] `internal/contracts/toolasset` 包存在。
...
```bash
go test ./internal/contracts/toolasset
...
go test ./internal/contracts/toolasset ./services/agent/internal/infra/repository ./services/business/internal/infra/repository/businesscore
...
```text
ok github.com/FigoGoo/Dora-Agent/internal/contracts/toolasset
