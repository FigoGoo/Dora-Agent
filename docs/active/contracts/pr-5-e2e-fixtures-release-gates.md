...
| Test Environment HTTP Service E2E | `tests/e2e/http/validate_release_http_service_e2e.py`、`scripts/validate-release-http-service-e2e.sh` | 已部署测试环境的 Business / Agent HTTP 主路径验收，依赖 `RELEASE_BUSINESS_BASE_URL`、`RELEASE_AGENT_BASE_URL` 和测试账号 |
| Test Environment HTTP Service E2E Report | `tests/reports/release-http-service-e2e-report.md` | 测试环境 HTTP E2E 自动报告，当前为 `pending_environment`，执行通过后由脚本覆盖为 `status: passed` |
...
- [ ] `scripts/validate-release-http-service-e2e.sh` / `make release-http-service-e2e` 在完整测试环境执行并归档 `status: passed` 测试报告。
