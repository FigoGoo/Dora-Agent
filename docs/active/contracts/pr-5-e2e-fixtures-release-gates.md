...
```text
PR-5 E2E fixture、fake provider 行为、发布治理、轻量 validator、本地 service-level PostgreSQL E2E、Agent HTTP router + Redis container E2E、Agent 独立进程 HTTP smoke、Business 独立进程 HTTP smoke、本地 Agent + Business 双服务 HTTP smoke 和本地真实浏览器前端联动 smoke 已冻结。
完整测试环境 HTTP 服务 E2E 仍是进入真实流量前的发布 gate。
```
