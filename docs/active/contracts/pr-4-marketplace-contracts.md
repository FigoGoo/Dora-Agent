...
- 不接外部打款、支付供应商或真实资金出账；本阶段冻结 settlement hold、内部出账治理和 settlement ledger 契约。
...
| Validator | `tests/contract/validate_skill_market_contracts.py` | PR-4 contract gate |
...
| `POST /api/admin/settlements/{settlement_id}/release-hold` | 管理端 | hold 到期后解除 settlement hold，状态进入 eligible |
| `POST /api/admin/settlements/{settlement_id}/confirm-payout` | 管理端 | 管理员确认内部出账引用，状态进入 settled |
...
```text
CommitSkillUsageAndSettle
  -> settlement.status=pending_hold
  -> hold_until 到期且无退款/冻结
  -> releaseSkillSettlementHold(action=release_hold)
  -> settlement.status=eligible
  -> confirmSkillSettlementPayout(action=confirm_payout, payout_reference)
...
skill_settlement_payout_records
  ...
  settlement_id
  creator_user_id
  action=release_hold|confirm_payout
  status_before
  status_after
  payout_reference
  reason_code
...
- `release-hold` 仅允许 `pending_hold -> eligible`，且 `hold_until <= now`。
- `confirm-payout` 仅允许 `eligible -> settled`，必须提供 `payout_reference`。
- 两个动作必须同步更新 `skill_usage_records.settlement_status`。
- 两个动作必须写入 `skill_settlement_payout_records`，同一 `idempotency_key` 重放返回原治理记录；同 key 但 settlement/action 不一致返回幂等冲突。
- 本阶段 `payout_reference` 是内部 ledger 或人工凭证引用，不得存储外部支付密钥、银行账号或敏感资金通道参数。
...
- [x] `python3 tests/contract/validate_skill_market_contracts.py` 通过。
...
- [x] Creator Portal HTTP 主路径已接入草稿、提交审核、创作者 listing 和脱敏 analytics。
...
- [x] 内部 settlement 出账治理已接入 hold 到期解除、人工出账确认、幂等治理记录和 usage/settlement 状态同步。
- [ ] 外部真实打款通道和财务供应商对账待后续阶段。
