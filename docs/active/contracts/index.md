...
| CreditLot | v1 | Business Credit | `api/schemas/credit/credit-lot.v1.schema.json` | Business Credit | Agent Runtime / Frontend / Test | `tests/fixtures/contracts/credit/redeem_lot_success.json`、`tests/fixtures/contracts/credit/recharge_order_mock_paid.json` | `B0` | active |
| RechargeOrder | v1 | Business Credit | `api/schemas/credit/recharge-order.v1.schema.json` | Business Credit | Frontend / Test | `tests/fixtures/contracts/credit/recharge_order_mock_paid.json` | `B0` | active |
| MockPayment | v1 | Business Credit / Test | `api/schemas/credit/mock-payment.v1.schema.json` | Business Credit / Test | Test | `tests/fixtures/contracts/credit/recharge_order_mock_paid.json` | `B0` | active |
| BillingPackageCatalog | v1 | Business Billing | `api/openapi/business-api.yaml`、`db/migrations/iterations/2026-06-27-business-core/business/0023_b0_recharge_order_mock_payment.up.sql`、`tests/business/seed/business_core_seed.sql` | Admin / Business Credit | User Frontend / Admin Frontend / Test | `tests/fixtures/contracts/credit/recharge_order_mock_paid.json`、`tests/business/seed/business_core_seed.sql` | `B0` | active |
| BillingPackageSKU | v1 | Business Billing | `api/openapi/business-api.yaml`、`db/migrations/iterations/2026-06-27-business-core/business/0023_b0_recharge_order_mock_payment.up.sql` | Admin / Business Credit | User Frontend / Admin Frontend / Test | `tests/business/seed/business_core_seed.sql` | `B0` | active |
| PackageEntitlementSnapshot | v1 | Business Billing | `api/openapi/business-api.yaml`、`db/migrations/iterations/2026-06-27-business-core/business/0023_b0_recharge_order_mock_payment.up.sql` | Business Credit | User / Enterprise / Admin / Test | `services/business/internal/transport/http/credit_b0_integration_test.go` | `B0` | active |
| EnterpriseContract | v1 | Business Billing | `api/openapi/business-api.yaml`、`db/migrations/iterations/2026-06-27-business-core/business/0023_b0_recharge_order_mock_payment.up.sql` | Business Credit / Admin | Enterprise / Admin / Test | `services/business/internal/transport/http/credit_b0_integration_test.go`、`services/business/internal/transport/http/smoke_b0_integration_test.go` | `B0` | active |
| CreditMaintenance | v1 | Business Credit / Admin Finance | `api/openapi/business-api.yaml`、`services/business/internal/application/credit/app.go` | Admin Finance / Business Credit | Admin Frontend / Test | `services/business/internal/application/credit/app_test.go` | `B0` | active |
...
| B0BusinessBaselineSmoke | v1 | 测试与验收 / Business Service / 管理端 | `services/business/internal/application/smoke/**`、`services/business/internal/transport/http/smoke_b0_integration_test.go`、`services/business/internal/transport/http/credit_b0_integration_test.go` | Business Service / Test | CI / Admin Frontend / Test | `tests/business/seed/business_core_seed.sql` | `B0` | active |
...
| PR-5 | [`pr-5-e2e-fixtures-release-gates.md`](./pr-5-e2e-fixtures-release-gates.md) | E2E fixtures、fake provider、M7 gates、service-level PostgreSQL E2E、Agent HTTP router + Redis container E2E、Agent / Business 独立进程 HTTP smoke、本地 Agent + Business 双服务 HTTP smoke、本地真实浏览器前端联动 smoke、release HTTP 服务 E2E 自动化入口和报告 | 默认 Skill、付费市场 Skill、企业安装、partial failure、listing suspended、refund、worker replay、Redis AG-UI replay E2E、Agent / Business main 启动 smoke、双服务 HTTP smoke、用户端 / 管理端 browser smoke、release HTTP E2E |
