...
Agent 领域库建表一律不添加数据库级外键约束，也不通过关联键表达表关联。确需保存 session_id、run_id、task_id、user_id、tenant_id 等运行时标识时，只作为普通字段、查询过滤条件或审计信息，不作为 schema 层关联关系、完整性约束或数据清洗依赖；跨表一致性通过业务流程、幂等键、必要索引、application 校验和测试维护。

## CRUD 规范

- 写入必须幂等，避免重复事件、重复 Tool 调用记录和重复恢复。
- 查询按 session_id、run_id、task_id、user_id、tenant_id 优化。
- 列表查询必须分页，默认 10 条每页；事件补偿、历史消息、任务列表和产物列表都必须有明确分页语义。
- 尽量避免在 `for` 循环中逐条查询关联数据；优先批量查询、`IN` 查询或按查询路径设计聚合读取。
- 删除优先软删除或按保留策略归档。
