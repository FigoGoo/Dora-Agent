...
Dora-Agent 第一版使用火山引擎 TOS 公共桶保存上传素材、生成资产和公开作品媒体。前端不接触 TOS AK/SK；业务服务按权限、空间、项目、文件类型和大小签发上传凭证，前端直接上传到 TOS，上传完成后再调用业务 API 确认资产。
...
- object key 只能由业务服务生成，前端不得自造 key。
...
用于用户在资产库或工作台上传素材。上传前由业务服务签发 key，前端直传后再确认。
