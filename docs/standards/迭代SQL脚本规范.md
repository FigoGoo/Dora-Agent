...
- 业务库和 Agent 领域库分开。
- 数据库创建和变更一律不添加数据库级外键约束；脚本不得包含 `FOREIGN KEY`、`REFERENCES` 或等价外键约束。
- 不通过关联键表达表关联；确需保存上游标识时仅作为普通业务字段、查询过滤条件或审计信息，不作为 schema 层关联关系、完整性约束或数据清洗依赖。
- 跨表一致性通过业务流程、幂等键、唯一约束、必要索引、application/domain 校验和测试保证。
