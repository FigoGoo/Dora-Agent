...
| `safety.prompt.evaluating` | LLM 提示词安全评估开始 | `message.stream` / `error.notice` |
...
| 提示词安全评估 | `safety_status`、`safety_evidence_id`、`checked_target`、`user_message`、`retryable` | 展示通过、阻止、失败或超时，不暴露安全策略细节 |
| 项目归档阻断 | `project_status`、`creative_allowed`、`read_only_reason`、`allowed_actions`、`user_message` | 归档项目只读，禁用创作入口 |
...
- 内容安全证据只用于证明“生成、扣费、分享前已完成评估”，前端不展示策略细节、内部评分、完整 Prompt 或供应商原始响应。
- 项目归档后工作台进入只读模式，Composer、确认、重试、继续生成、保存新资产和上传到项目入口全部禁用。
- 媒体预览和下载不从 AG-UI 事件读取长期 URL，统一通过业务 API 获取授权后的 TOS 公共 URL。
