...
2. 以 `code-plan/agent/**` 与 `code-plan/business/**` 作为正式开发设计计划和需求映射矩阵。
3. 先按 `code-plan` 定义账户、空间、模型、Tool、Skill、积分、资产的 RPC/API 契约。
4. 按 `code-plan` 定义 AG-UI 事件协议和 Agent 领域数据模型。
5. 按 `code-plan` 准备本地 SQL 脚本和测试夹具。
6. 按功能点进入本地开发、联调和测试。
...
- [x] 已明确第一版非目标。
- [x] 已提供可用于后续模块 PRD、契约和 `code-plan` 正式开发计划的功能大纲。
