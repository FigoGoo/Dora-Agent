...
Ingress/Command Receipt 子契约使用独立 Corpus 和 Manifest，避免把入队事务与 Owner/Fence 状态迁移混成一个模型；详见 [`session-lane-ingress-command-contract-v1.md`](./session-lane-ingress-command-contract-v1.md)。PostgreSQL Header/Result、legacy 升级、Turn/Run/Lease/HOL 和真实 PG 证据候选见 [`session-lane-postgresql-design-v1.md`](./session-lane-postgresql-design-v1.md)。
...
4. PostgreSQL 物理候选已进入独立 Database Design Draft，但 Claim/Heartbeat/Takeover/Terminal SQL、索引、前向 Migration、legacy 回填和真实数据库时间测试仍未 Approved/实现；
