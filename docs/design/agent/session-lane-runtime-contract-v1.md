...
Ingress/Command Receipt 子契约使用独立 Corpus 和 Manifest，避免把入队事务与 Owner/Fence 状态迁移混成一个模型；详见 [`session-lane-ingress-command-contract-v1.md`](./session-lane-ingress-command-contract-v1.md)。PostgreSQL Header/Result、Turn/Run/Lease/HOL 和真实 PG 证据候选见 [`session-lane-postgresql-design-v1.md`](./session-lane-postgresql-design-v1.md)。Legacy Authority、72 个 blocker、Session-rooted/global-orphan anti-join、Ledger crash、Readiness 分层与 Down guard 另由 [`session-lane-legacy-upgrade-contract-v1.md`](./session-lane-legacy-upgrade-contract-v1.md) 的 107 条向量冻结。
...
4. PostgreSQL 物理候选已进入独立 Database Design Draft；legacy Authority/升级分类已进入 test-only Executable Draft，但 Receipt 不可变、created/accepted Event Retention、Turn Context、Claim/Heartbeat/Takeover/Terminal SQL、索引、前向 Migration、升级 Helper 和真实数据库时间测试仍未 Approved/实现；
5. Eino Runner/Cancel/Checkpoint wrapper 的装配、事件 drain、goroutine 泄漏与旧 Fence Set/Delete 拒绝测试；
6. Redis wake envelope、Scanner SQL/周期、Foundation/Lane/Processor/Claim generation 分层 Readiness 与完全不可用启动策略；
...
本批将 W2-R02 从纯文字 Draft 推进到 **Agent-owned Executable Draft**：核心状态、Lease/Fence 纪元、严格 HOL、Takeover、Cancel first-write-wins/非抢占、PG Scan trigger 和 Drain Handoff 候选已有共享 Corpus 约束；`enqueue_input_v1` 的独立逻辑候选见 Ingress 子契约；legacy upgrade 子契约另有 107 条向量。由于权威物理 Evidence、Turn Context、Event Retention、真实 PG、Processor 生命周期与第 6 节仍未关闭，本文不是 Review Ready 或 Approved，生产 Runner/Repository/Migration 和 `SMK-017/018` 均仍未完成。
