...
ResultCode Registry 必须逐 code 固定 effect class。R01 增量 Corpus 已接受 `permanent_failure_after_side_effects_resolved`：它只允许 `status=failed + retryable=false + cancellation_stage absent`，表示主业务目标确定失败，但 ledger 已有不可逆且全部 resolved 的副作用（例如 single-use Approval Consumption）。它不能用于 unknown、prepared/unresolved slot 或自动重试，也不能由通用 `permanent_failure` 临时替代。
...
Future Business Decision Authority 的 `outcome=not_committed` 可 resolve 一个 `effect_class=side_effect` 的 prepared slot，但它以正式 negative authority 证明该业务副作用未提交；该 slot ref 仍必须进入 failed Receipt 的内部 `result_refs`，却不计入“已提交副作用至少一个”的判断。若同一 ledger 另有 Consumption/Charge 等 `outcome=committed` 或天然不可逆 ref，则必须使用 `permanent_failure_after_side_effects_resolved`；否则可使用 pinned `permanent_failure`。R01 增量 Corpus 已固定这两种 outcome-aware 组合以及漏 ref/unknown 拒绝，但在 Business 公共权威契约和真实 PostgreSQL 事务完成前仍不得生产使用。
...
每个 `(tool_key, definition_version)` 必须冻结 execution slot Registry；`ref_slot/slot_ordinal/ref_type/ref_schema_version/authority_owner/query_contract/effect_class` 必须逐值命中 Registry，不能由模型或 Runner 临时扩展。`effect_class` v1 只允许 `side_effect/evidence_only`，失败与取消 evidence 必须按 pinned policy 分类，禁止用硬编码 `ref_type` 白名单猜测副作用；因此未来增加 `ApprovalConsumptionReceipt` 等类型也不能逃逸账本检查。`idempotency_key` v1 精确派生为 `tr:<receipt_id>:<ref_slot>:v1`。基础 Corpus 的七条与 failed-after 增量 Corpus 的三条 slot policy 都只是用于评审的代表性 pinned Definition，不等于六个 Tool 已完成全部生产 Registry。
...
证据追溯适用于全部状态，不只适用于 `accepted`：`waiting_user` 必须逐值命中 Approval slot，且 `card_id` 等于该 slot 的稳定 `projection_id`；`completed/partial` 的每个 Resource 必须与 resolved resource slot 形成 exact-set；`safe_failure_before_side_effect` 与 `cancelled/before_side_effect` 不得在 ledger 中存在任何 side-effect slot；普通 `permanent_failure` 不得隐藏已提交副作用，但允许且必须 exact 投影全部 `outcome=not_committed` negative authority；只有 pinned ResultCode 明确使用 `permanent_failure_after_side_effects_resolved` 的非重试 failed，以及 `cancelled/after_side_effects_resolved`，才允许至少一个已提交副作用，并必须携带全部已解决 side-effect slot 的内部 `result_refs` exact-set。`retryable=true` 的失败只允许在 ledger 证明无副作用时冻结。
...
- [`graph_tool_result_v1.json`](../../../agent/tests/contract/testdata/w2_r01/graph_tool_result_v1.json)：9 个合法向量（覆盖六状态、Unicode canonical edge、取消后副作用收口与副作用后永久失败）、41 个固定拒绝向量、Registry；
...
- [`tool_receipt_failed_after_v1.json`](../../../agent/tests/contract/testdata/w2_r01/tool_receipt_failed_after_v1.json)：3 个合法、8 个拒绝向量，固定 Consumption 已提交、Business `not_committed`、prepared unknown、漏 ref 与 outcome 严格校验；
...
- [`tool_receipt_failed_after_v1_corpus_test.go`](../../../agent/tests/contract/tool_receipt_failed_after_v1_corpus_test.go)：failed-after/outcome-aware Result evidence 纯状态模型。
...
- [x] 九个合法向量覆盖六状态、Unicode canonical edge、两种取消阶段与副作用后永久失败，并使用真实 UUIDv7/Result digest；
...
- [x] R01 Corpus 已增加 `permanent_failure_after_side_effects_resolved`、outcome-aware negative authority、合法 Consumption side-effect exact refs，以及遗漏/unknown/错误 outcome/effect-class 的拒绝向量；child 自身 Decision/slot exact-set Corpus 已由独立 1 个 fixture、172 条向量和 11 个目标测试固定，仍待跨角色联合评审；
