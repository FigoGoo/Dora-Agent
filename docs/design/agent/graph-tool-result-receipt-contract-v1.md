...
- [`manifest.json`](../../../agent/tests/contract/testdata/w2_r01/manifest.json)：绑定 87 条向量、Corpus 文件、三份设计源与两份 Validator 源的 raw SHA-256；
...
测试代码只存在于 `_test.go`，不会进入 `agent-service` 二进制。未来 TS 只读取同一 raw Corpus 验证投影/Reducer，不复制 Result 作为浏览器生产 Transport。当前 `validator_sources` 已阻止已登记校验器源码沿用旧摘要，但两份文件仍位于共享 `contract_test` 包；未绑定的新 `TestMain`、`init`、共享全局，以及未冻结的 `go.mod/go.sum`、toolchain/replace 或第三方依赖变化仍可能影响测试进程。因此它只关闭 D0-03 的文件级 source closure，不替代正式 Freeze 前的独立 stdlib-only 包或完整 package/build source exact-set。
...
- [ ] 正式 Review Freeze 前将 R01 Validator 隔离为独立且 stdlib-only 的 Go 包，或冻结完整 package/build source exact-set，排除未绑定 `TestMain`/`init`/共享全局和 dependency replacement 注入；
