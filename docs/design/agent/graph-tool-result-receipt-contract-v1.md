...
- [`manifest.json`](../../../agent/tests/contract/testdata/w2_r01/manifest.json)：绑定 87 条向量、Corpus 文件、三份设计源、`agent/tests/contract` 共享 `contract_test` 包全部 11 个直接 `.go` 源文件，以及 `agent/go.mod`、`agent/go.sum` 的 raw SHA-256；
...
优先关闭方案是把每个 Gate verifier 迁入独立、stdlib-only、无未登记 embed 输入的包；若保留共享包，则必须冻结完整 transitive build-input closure，包括 embed、内部依赖、第三方模块、toolchain 和执行 workflow/action trust root。共享包导致的 R01/R04 联动重审也是额外架构债。
