...
Future Business Decision Authority 的 `outcome=not_committed` 可 resolve 一个 `slot_effect_class=side_effect` 的 prepared slot，但它以正式 negative authority 证明该业务副作用未提交；该 slot ref 仍必须进入 failed Receipt 的内部 `result_refs`，却不计入“已提交副作用至少一个”的判断。若同一 ledger 另有 Consumption/Charge 等 `outcome=committed` 或天然不可逆 ref，则必须使用 `permanent_failure_after_side_effects_resolved`；否则可使用 pinned `permanent_failure`。这一 outcome-aware 规则同样需要 R01 Corpus 扩展后才可生产使用。
...
```text
result_code -> exact status -> retryable constant -> result_effect_policy -> cancellation_stage（仅 cancelled）
```

当前 R01 Corpus 的 ResultCode Registry JSON 为兼容既有候选仍使用字段名 `effect_class`；其语义名称固定解释为 `result_effect_policy`，不得与 execution slot 的 Registry-derived `slot_effect_class` 混用。是否在后续 Schema Version 重命名字段必须单独评审，不能原地改写 v1 Corpus。
...
每个 `(tool_key, definition_version)` 必须冻结 execution slot Registry；`ref_slot/slot_ordinal/ref_type/ref_schema_version/authority_owner/query_contract/slot_effect_class` 必须逐值命中 Registry，不能由模型或 Runner 临时扩展。`slot_effect_class` v1 只允许 `side_effect/evidence_only`，由 retained Registry entry 派生而不在 Execution Slot JSON、数据库列或命令中重复携带；失败与取消 evidence 必须按 pinned policy 分类，禁止用硬编码 `ref_type` 白名单猜测副作用。因此未来增加 `ApprovalConsumptionReceipt` 等类型也不能逃逸账本检查。`idempotency_key` v1 精确派生为 `tr:<receipt_id>:<ref_slot>:v1`。Corpus 中出现的七条 slot policy 是用于评审的代表性 pinned Definition，不等于六个 Tool 已完成全部生产 Registry。

当前 `plan_creation_spec` 的 `approval.primary` 与 `consumption.primary` 分别出现在 root/child 语义的独立 Receipt fixture，代表性 policy 都使用 `slot_ordinal=1`。在 Owner 明确 ordinal 是按 Definition 全局唯一、按 Receipt 重置，还是按 execution segment 分域之前，这两项不得被合并解释为同一完整 Registry，也不得自行把 Consumption 改成 ordinal 2。该范围与 `consumption.primary` 名称一并由 `R01-D05` 裁决。

`prepared` 时禁止出现 authority ref 或 resolved digest。外部响应或查询确认后，原 slot 一次性写入：

```text
authority_id + authority_version + authority_semantic_digest + authority_outcome（仅条件适用时）
```
...
仅有 `execution_refs` 而没有调用前 slot 预留无法区分“未发送”和“已提交但响应丢失”，因此不允许用于任何跨事务或外部调用。

Agent 本地 authority 可使用唯一的 direct-resolved 例外：authority 必须由同一 Agent PostgreSQL Owner 管理，并在一个短事务的受信快照中严格验证为既有不可变记录，或与 resolved execution slot 在该事务内原子创建；中间不得出现 RPC、模型、Redis、对象存储或其他**外部** unknown-outcome 窗口。该事务仍必须锁定 open parent Receipt、校验当前 Fence/expected version、命中 retained slot Registry、使用相同 `tr:<receipt_id>:<ref_slot>:v1` 身份并一次写入完整 authority ref 与 resolved digest；同义重放只读、异义冲突。Approval Decision evidence 可引用同事务锁定并验证的既有不可变 Decision，Approval Consumption Core 则必须与 slot 原子创建/重放；Business Decide、Charge、Operation、Dispatch 等跨边界副作用严格禁止采用。

direct-resolved 只消除“authority 已提交但 slot 未追加”的部分提交窗口，不能消除 PostgreSQL commit 成功但响应丢失。该事务返回 unknown 时 child 必须保持 open/quarantined，恢复方按原 child/slot/idempotency key 同时查询 local authority 与 resolved slot；同义完整记录只读收敛，冲突/单边记录视为损坏，`NOT_FOUND` 在真实 PG 事务模型证明 late commit 不可能前仍不得换 key、重发或冻结为未提交。
...
stateDiagram-v2
    ...
    open --> open: atomically create local authority and resolved slot / version+1
...
- 把跨事务/外部 authority 伪装成 direct-resolved local slot；
...
| `TR-N14/N15` | reserve/resolve 同义 slot 重放夹带另一命令类型字段 | `INVALID_TOOL_RECEIPT` |
...
- [`manifest.json`](../../../agent/tests/contract/testdata/w2_r01/manifest.json)：绑定 87 条向量、Corpus 文件、三份设计源、独立 `agent/tests/contract/w2r01` package 的六个直接 `.go` 源文件，以及 `agent/go.mod`、`agent/go.sum` 的 raw SHA-256；
...
- [`tool_receipt_v1.json`](../../../agent/tests/contract/testdata/w2_r01/tool_receipt_v1.json)：11 个 prepared/resolved/frozen/replay 正向迁移、15 个负向迁移、13 个跨状态 evidence 向量和 7 条 pinned slot policy；命令 shape 在同义 slot 重放前校验，禁止把额外动作夹带进只读幂等路径；
- [`graph_tool_result_v1_corpus_test.go`](../../../agent/tests/contract/w2r01/graph_tool_result_v1_corpus_test.go)：递归重复 key、严格解码、Canonical、Registry、状态矩阵和 Result digest；
- [`tool_receipt_v1_corpus_test.go`](../../../agent/tests/contract/w2r01/tool_receipt_v1_corpus_test.go)：slot/ref digest、Receipt CAS 纯状态模型和 Result evidence 追溯。

运行：

```bash
GOWORK=off /Users/figo/sdk/go1.26.3/bin/go -C agent test ./tests/contract/w2r01 -count=1
GOWORK=off /Users/figo/sdk/go1.26.3/bin/go -C agent test -race ./tests/contract/w2r01 ./tests/contract -count=1
```

R01 校验器已经迁入独立 `w2r01_test` package，不再与根 `contract_test`、R04 或 Session Lane 测试共享编译单元。普通 `.go` 承载纯 evaluator 与 R03 所需的窄只读 API，顶层 Test 只存在于两份 `_test.go` wrapper；整个 package 位于 `agent/tests`，生产 `cmd` 不得导入。未来 TS 只读取同一 raw Corpus 验证投影/Reducer，不复制 Result 作为浏览器生产 Transport。当前 `validator_sources` 必须与该独立 package 六个直接 `.go` 文件形成 exact-set，并逐文件绑定 raw SHA-256；新增、删除或替换 `init`、共享全局或任意直接源码都会使门禁失败。Execution-shape 校验继续拒绝 modern/legacy build constraint、package-level `TestMain`、Go 忽略文件名和 GOOS/GOARCH 平台选择文件。`validator_build_sources` 继续 exact-set 绑定 `agent/go.mod`、`agent/go.sum`，拒绝 `replace`，Git transition 要求登记输入均为普通 `100644 blob`。

迁移已拆除 `//go:embed`、同 Module `internal` 依赖和 `github.com/google/uuid`；UUIDv7 改为标准库字节级校验。R01 仍有唯一受管例外 `golang.org/x/text/unicode/norm`，因为标准库不提供 NFC。当前 R01 manifest 有意不声明 `validator_build_closure`：真实 module zip+h1 provenance、固定环境下 `transform`/`unicode/norm` selected-source resolver、锁定 Go 1.26.3 的 `go list`/typecheck/compile attestation，以及 base-owned workflow/action trust root 尚未接入。故 `GOV-D06` 和 `W2_R01_VALIDATOR_BUILD_CLOSURE_PENDING` 继续失败关闭，不能把 package 隔离误报为完整构建闭包。

R03 不再调用 R01 未导出的内部函数，只能通过 `EvaluateApprovalContinuationParentReceiptV1` 输入完整 parent Receipt 与 Graph Result corpus raw bytes，并消费其最小投影；R03 manifest 与 20 条向量保持字节不变。后续批次只补齐上述 x/text resolver、toolchain/compile 与 workflow/action 信任根，不回退到共享包或自写不完整 NFC。
...
- [x] R01 Validator 已迁入独立 `w2r01_test` package，冻结六个直接源码及 `agent/go.mod/go.sum`，拆除 embed、同 Module internal 与 google/uuid 依赖，并拒绝 build constraint、忽略/平台选择文件名、package-level `TestMain` 与显式 `replace` 注入；
- [ ] 正式 Review Freeze 前为唯一 `x/text/unicode/norm` 例外接入 module zip+h1 provenance 与 selected-source resolver，并冻结受信 toolchain compile attestation、CI workflow/action digest；完成前不得新增 `validator_build_closure` 或移除 R01 blocker；
