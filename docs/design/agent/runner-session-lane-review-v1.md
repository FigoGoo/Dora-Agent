...
4. 首轮 fresh guard 校验 Definition/Schema、Approval effective guard、current scope/resource/target/activation policy；失败且 Business slot 从未 prepared/发送时，以 Decision-only stable failed Result 冻结；
5. approve 用 `(approval_id, consumption_key)` first-write-wins 写/replay unsigned Consumption Core，并在同一 Agent 事务 append Consumption ref；reject 严格禁止 Consumption；
...
- [ ] `DecideCreationSpecCandidate(approve)` 已有 test-only strict union/Consumer 向量，但正式请求仍须同时携带不可变 Decision 与安全 Owner 已批准、Business 可验证的 Consumption envelope/query binding，并完成 Business producer parity；任一缺失/不符失败关闭；Storyboard/Prompt/Assembly Plan 分别评审；
- [ ] `DecideCreationSpecCandidate(reject)` 已有 test-only strict absence 向量，但正式请求仍须只携带不可变 reject Decision；出现 Consumption 必须拒绝且不得消耗 Approval；其他 Graph Tool 分别评审；
