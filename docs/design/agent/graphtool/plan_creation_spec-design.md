...
- `creation_spec_activation`：仅 approved 先创建唯一 Consumption Receipt，再按 Approval 绑定的 Candidate ID/version/digest 调 `BIZ-AIGC-008`；Approval 保持 `approved`。rejected 不创建 Consumption Receipt，直接依赖不可变 Decision Receipt 和 `BIZ-AIGC-008` 的 `approval_id + decision_version` 幂等拒绝候选。expired/cancelled 的产品语义冻结前不得实现。
...
- Consumption Receipt 共同键为 `(approval_id, consumption_key)`；本 Tool 的 `consumption_key` 由 `decision_version + effect_kind` 稳定派生。digest 绑定 source/parent Receipt、主体、scope/action、definition/schema version 以及 execution 或 Candidate version/digest。同键同 digest返回原回执，异 digest 返回 `IDEMPOTENCY_CONFLICT`。
...
| CreationSpecRevision / Business | Business PostgreSQL | `reviewing` | Approval approved 后调用 `BIZ-AIGC-008 DecideCreationSpecCandidate(approve)` | Agent Continuation→Business | 校验 actor/action、Candidate ID/version/digest、decision version、Consumption binding；CAS 激活 | `active` | 当前业务版本可用；同键重放 | Business 单事务；`approval_id + decision_version` | Continuation fence metadata；Candidate CAS/version；激活 Outbox | mismatch→确定失败；unknown→open/recovery_pending 查询同键 |
| CreationSpecRevision / Business | Business PostgreSQL | `reviewing` | Approval rejected 后调用 `BIZ-AIGC-008 DecideCreationSpecCandidate(reject)` | Agent Continuation→Business | 校验 actor/action、Candidate ID/version/digest、decision version；禁止 Consumption；CAS 拒绝 | `rejected` | 终态；同键重放 | Business 单事务；`approval_id + decision_version` | Continuation fence metadata；Candidate CAS/version；拒绝 Outbox | mismatch→确定失败；unknown→open/recovery_pending 查询同键 |
...
- Approval Consumption：共同键 `(approval_id, consumption_key)` first-write-wins，其中本 Tool 的 `consumption_key=derive(decision_version,effect_kind)`；仅 approved 创建回执，同键同 digest 重放、异 digest 冲突。rejected 只使用 Decision Receipt 和 Business 幂等，不创建 Consumption Receipt。
...
- Activation decision：`approval_id + decision_version`。`BIZ-AIGC-008` 必须支持同 key 重放或提供权威查询；在该契约冻结前不得实现 `get_creation_spec_decision_receipt`。
