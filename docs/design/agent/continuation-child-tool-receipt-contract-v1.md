...
> 状态：Agent-owned Design Draft / Corpus Pending / 未 Approved
>
...
>
> Corpus 状态：pending；第 12 节只是后续独立 Corpus 的必要条件，当前不声称已有 executable child evidence。
...
| 逻辑角色 | approve | reject | `slot_effect_class` | authority | 规则 |
|---|---:|---:|---|---|---|
...
`approval_decision` 与 `approval_consumption` 都必须遵循 R01 的 Agent-local resolved-slot 规则：Decision 只能在同一 Agent PostgreSQL 事务的受信快照中严格验证既有不可变 authority 后 append；Consumption 必须在同一事务原子创建/重放 Core 与 append ref。两者都不经过外部 RPC，因此不需要外部 prepared slot；但 PostgreSQL commit 响应丢失仍是 unknown，必须按原 child/slot/key 同时查询 authority/core 与 ref，`NOT_FOUND` 在 late commit 排除前保持 open/quarantined。任一跨事务或外部 authority 仍必须先 prepared。

pre-Business determined failure 的 terminal slot exact-set 单独冻结，不能与正常 approve/reject 成功集混用：

- approve 只有在 Consumption Record 已权威确认 exact key 与 approval-only backstop **都不存在**，随后在首次写入前因 eligibility/expiry/revalidation 确定失败时，才允许 `approval_decision` only；`approval_consumption/business_decide` 严格不存在；
- exact key 命中且 stored core 完整、scope 已授权、request digest 相同，不是“消费失败”：必须重放该 Core，并在同一 Agent 事务 append `approval_consumption` ref，随后进入 post-Consumption fresh guard；
- exact-key digest conflict、approval-only single-use 命中、stored core/index 损坏、stored scope 拒绝或 Query expected-binding conflict 都证明或无法排除 Consumption 已存在，却没有可安全投影的当前 ref，因此不得冻结 Decision-only failed；child 保持 `open/recovery_pending`、Input `quarantined`，等待 Owner-approved 冲突 authority/人工处置。Record/Query transport unknown、unknown 后 `NOT_FOUND` 同样保持隔离；
- 在完成“两级索引都 absent”的权威判定前发生 schema/command/key/identity/causal/request 失败，也不能证明零 Consumption，必须隔离而不是冻结本地终态；
...
| ResultCode candidate | status | retryable | `result_effect_policy` | 必需内部 refs |
|---|---|---:|---|---|
...
`permanent_failure_after_side_effects_resolved` 是 R01 设计候选，当前 R01 Corpus 尚未接受；在 R01 增加正反向向量并重新达到 Review Ready 前，第 12 节 child Corpus 不得把 post-Consumption terminal 标成 executable evidence。
...
在发送 Business RPC 之前，Agent 必须以当前 Fence 原子保存一个 prepared execution slot。它必须复用 `graph-tool-result-receipt-contract-v1` 已冻结的通用 slot canonical，不得创建平行的 Business 专用 slot schema；候选字段 exact-set 为：
...
`slot_effect_class=side_effect` 由 immutable `(tool_key,definition_version,ref_slot)` 的 retained Tool Definition Registry 派生；它不是上述 Execution Slot 的序列化字段、数据库列或调用方命令字段。resolved-ref digest 通过 Tool/Definition/slot identity 传递绑定该 Registry policy。
...
4. `query_contract` 必须指向与该写方法配套的只读权威查询；为空时不得发送 Business 命令；`ref_slot/slot_ordinal/ref_type/ref_schema_version/authority_owner/query_contract` 必须逐值匹配 serialized slot，`slot_effect_class` 必须从未来 Approved 的 pinned Tool Definition Registry 唯一派生；
...
10. 严格解码的 Business 响应或 Query 必须逐值绑定同一 action、Decision、approve-only Consumption、Candidate baseline 和 request digest，才可用 `outcome=committed` 的正向 Business authority resolve 原槽；Agent 必须把该值投影为 authority ref 的条件必填 `authority_outcome`，使其进入 resolved-ref digest；
11. 确定未提交只能由同一 Business Decision Authority envelope 以 `outcome=not_committed` resolve 原槽，并同样把 `authority_outcome=not_committed` 写入 authority ref；`committed/not_committed` 是 authority 语义而不是第二套 slot 状态。在该 authority 存在前，任何 transport/RPC 错误都仍是 `resolution_state=prepared` + unknown；
...
```text
Decision determined
  -> resolve Consumption by existing-first Record/Query
     (only absent path executes first fresh eligibility guard and first-write)
...
- Record 只有在 exact 与 approval-only 两级索引都 absent 后，才执行首次写入的 fresh eligibility/expiry/revalidation；该 absent-only guard 确定失败可按 `APPROVAL_PRE_CONSUMPTION_NOT_ELIGIBLE` 冻结 Decision-only Result。Consumption 后的第二次 guard 失败按 Decision+Consumption determined failure 冻结。两者都要求 `business_decide` 从未 prepared/发送，且不得伪造 Business authority；
- exact-key 同 request 的合法存量必须重放并 append Consumption ref，不能映射为失败；exact digest conflict、approval-only single-use 命中、stored-invalid、stored scope 拒绝、expected-binding conflict，或任何尚未证明两级索引 absent 的前缀错误，都保持 `open/recovery_pending/quarantined`，不得用 Decision-only Result 隐藏可能已存在的 single-use 副作用；
- Record/Query transport error、超时、结果歧义，或 unknown 后 Query `NOT_FOUND` 一律保持 `open/recovery_pending`，不得冻结；在安全 Owner 冻结可投影的 conflict authority 之前，这些隔离分支只能核对原 key 或升级人工，不能换 key/Approval 继续；
...
-> CONSUMPTION_IF_APPROVE(existing-first; absent-only eligibility/revalidation)
-> REJECT_FRESH_GUARD -> PRE_BUSINESS_FRESH_GUARD
-> BUSINESS_PREPARED -> BUSINESS_POSITIVE_AUTHORITY
-> BUSINESS_NEGATIVE_AUTHORITY -> UNKNOWN_OUTCOME -> RESULT_PROJECTION
```

approve 的 `CONSUMPTION_IF_APPROVE` 必须遵循 R04 固定优先级：先查 exact key，再查 approval-only backstop；合法 exact replay 直接 append Consumption ref；只有两级索引都 absent 才按 pinned Definition/Schema availability → authenticated scope → Candidate resource/version/digest → target exact-set → Approval effective guard → activation policy 执行首次写入重验。absent-only 重验失败才允许 Decision-only terminal。reject 不创建 Consumption，使用 `REJECT_FRESH_GUARD` 执行同序 guard 与 rejected Decision binding。approve 在 Consumption append 后、Business prepare 前还必须执行 `PRE_BUSINESS_FRESH_GUARD` 的同序再验证，包括 Definition/Schema revoke；此时 drift 走 Decision+Consumption determined failure，不能进入 Business。reject fresh guard 已紧邻 prepare，不重复消费相关阶段。合法 negative authority 的判断先于 transport unknown，不能因同次观察还带网络错误而丢弃确定未提交证据。
...
| `CONSUMPTION_IF_APPROVE` | absent-only eligibility/revalidation：`APPROVAL_PRE_CONSUMPTION_NOT_ELIGIBLE`；exact digest conflict / single-use / stored-invalid / scope / binding conflict 保持隔离，public conflict authority 待 Owner 冻结 |
| `REJECT_FRESH_GUARD` | `APPROVAL_PRE_CONSUMPTION_NOT_ELIGIBLE` |
...
9. approve Record 权威证明 exact/backstop 都 absent，随后首次写入 guard 失败：Decision-only frozen failed，零 Consumption、零 Business slot；
...
12. Consumption exact-key 同 request replay：append 原 Consumption ref，再执行 post-Consumption guard，不产生第二条 core；
13. Consumption exact digest conflict、approval-only single-use、stored-invalid、stored scope 拒绝或 expected-binding conflict：保持 open/recovery_pending/quarantined，绝不冻结 Decision-only；同点 transport unknown/Query `NOT_FOUND` 也保持隔离；
14. Consumption 后 Definition/Schema revoke：以 `APPROVAL_POST_CONSUMPTION_NOT_ELIGIBLE` 非重试 failed，Decision+Consumption refs exact-set。
15. Consumption Core + child ref 原子事务响应丢失：按原 child/slot/key 查询并收敛同一记录；`NOT_FOUND` 或 core/ref 单边记录保持 open/quarantined，不生成第二个 key/Receipt。
...
- Business positive/negative envelope 缺 `authority_outcome`、outcome 与 action/request 不匹配，或 resolved-ref digest 未绑定 outcome；
...
这些测试只能证明候选 canonical 和纯状态机。没有真实 PostgreSQL 唯一约束、事务故障注入、公共 RPC 与跨 Module owner 证据时，不得称为 child ToolReceipt 生产能力。
...
在这些条件全部满足前，本文件结论保持：**Design Draft / Corpus Pending，不批准生产 schema、签名、IDL 或 Runtime，不解锁任何 SMK。**
