...
测试模型中的 `allocated_*_id`、`committed_tick`、`fail_at` 是确定性分配和 crash-point 夹具，`trusted_authority` 是受信 Source fixture；它们都不是候选 HTTP/RPC DTO，禁止直接复制到外部协议。
...
- 保留现有全局 `session_command_receipt(command_id)` 真源；若扩展同表，按 `command_type` 建字段组 CHECK 并保持 Ensure legacy 行可验证；禁止另建无法跨表拒绝 CommandID 碰撞的孤立 Runtime Receipt；
