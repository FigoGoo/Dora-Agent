...
| `authority_owner` | `varchar(64)` | 否 | 是 | `business.authorization` 或仅供 legacy chat 的 `agent.legacy_authority` |
...
当前独立 test-only 子批已新增 [`session_message_set_v1_corpus_test.go`](../../../agent/tests/contract/session_message_set_v1_corpus_test.go) 与 [`w2_r02_turn_context/manifest.json`](../../../agent/tests/contract/testdata/w2_r02_turn_context/manifest.json)，用 3 个 fixture、32 条 exact vector 和 11 个目标测试冻结 `session_message_set.full_array.v1`：空集合、Unicode 强类型正文重编码→content digest、User→Assistant ToolCall→Tool Result→Assistant、六 Tool key、256/257 上限、cutoff 后消息隔离、物理顺序 canonical 化、逐字段 sensitivity、运维元数据排除以及 schema/session/cutoff/leaf/order/Tool 因果/digest 失败关闭。该证据不包含 58 字段 Context golden，也不替代 Owner Ref、Ingress/Receipt/legacy 绑定或真实 PostgreSQL 测试。
...
Message Set full-array test-only Corpus 已形成首批可执行证据，但它只冻结推荐候选的数组、Tool 因果和摘要边界；Turn Context 58 字段 Corpus、真实 DDL/Store/Runner 与 Owner Approval 仍不存在。
