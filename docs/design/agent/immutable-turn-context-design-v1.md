...
| `authority_owner` | `varchar(64)` | 否 | 是 | `authenticated_user` 固定 `business.authorization`；Approval 固定 `agent.approval_store`；Batch 固定 `agent.operation_store`；legacy chat 固定 `agent.legacy_authority` |
...
| `message_set_schema_version` | `varchar(64)` | 否 | 是 | 推荐候选固定 `session_message_set.full_array.v1` |
| `message_set_digest` | `char(64)` | 否 | 是 | 第 6 节 canonical 的 domain-separated SHA-256 |
| `event_cutoff_seq` | `bigint` | 否 | 是 | 锁定 Event Counter 后冻结的 Event 边界，必须 `>=0`；History Summary 和事件派生证据不得越过 |

在 `full_array.v1` 中，User/Assistant/Tool 都占用同一稠密 `message_seq`，因此 `message_count = message_cutoff_seq`；History Summary 只是额外引用，不替代或过滤数组。未来若引入可见性过滤、非稠密序号或其他物化模式，必须使用新的 schema/domain 并重新评审 CHECK，不能改变 v1 等式。

### 5.3 History Materialization 条件组

| 字段 | 候选类型 | Null | 进入 Context digest | 语义与约束 |
|---|---|---:|---:|---|
| `history_summary_present` | `boolean` | 否 | 是 | 是否使用不可变 History Summary；原始 Message/Event 仍是真源 |
| `history_summary_schema_version` | `varchar(64)` | 条件 | 是 | Summary 强类型 canonical schema |
| `history_summary_algorithm_version` | `varchar(64)` | 条件 | 是 | 摘要/压缩算法和参数的稳定版本 |
| `history_summary_owner` | `varchar(64)` | 条件 | 是 | 固定 `agent.history_materialization` |
| `history_summary_ref` | `varchar(255)` | 条件 | 是 | Agent History Materialization Store 发布的不可变版本引用 |
| `history_summary_digest` | `char(64)` | 条件 | 是 | Summary canonical digest，不摘要可变缓存或 Provider 原始响应 |
| `history_summary_message_cutoff_seq` | `bigint` | 条件 | 是 | Summary 覆盖的最大 Message Seq，必须 `0..message_cutoff_seq` |
| `history_summary_event_cutoff_seq` | `bigint` | 条件 | 是 | Summary 覆盖的最大 Event Seq，必须 `0..event_cutoff_seq` |

`history_summary_present=false` 时其余七项必须以 JSON `null` 进入 Context canonical；为 `true` 时必须全非空、引用可解析、摘要一致且两个来源 cutoff 不越过 Context。Summary 不能覆盖或删除原始 Message/Event，也不能吸收 Context cutoff 之后的事实。本组是 [`immutable-turn-context-decision-review-v1.md`](./immutable-turn-context-decision-review-v1.md) TC-P02 的推荐候选，仍未 Approved。

### 5.4 固定 Snapshot 和执行策略
...
| `skill_snapshot_owner` | `varchar(64)` | 否 | 是 | 固定 `agent.session_skill_snapshot` |
| `skill_snapshot_ref` | `varchar(255)` | 否 | 是 | Session Skill Snapshot Header 不可变引用 |
| `skill_snapshot_digest` | `char(64)` | 否 | 是 | 复用 Session Skill Snapshot set digest，不重算另一套 Skill 语义 |
| `prompt_bundle_owner` | `varchar(64)` | 条件 | 是 | 固定 `agent.prompt_registry` |
...
| `tool_registry_owner` | `varchar(64)` | 否 | 是 | 固定 `agent.tool_registry` |
...
| `runtime_policy_owner` | `varchar(64)` | 否 | 是 | 固定 `agent.runner_policy` |
...
| `model_route_owner` | `varchar(64)` | 条件 | 是 | 固定 `agent.model_gateway` |
...
| `budget_snapshot_owner` | `varchar(64)` | 否 | 是 | 固定 `agent.budget_policy` |
...
| `access_scope_owner` | `varchar(64)` | 否 | 是 | 固定 `agent.access_snapshot`；其输入来自 Business Authorization |
...
### 5.5 Continuation/Approval 条件组

| 字段 | 候选类型 | Null | 进入 Context digest | 语义与约束 |
|---|---|---:|---:|---|
| `continuation_present` | `boolean` | 否 | 是 | 普通模型 Turn 为 `false`；Approval Continuation 为 `true` |
| `parent_tool_receipt_owner` | `varchar(64)` | 条件 | 是 | 固定 `agent.tool_receipt` |
| `parent_tool_receipt_ref` | `varchar(255)` | 条件 | 是 | 原 ToolReceipt 稳定引用 |
| `parent_tool_receipt_digest` | `char(64)` | 条件 | 是 | 原 ToolReceipt frozen semantic digest |
| `parent_request_semantic_digest` | `char(64)` | 条件 | 是 | 原请求语义摘要，Continuation 不得改写 |
| `approval_owner` | `varchar(64)` | 条件 | 是 | 固定 `agent.approval_store` |
...
| `pinned_tool_owner` | `varchar(64)` | 条件 | 是 | 固定 `agent.tool_registry` |
...
为支持推荐的 `user/assistant/tool` append-only 模型历史，按 `message_seq ASC` 编码 exact-set leaf 候选：

```json
{
  "message_id": "uuidv7",
  "message_seq": 1,
  "origin_turn_id": "uuidv7",
  "role": "user",
  "content_schema_version": "message.user_text.v1",
  "content_digest": "lowercase-sha256",
  "source_kind": "authenticated_user_input",
  "source_id": "uuidv7",
  "tool_calls": [],
  "tool_result": null
}
...
- `user + message.user_text.v1 + authenticated_user_input`：`tool_calls=[]`、`tool_result=null`；
- `assistant + message.assistant_text.v1 + model_receipt`：`tool_calls=[]`、`tool_result=null`；
- `assistant + message.assistant_tool_call.v1 + model_receipt`：`tool_calls` 非空，每项固定 `tool_call_id/ordinal/tool_key/request_digest`，ordinal 从 1 稠密递增；`tool_result=null`；
- `tool + message.tool_result.v1 + tool_receipt`：`tool_calls=[]`；`tool_result` 固定 `tool_call_id/assistant_message_seq/tool_key/tool_receipt_owner/tool_receipt_ref/tool_receipt_digest`；
- 每个 Tool Result 必须逐值匹配同 `origin_turn_id`、更早 Assistant Message 的唯一 ToolCall；每个 call 恰好一个 Result，跨 Turn、乱序、孤儿、重复或未完成配对均拒绝；
- System Prompt、Skill、Approval Continuation 和 History Summary 都不伪造为 Message leaf。
- `full_array.v1` 的推荐上限为 256 条 Message；256 合法、257 失败关闭。该上限仍需数据/产品/Runtime Owner 结合 Summary 策略和真实 PostgreSQL Explain 签字。

`content_digest` 摘要对应角色的强类型内容 canonical；正文、Tool 参数或结果不直接进入 Message Set canonical。角色内容 DTO、大小边界和敏感数据策略仍需独立审核，本文只冻结 Message Set 所需的因果引用候选。
...
```text
SHA-256("dora.session_message_set.full_array.v1" || 0x00 || canonical_json)
```
...
TC-P03 已推荐 v1 使用完整有界数组摘要。prefix-chain 只允许作为新的 schema/domain v2 另行评审，禁止与 `full_array.v1` 混用。该推荐仍未 Approved，因此不得冻结生产字段和 Migration；test-only Corpus 可以先提供 full-array golden 与性能上限评审输入。

### 6.4 模型可见历史推荐候选与剩余缺口

当前生产表只允许 User Message，而 Runner 设计要求后续模型读取版本化的 Assistant/Tool 历史。TC-P02 已推荐扩展 `session_message` 为受控的 `user/assistant/tool` append-only 历史，并用第 5.3 节不可变 History Materialization Snapshot 处理有界摘要；System Prompt、Skill 与 Continuation 继续通过独立不可变引用注入。

该推荐仍未 Approved。角色内容 canonical、Assistant 多 ToolCall 的冻结时点、Tool Result 大小/脱敏、Summary 算法与保留期、legacy User-only 回填和真实 PostgreSQL 查询上限仍需 Owner 签字；test-only Corpus 可以冻结本节候选，不得据此宣称生产已支持正确的全功能多轮冒烟。
...
11. `message_set_schema_version`
12. `message_cutoff_seq`
13. `event_cutoff_seq`
14. `message_count`
15. `message_set_digest`
16. `history_summary_present`
17. `history_summary_schema_version`
18. `history_summary_algorithm_version`
19. `history_summary_owner`
20. `history_summary_ref`
21. `history_summary_digest`
22. `history_summary_message_cutoff_seq`
23. `history_summary_event_cutoff_seq`
24. `skill_snapshot_schema_version`
25. `skill_snapshot_kind`
26. `skill_count`
27. `skill_snapshot_owner`
28. `skill_snapshot_ref`
29. `skill_snapshot_digest`
30. `prompt_bundle_owner`
31. `prompt_bundle_ref`
32. `prompt_bundle_digest`
33. `tool_registry_owner`
34. `tool_registry_ref`
35. `tool_registry_digest`
36. `runtime_policy_owner`
37. `runtime_policy_ref`
38. `runtime_policy_digest`
39. `model_route_owner`
40. `model_route_ref`
41. `model_route_digest`
42. `budget_snapshot_owner`
43. `budget_snapshot_ref`
44. `budget_snapshot_digest`
45. `access_scope_owner`
46. `access_scope_ref`
47. `access_scope_digest`
48. `continuation_present`
49. `parent_tool_receipt_owner`
50. `parent_tool_receipt_ref`
51. `parent_tool_receipt_digest`
52. `parent_request_semantic_digest`
53. `approval_owner`
54. `approval_ref`
55. `approval_digest`
56. `pinned_tool_owner`
57. `pinned_tool_ref`
58. `pinned_tool_digest`
...
当前独立 test-only 子批已新增 [`session_message_set_v1_corpus_test.go`](../../../agent/tests/contract/session_message_set_v1_corpus_test.go)、[`session_turn_context_v1_corpus_test.go`](../../../agent/tests/contract/session_turn_context_v1_corpus_test.go) 与 [`w2_r02_turn_context/manifest.json`](../../../agent/tests/contract/testdata/w2_r02_turn_context/manifest.json)，联合 manifest 绑定 8 个 fixture、70 条 exact vector 和 27 个目标测试：

- Message Set 使用 3 个 fixture、32 条 vector 冻结 `session_message_set.full_array.v1` 的空集合、Unicode 强类型正文重编码→content digest、User→Assistant ToolCall→Tool Result→Assistant、六 Tool key、256/257 上限、cutoff 后消息隔离、物理顺序 canonical 化、逐字段 sensitivity、运维元数据排除以及 schema/session/cutoff/leaf/order/Tool 因果/digest 失败关闭；
- Turn Context 使用 5 个 fixture、38 条 vector 冻结 chat 单消息、chat Tool pair 后追加当前 User Message、batch 空历史、batch History Summary 和 Approval Continuation 的 58 字段 exact-set、domain golden、Message/Event cutoff、Message Set 同 Session 绑定、chat 末条 Message 的 `origin_turn_id/source_id` 与当前 Turn/Input 绑定、Summary schema/algorithm exact token、Prompt/Model/Continuation 条件组、Owner/ref/digest 结构闭包、legacy Authority 分类边界、retry/resume/takeover/配置更新/后续 Message/Event 隔离、组合 Reason 优先级和 tamper 失败关闭；
- 条件字段在 canonical 中始终显式编码 JSON `null`，运行时 Trace、Request、Processor、Lease、Attempt、Checkpoint、读取时间和“当前配置”元数据不进入摘要。
- 当前 `resolved_refs`、Continuation 与 legacy 测试只冻结结构闭包、Owner token 和分类失败关闭，不声称已验证 Receipt→Approval→Tool Pin 的真实跨对象因果，也不把合成 legacy 分类检查当成 Attestation golden；这些必须由独立 Owner Corpus 和真实 Store 补齐。

该证据仍不替代真实 Owner Ref 解析、Ingress/Receipt/legacy 原子写入、PostgreSQL 约束/触发器、Runner 模型输入或端到端冒烟。
...
5. 由各 Owner 复核现有 test-only canonical Corpus，并补充独立实现的跨语言 golden；
