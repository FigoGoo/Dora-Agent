...
本目录承接 `docs/product/prd/` 和 `docs/design/00-11` 的产品与体验约束，只用于页面视觉和信息架构评审。当前 PRD 套件已确认 Done，本目录可作为前端需求映射和页面实现参考；正式字段以 API 和 AG-UI 契约为准。

本轮新增 Markdown 线框草图和 image2 生成的 PNG 草图，不修改前端代码、不修改 PRD 状态、不进入 React / Vue 等实现。用户端非工作台页面统一采用左侧导航 + 右侧内容结构；Agent 创作页采用顶部项目工具条 + 左侧故事板 + 中央预览 + 右侧对话结构。
