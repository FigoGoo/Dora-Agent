...
> 状态：Implemented / Verified；W1-F Addendum active
>
> 设计日期：2026-07-14
>
> 冻结日期：2026-07-14
...
本设计只关闭公开发现的只读门禁，不表示完整 Skill Market 或完整 `SMK-005/006/031` 已完成。跨发布者 QuickCreate 已在后续 [W1-F Public Market Binding v1](./w1-public-market-binding-v1.md) 中通过独立权限 basis 开放；本文保留的“只读/不得使用”措辞仅描述 W1-E 上线时的基线，不再代表当前能力。

## 2. W1-E 基线排除与 W1-F 后现状

W1-E 交付时不实现：

- 跨 Owner 的 Project Skill Binding、市场“立即使用”、安装记录或给已有 Project 追加 Skill；其中前两项仅针对 W1-E 基线，现已由 W1-F 对显式 QuickCreate v2 的 `public_market` user Skill 关闭，安装记录和已有 Project 追加 Skill 仍未实现；
...
“收藏”是消费者偏好，“启用/使用”是 Project Skill Binding，“最近使用”只能来自真实 Skill Invocation；查看详情或仅创建 Binding 均不能冒充已经使用。三者不能混成含义不明的 `skill_install`。W1-E 上线时的 Owner-only QuickCreate 已由 W1-F 双版本 Permission 取代：公开 Market Skill 可作为独立预选进入显式 QuickCreate v2，但不能伪装成 Owner Skill，也不能绕过 Business 最终权威校验。
...
W1-E 基线不显示可点击“立即使用”，页面只标记“基础预览”。W1-F 已在详情页增加“使用此 Skill”入口，并以独立 Market 预选恢复到首页显式 QuickCreate；它不把跨发布者 Skill 注入 Owner Picker，也不表示搜索、收藏、费用、指标或已有 Project 追加 Skill 已开放。
...
- W1-E 基线页面不出现无后端契约的可使用按钮；当前 W1-F CTA 必须只进入显式 QuickCreate v2，且登录恢复不得自动提交。
...
8. 读取 active 详情后由 Governor suspend，再让用户以陈旧选择提交 QuickCreate，断言精确 `PROJECT_SKILL_UNAVAILABLE`、非重试错误和九类创建事实零增量；
...
该门禁已于 2026-07-14 在 [W1-F Public Market Binding v1](./w1-public-market-binding-v1.md) 冻结、实现并由真实 Evidence 验证。Market Evidence 已升级为 v2，不再产出“active 跨 Owner 必须拒绝”的 v1 passed 结论。

W1-E 通过时，跨发布者“立即使用”尚需关闭的事项如下；现均由 W1-F 完成：
...
上述八项均已关闭；收藏、安装、搜索、指标和已有 Project 的 Binding Replace 仍不在本批范围。
...
- [x] W1-E 的 Owner-only 基线已由 W1-F 明确 supersede，当前只允许满足 `public_market` eligibility 的跨 Owner QuickCreate；
- [x] 文档没有把公开读取或 W1-F QuickCreate 夸大为收藏、安装、已有 Project 追加或完整市场完成。
