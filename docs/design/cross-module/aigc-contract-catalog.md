...
> 文档状态：完整生产契约 Draft；CreationSpec、Material Evidence、Storyboard 与 Prompt Development Preview 子集已冻结
>
> 契约版本：`aigc.contract.v1alpha1`
>
> 设计日期：2026-07-14
>
> 实现门禁：本文完整生产范围冻结为 `v1` 前不得宣称生产可用；[`plan_creation_spec` 第 0 节](../agent/graphtool/plan_creation_spec-design.md#0-v1-开发预览设计冻结2026-07-16)批准的无计费/无 Approval Preview RPC、Draft 表、Runner 与 Event 子集可以实现。

> Runner、PostgreSQL Session Lane、Receipt、Approval Continuation、Checkpoint 和 A2UI/Event 的完整生产契约详见 [Agent Runner 与 PostgreSQL Session Lane v1 设计评审](../agent/runner-session-lane-review-v1.md)。V1 只实现 Preview 设计冻结的 HOL/Lease/Fence/Receipt/Event 最小子集，不能据此关闭完整生产评审。
...
- 为 V2/V3 最小 Profile 评审与 P1 完整生产验收提供可追踪的契约编号；开发顺序只以[功能优先开发与试跑计划](../../requirements/full-function-smoke-development-plan.md)为准。
...
截至 2026-07-16 开工前，Agent 仅具备 W0/W1 Session/Input/Event 基础：`session_runtime_lease` 仍是空租约骨架；`session_input` 虽声明 `pending/claimed/running/retry_wait/resolved/dead`，生产路径只写入 `pending`。Turn、Run、ModelReceipt、ToolReceipt、Approval、Checkpoint、A2UI 后端、输入 Claim/处理器仍待实现；Eino `v0.9.10` 与 DeepSeek Adapter `v0.1.6` 已精确锁定，但尚未进入生产依赖图。前端遗留 `/api/aigc/**` A2UI 资产没有匹配的当前后端，不能被当成目标契约或兼容承诺。

后续实现必须遵守：

- 已锁定的 `github.com/cloudwego/eino v0.9.10` 与 DeepSeek Adapter 版本不得在 V1 业务实现中升级；
- 现有 W0 Migration 不得回改，所有新表、字段、约束和索引只允许使用向前 Migration；
- V1 只实现 Preview 设计已冻结的 HTTP/RPC/Event/DTO 和 Session Lane 子集；其余生产范围仍须先评审；
...
### 4.0 V1 Preview 已冻结 RPC

下列十个方法已进入 Business Owner 的 `foundation.rpc.v1` 单一 Thrift IDL。CreationSpec 方法使用 `creation_spec.preview.rpc.v1`，Material Evidence 方法使用 `asset_analysis_inputs.preview.rpc.v1`，Storyboard 方法使用 `storyboard.preview.rpc.v1`，Prompt Draft 方法使用 `prompt.preview.rpc.v1`。它们只服务开发预览，后续生产契约使用新方法/版本，不复用已发布字段号：

| 编号 | 方法 | 语义 | 幂等/恢复 |
|---|---|---|---|
| `BIZ-PREVIEW-001` | `GetCreationSpecContextPreviewV1` | 校验 User/Project Owner，返回 Project ID/version/title 最小上下文 | 只读；跨 Owner 与不存在统一 NOT_FOUND |
| `BIZ-PREVIEW-002` | `SaveCreationSpecDraftPreviewV1` | 保存 Validator 通过的 `creation_spec.draft.v1`，状态固定 `draft` | `command_id` first-write-wins；同 digest 重放，异 digest 冲突 |
| `BIZ-PREVIEW-003` | `QueryCreationSpecDraftCommandPreviewV1` | 查询原保存命令的权威结果 | 只接受原 command+request digest；not_found 不允许换键 |
| `BIZ-PREVIEW-004` | [`BatchGetAssetAnalysisInputsPreviewV1`](./material-analysis-evidence-preview-v1.md) | 单批授权读取本地 PostgreSQL 中已持久化的 text/image Evidence exact-set | 只读、单 SQL、完整响应；不存在/越权统一 NOT_FOUND，授权后检查可选版本 |
| `BIZ-PREVIEW-005` | `GetStoryboardPlanningContextPreviewV1` | 按可信 Ref 读取同 Owner/Project 的 CreationSpec Draft exact snapshot | 只读；ID/version/digest 任一不符失败关闭 |
| `BIZ-PREVIEW-006` | `SaveStoryboardDraftPreviewV1` | 保存隔离的 `storyboard.preview.draft.v1` JSON，状态固定 `draft` | `command_id` first-write-wins；同 digest 重放，异 digest 冲突；保存时复核 Project 与 CreationSpec Guard |
| `BIZ-PREVIEW-007` | `QueryStoryboardDraftCommandPreviewV1` | 查询原 Storyboard Preview 保存命令的权威结果 | 只接受原 command+request digest；not_found 不允许换键 |
| `BIZ-PREVIEW-008` | `GetPromptGenerationContextPreviewV1` | 按可信 Ref 读取同 Owner/Project 的 Storyboard Preview Draft 最小安全投影 | 只读；Source ID/version/digest/status/schema 任一不符失败关闭 |
| `BIZ-PREVIEW-009` | `SavePromptDraftPreviewV1` | 保存覆盖 Source 全部 Slot 的隔离 `prompt.preview.draft.v1`，状态固定 `draft` | `command_id` first-write-wins；同 digest 重放，异 digest 冲突；保存时复核 Project、Source 与目标全集 |
| `BIZ-PREVIEW-010` | `QueryPromptDraftCommandPreviewV1` | 查询原 Prompt Preview 保存命令的权威结果 | 只接受原 command+request digest；not_found 不允许换键或换正文重发 |

Preview 不调用 `PrepareBillableExecution`，不创建 Approval，不执行生产 Candidate 写入/决策，也不触发 Worker。`BIZ-PREVIEW-004` 不是 `BIZ-AIGC-002`，不支持上传、提取或 MaterialAnalysis 保存；`BIZ-PREVIEW-005..007` 不是生产 `BIZ-AIGC-009/010`，不创建 Storyboard Revision、稳定 Element/Slot、Diff 或激活状态；`BIZ-PREVIEW-008..010` 也不是生产 `BIZ-AIGC-011/012`，不创建 PromptArtifact/Revision、Approval 或 ready/active 状态。
...
| `AGT-PREVIEW-001` | CreationSpec Preview Input | 同源 BFF `POST /api/v1/agent/sessions/:session_id/creation-spec-previews`，CSRF + Owner + 内部签名；BFF 到 Agent 的 canonical POST 使用唯一精确 `Content-Type: application/json`，HTTP 只持久化并返回 202 |
| `AGT-PREVIEW-002` | CreationSpec Preview Projection | `creation_spec.preview.completed/failed` 使用严格安全 DTO；Snapshot 携带 nullable 最新 Draft 投影，SSE/Reset 共用 reducer |
| `AGT-PREVIEW-003` | Preview Session Lane 边界 | Processor 只消费 `creation_spec_preview`；非终态非 Preview 前驱存在时 Agent/BFF 稳定返回不可重试 `409 SESSION_LANE_BLOCKED` 且事务零增量。Preview-enabled QuickCreate 只把首页目标经前端进程内一次性交接到表单，服务端 `initial_prompt=null`；禁止跳 HOL、改型 legacy Input 或把目标写入浏览器持久化介质 |
...
下列编号只描述 P1 完整生产能力的依赖闭合关系，不得被解释为当前开发顺序或反向阻塞 V1～T1。V1 `plan_creation_spec.v1preview1` 继续执行文档顶部冻结的 Preview 例外；V2/V3 的最小 Profile 由唯一计划按阶段批准。

1. 完整生产公共契约依赖 Business、Agent、Worker 共同冻结 `v1` 字段、状态、错误码和 Owner；
2. 完整生产 Agent Runtime 依赖 `runner-session-lane-review-v1.md` 冻结 Session Lane、Input/Turn/Run、Receipt、Approval Continuation、Checkpoint、EventLog 和预算/取消规则；
3. 生产 Agent 运行基础依赖已审核 Eino 版本和向前 Migration；Preview 依赖例外不等于该依赖全部关闭；
4. 完整生产创作链依赖 Business 读契约、候选写入、计费与 Generation Preparation/Finalization；
5. 完整生产异步链依赖 Agent Approval、Operation/Batch/Job、Outbox/Inbox 和 Job Contract V1；
6. 生产 Worker 依赖 V1 Claim/Fence/Receipt/Finalize，旧消费者不得直写新表；
7. 六个 Graph Tool 的完整生产 Profile 依赖各自独立设计与上游契约 Approved；
8. 生产前端能力依赖版本化 Tool Catalog、Approval 和 A2UI EventLog/SSE；
9. P1 发布结论依赖单 Module 测试、三 Module 契约测试、unknown-outcome 故障注入和 35 条 SMK-P0 全功能冒烟。
...
当前完整生产结论：**待评审，不通过生产实现门禁。** 本目录及 `runner-session-lane-review-v1.md` 的完整生产范围均保持 Draft。六个 Graph Tool 可继续细化完整生产设计，但生产代码、Migration、IDL 和跨 Module Consumer 必须等待对应契约冻结为 `v1`。该结论不撤销文档顶部的 V1 Preview 例外，也不将 Preview 升级为生产完成。
