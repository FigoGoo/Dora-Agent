...
| `BIZ-AIGC-008` | `DecideCreationSpecCandidate` | Agent Continuation | 按已验证 Approval 激活或拒绝候选 | `approval_id + decision_version`；候选版本/digest Guard | 查询候选状态 |
...
| `BIZ-AIGC-010` | `DecideStoryboardCandidate` | Agent Continuation | 激活或拒绝 Storyboard 候选 | `approval_id + decision_version`；候选版本/digest Guard | 查询 Revision 状态 |
...
| `BIZ-AIGC-012` | `DecidePromptResults` | Agent Continuation | 批量批准、拒绝或部分批准固定目标集 | `approval_id + decision_version`；目标集合 digest Guard | 查询 Prompt Revision 集合 |
...
| `BIZ-AIGC-014` | `DecideAssemblyPlanCandidate` | Agent Continuation | 激活或拒绝 Assembly Plan | `approval_id + decision_version`；计划版本/digest Guard | 查询计划状态 |
...
| 编号 | 能力 | 约束 |
|---|---|---|
| `AGT-HTTP-001` | Tool Catalog | Definition Catalog 与 Executable Registry 分离：Graph 审核/编译前只返回六个稳定 key、顺序和 unavailable 原因；只有唯一可执行 Definition 已注册后才返回对应 Schema Version 和能力摘要 |
...
| `AGT-HTTP-004` | Approval Decision | 只接受已认证用户对指定 Approval 的结构化动作；CAS `approval_version` |
...
- `requested/approved/rejected/expired/consumed/cancelled` 状态及版本；
- 创建、到期、决策、消费时间；
- 决策者、来源 Card/Action Receipt、原 Run/Tool Call；
- 消费幂等键和一次性/可复用策略。

以下内容不是 Approval：自然语言中的“确认”、模型 Tool 参数、前端隐藏字段、过期 Card Action、只存在于 Redis 的标记。

一个已批准的 Approval 只能覆盖其冻结摘要和金额范围。资源、目标、价格或计划发生变化时必须创建新 Approval。消费 Approval 使用 CAS；失败时不得继续扣费或派发。

Agent 成功消费 Approval 后生成 `ApprovalConsumptionReceiptV1`，至少包含 Approval ID/version、消费键、用户/项目、动作、资源和目标摘要、Quote/金额摘要、消费时间及 Agent 审计签名。Business 仅在已认证的 Agent RPC 调用中接受该回执，并逐字段比对 `Prepare*` 请求；来自前端、模型或未认证调用方的同名字段一律无效。消费回执响应丢失时查询 Agent Approval 权威状态，不再次消费。
...
| 风险点 | 稳定键 | 权威判断 | 恢复动作 |
|---|---|---|---|
| Tool 重放 | `user_id + turn_id + tool_call_id` | Agent ToolReceipt | 返回原 Result 或从记录节点继续 |
...
| Approval 重复消费 | `approval_id + consumption_key` | Agent Approval CAS | 已消费则返回原消费结果 |
...
- Approval 的 `requested/approved/rejected/expired/consumed` 属于 Agent。
...
2. Business 先实现读契约、候选写入、计费与 Generation Preparation/Finalization；
3. Agent 实现 Approval、Operation/Batch/Job、Outbox/Inbox、Job Contract V1；
4. Worker 实现 V1 Claim/Fence/Receipt/Finalize，旧消费者不得直写新表；
5. 六个 Graph Tool 按依赖顺序接入；
6. 前端接 Tool Catalog、Approval 和 A2UI EventLog/SSE；
7. 运行单 Module 测试、三 Module 契约测试、unknown-outcome 故障注入和 35 条 SMK-P0 全功能冒烟。
...
当前结论：**待评审，不通过实现门禁。** 六个 Graph Tool 设计可以基于本草案继续细化，但生产代码、Migration、IDL 和跨 Module Consumer 必须等待本目录冻结为 `v1`。
