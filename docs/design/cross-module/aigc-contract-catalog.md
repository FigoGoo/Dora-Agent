...
> W2 同步模型计费的细化候选见 [W2-R00 Graph Execution Billing 跨模块契约 v1](./graph-execution-billing-contract-v1.md)。该文档仍为 Draft；下列 `BIZ-AIGC-003`～`BIZ-AIGC-005` 已按其双层幂等与终态边界收口，但不代表 W2-R00、IDL、Migration 或生产实现已经 Approved。
...
| `BIZ-AIGC-003` | `PrepareBillableExecution` | Agent | 对已批准 Policy 覆盖的 primary model logical execution 冻结计价并立即扣费，返回 CHARGED 或 immutable NOT_CHARGED Receipt；确定性 rule 默认不计费 | transport：Prepare ref slot 的唯一 `tr:<tool_receipt_id>:<ref_slot>:v1` command id + request digest；Business domain unique：`(logical_tool_execution_id, model_call_ordinal)` | `GetBillableExecutionReceipt` |
| `BIZ-AIGC-004` | `GetBillableExecutionReceipt` | Agent | 按原 Prepare/可选 Finalize command 查询 `NOT_FOUND/PREPARE_REJECTED/CHARGED/FINALIZED/CONFLICT`；裸 NOT_FOUND 只是 observation，只有 NOT_CHARGED Receipt 是确定零扣费 authority | 原 `tr:` command id/digest + execution identity + expected user/project binding | 本方法即权威查询；NOT_FOUND 只允许原 command/digest 有界重发 |
| `BIZ-AIGC-005` | `FinalizeBillableExecution` | Agent | 将已冻结 terminal ModelReceipt/outcome 绑定原 Charge；不再次扣费、不退款、不冲正、不生成或确认发布者收益 | 独立 Finalize ref slot 的唯一 `tr:<tool_receipt_id>:<ref_slot>:v1` command id + request digest；绑定原 Prepare Receipt、execution identity 与 terminal ModelReceipt | `GetBillableExecutionReceipt` |
...
| `BIZ-AIGC-008` | `DecideCreationSpecCandidate` | Agent Continuation | `action=approve` 激活候选时必须携带不可变 Decision Receipt 与已认证 Agent 生成的 `ApprovalConsumptionReceiptV1` 并逐字段验证；`action=reject` 只接受不可变 Decision Receipt且禁止 Consumption Receipt | Decision identity + action；approve 另校验 consumption key/digest；候选版本/digest Guard | 按 Decision/Consumption/候选幂等键查询候选状态 |
...
- W2 `PrepareBillableExecution` 只覆盖已批准的 primary model call。Provider/RPC Attempt 不进入计费唯一键；同一 logical ordinal 的 Query、恢复和投影重试复用原 Charge。
- Prepare Charge 与 Finalization 使用两个不同的 retained Execution Ref Slot/`tr:` command id；Billing `model_call_ordinal` 与 R01 从 1 开始的 Registry `slot_ordinal` 是独立序号，不得互相复制或推导。
- `GetBillableExecutionReceipt=NOT_FOUND` 不证明原请求永远未提交；只能在单一 Retry Owner 下用原 command/digest 有界重发。`PREPARE_REJECTED` 的 immutable NOT_CHARGED Receipt 才是稳定零扣费终态。
- `FinalizeBillableExecution` 只记录模型终态绑定与未来可核对的 attribution/earning disposition；W2 不生成或确认收益 Ledger，收益、结算和账务差错冲正留在 W4。
...
| 同步模型重复扣费 | transport 为 Prepare ref slot 的唯一 `tr:` command id + request digest；领域 backstop 为 `(logical_tool_execution_id, model_call_ordinal)` | Business Prepare Command/Charge Receipt | 查询原 Receipt；CHARGED 复用，NOT_CHARGED 保持零扣费终态，NOT_FOUND 只以原 command/digest 有界重发 |
...
| 同步模型 Provider 重复调用 | Stable Provider Request Key | Agent ModelReceipt、dispatch marker 与 Provider Query | marker 前可证明未进入 Adapter 才允许首次调用；marker 后查询或同 key 幂等恢复，无权威能力则 quarantine |
| Worker Provider 重复调用 | Provider Idempotency Key | Worker ProviderReceipt/Provider 查询 | 查询优先；只在证明未开始后重试 |
