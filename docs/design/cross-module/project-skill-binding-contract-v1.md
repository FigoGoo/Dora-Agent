...
>
> W1-F Addendum：[Public Market Binding v1](../business/w1-public-market-binding-v1.md)；冻结后仅取代本文 owner-only / 跨 Owner unavailable 结论，不改变 Bootstrap v2、Outbox、Agent wire 或历史 Snapshot 规则。
>
...
3. W1-B1 基线只允许 Project Owner 绑定自己拥有、已经发布且治理状态为 `active` 的用户 Skill；W1-F 只按独立 Addendum 开放 current-published + active 的 `public_market` 跨 Owner user Skill，系统 Skill、企业/团队共享和管理员代绑仍全部失败关闭。
...
### 1.2 W1-F Public Market Binding Addendum 覆盖关系

W1-F 不修改本文的 QuickCreate v2 HTTP variant、Binding Set、Resolution、加密 Outbox、Unknown Outcome、Agent IDL 或不可变 Session Snapshot。它只扩展 Business 内部权限派生：

- `project_skill_permission_snapshot.v1` 继续唯一对应 `owner_private + project-skill-permission:owner-private:v1`；
- 跨 Owner 公共 Skill 使用独立 `project_skill_permission_snapshot.v2 + public_market + project-skill-permission:public-market:v1`；
- Agent 仍只接收 `publisher_user_id` 与 opaque permission digest，不消费 schema/basis/policy ref；
- 本文 `PSB-DEC-003` 与 `PSB-BLK-001` 在 Addendum Frozen/Approved 前保持阻塞；冻结后仅对 current-published + active 的 user Skill 标记为被 W1-F 取代。

任何未被 Addendum 明确取代的 owner-safe、全量失败、锁、幂等、加密、限额、旧 Session 隔离与禁止公共 Tool 规则继续有效。
...
- 不定义通用管理员 RBAC、企业共享或系统 Skill 分发；W1-B1 当时未定义的跨用户公开 Skill 使用，现仅由 W1-F Addendum 对满足 eligibility 的 user Skill 关闭。
...
4. 用一个集合查询锁定/读取全部 enabled Binding、Skill 当前 Published Snapshot 与不可变 Definition；对 Skill/current pointer 取得 `FOR SHARE`，与治理写的 `FOR UPDATE` 串行化。
5. 对每项从可信 Owner 关系选择 v1 `owner_private` 或 v2 `public_market`，再执行权限、治理、Runtime Policy、Canonical 和 limits 校验；任一项失败时整个集合失败。
...
3. Skill 存在；若 `skill.owner_user_id == project.owner_user_id`，选择 v1 `owner_private`，否则必须满足 W1-F current-published + active public user Skill 条件并选择 v2 `public_market`。
...
### 6.3 Permission Snapshot v1/v2

W1 当前只允许两个不可交叉的版本对：资源所有权规则 `project_skill_permission_snapshot.v1 + owner_private + project-skill-permission:owner-private:v1`，以及 W1-F 公开市场规则 `project_skill_permission_snapshot.v2 + public_market + project-skill-permission:public-market:v1`。二者都不是通用 RBAC 或 bearer token；v1 字节与 digest 永久保持不变，禁止在 v1 内扩枚举。
...
v2 保持相同字段顺序，只将固定组合改为：

```text
schema_version = project_skill_permission_snapshot.v2
decision       = allow
basis          = public_market
namespace      = user
allowed_actions = ["session_snapshot"]
...
v1 要求 `subject_user_id == project_owner_user_id == skill_owner_user_id`；v2 要求 `subject_user_id == project_owner_user_id` 且 `skill_owner_user_id != project_owner_user_id`。两版的 Publisher 都固定为 Skill Owner，批准发布的 Reviewer 不得成为 Publisher。Business 必须根据可信数据库事实派生版本、basis 与 policy ref，浏览器不得提交这些字段；完整 v2 Canonical 与 golden 以 W1-F Addendum 为准。

以下能力 W1 明确不可用：

- 绑定未满足 W1-F current-published + active public eligibility 的其他用户 Skill；
...
| Skill 不存在，或未满足 v1 owner-private / v2 public-market eligibility | `PROJECT_SKILL_UNAVAILABLE` | 不泄露存在性 |
| Skill 未发布 | `PROJECT_SKILL_UNAVAILABLE` | 不退化为 Draft |
| suspended/offline | `PROJECT_SKILL_UNAVAILABLE` | 不泄露治理事实，不静默丢弃 Item |
...
W1-F 009 是独立的 metadata-only COMMENT Migration，其 Down 还必须在同一事务中按 QuickCreate 写入顺序对 Resolution Header/Item 取得 `SHARE` 表锁，再执行 public-market history guard。并发写事务先提交时，Down 必须观察到跨 Owner Publisher 并以 SQLSTATE `55000` 拒绝；Down 先取得锁时，新 Resolution 必须等待注释恢复完成。异常不得把 public-market 新注释部分恢复为 owner-private。
...
- owner-private 成功；W1-F 启用后 current-published + active 的 public-market 成功；其他 cross-owner、system/public Tool/RBAC/计费全部失败关闭；
...
| PSB-DEC-003 | W1-B1 permission | **W1-B1 关闭：仅 owner-private；W1-F Addendum 冻结后对 public-market 部分 superseded** | 未满足 W1-F eligibility 的跨 Owner Skill 仍不可用 |
...
| PSB-BLK-001 | 跨 Owner 市场 Skill 授权 | **由 W1-F Addendum 冻结后仅对 public-market user Skill 关闭** | system/team/admin 代绑仍不可用 |
...
仍禁止：Binding Replace HTTP、未满足 W1-F public-market eligibility 的跨 Owner Skill、system Skill、非空 Public Tool、Graph/Runner/Executable Registry、计费与历史 Session 治理执行。上述能力必须分别关闭对应阻塞并更新本契约或其明确 Addendum 后才可实现。
