...
> 状态：Frozen / W1 Foundation 实现基线；W1-D/W1-E/W1-F Addendum active
>
> 冻结日期：2026-07-14
>
...
- 企业用户类型不等于 Reviewer 或管理员；Reviewer 最小入口只允许按 [W1-C2 Reviewer RBAC 与 Skill 管理审核 v1](../business/w1-reviewer-rbac-admin-review-v1.md) 解析正式 `skill.review`。本文冻结时尚待独立设计的 `skill.govern` 已由 [W1-D Skill 治理](../business/w1-skill-governance-v1.md) Addendum 接线，不改变本基础契约的角色隔离原则；
...
Reviewer HTTP 只允许在 [W1-C2 Reviewer RBAC 与 Skill 管理审核 v1](../business/w1-reviewer-rbac-admin-review-v1.md) 的角色 Assignment、动态 Session Resolver、事务内 account/assignment 复核、Strong ETag 和专用审计回执全部接线后注册。W1-C2 仍只覆盖待审列表、冻结详情和批准发布；驳回/撤回仍未注册，暂停、恢复、下架已由独立 W1-D Governance API 注册，不能反向扩张 Reviewer capability。
...
公开 Skill 市场和详情已在 W1-E 真实发布与治理 capability 接线后注册：
...
1. 上述契约中的 `project_skill_binding` 状态、排序、CAS、owner-private 权限和 Feature Flag 门禁；W1-F 仅按 [Public Market Binding v1](../business/w1-public-market-binding-v1.md) Addendum 增加独立 Permission v2 public-market 路径；
