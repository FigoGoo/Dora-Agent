...
- 未命中可路由 Skill 时只进入文本模型兜底，事件 `agent.skill.missing` 必须标记 `fallback_mode=text_model` 且 `recommend_create_skill=false`；不得在兜底中向用户推销或建议创建 Skill。
...
- Tool：权限不足、停用、超时、幂等重试、非幂等不重试；Skill 版本中的 `tool_refs`/`tool_bindings` 只是声明，执行前必须逐 Tool 调用业务服务策略 RPC 复检白名单、风险和确认要求。
- 长任务：进度、取消、断线重连、快照恢复。
