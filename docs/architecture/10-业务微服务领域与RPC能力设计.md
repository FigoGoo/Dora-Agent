...
PRD 套件已经定义 Dora-Agent 第一版的账户空间、平台后台、模型、Tool、Skill、积分、资产、通知和作品中心能力。业务系统微服务需要在正式契约和代码开发前明确业务事实、权限、事务、RPC server 能力和测试映射，避免 Agent 服务或前端绕过业务边界。

当前 PRD 套件仍为 `product_status: Draft`，因此本文只作为设计草案和评审输入，不进入 Go 代码实现，不创建 migration，不直接覆盖 `docs/contracts/**`。
...
- 当前 PRD 套件仍为 Draft；正式开发前需要产品体验设计师完成 Done Gate，并由主控 Codex 确认契约和开发计划。
