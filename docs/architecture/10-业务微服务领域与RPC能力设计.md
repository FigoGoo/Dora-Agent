...
owner：业务服务责任域
更新时间：2026-06-28
适用范围：业务系统微服务领域边界、Kitex RPC server 能力清单、Agent 调用需求和契约前置映射的历史追溯
相关代码路径：`services/business/**`、`api/thrift/business_agent_service.thrift`、`tests/contract/**`、`tests/business/**`
相关契约：当前以 `docs/contracts/rpc/业务微服务RPC契约草案.md` 和 `api/thrift/business_agent_service.thrift` 为准；本文件不作为正式契约或新增开发入口
...
当前 PRD 套件已确认为 `product_status：Done`，第一阶段服务端开发已完成。本文保留为早期业务微服务设计追溯，不能作为业务微服务契约、Go 代码实现、migration 或 contract test 的当前工程输入；正式字段、方法和 fixture 以 `docs/contracts/**`、`api/thrift/business_agent_service.thrift` 和 `tests/contract/fixtures/**` 为准。
...
| --- | --- | --- | --- | --- |
| 当前空间决定 Skill、积分、资产、历史 | 空间上下文必须成为业务 RPC 的权限入口 | SpaceContext DTO、ResolveSpaceContext RPC | 业务服务责任域 | 空间权限 contract test、跨空间拒绝集成测试 |
| 项目承载创作资产和过程 | 项目是业务容器；会话和 run 在 Agent DB，项目事实和资产归属在业务 DB | ProjectService、ProjectAsset 关系、项目权限校验 | 业务服务责任域 | 项目权限、项目资产、跨空间拒绝 contract test |
| 个人注册后拥有个人空间 | 注册必须原子创建用户、个人空间和个人积分账户 | AccountSpaceService、注册事务 | 业务服务责任域 | 注册幂等、默认空间、默认积分账户测试 |
| 企业注册、邀请、移除、转让 | 企业成员和角色是业务服务事实源 | EnterpriseService、preview/confirm 写操作 | 业务服务责任域 | 成员权限、转让原子性、重复邀请测试 |
| 平台管理员独立后台 | 管理员身份和普通用户身份隔离 | AdminService、AdminAuditLog | 业务服务责任域 | 后台登录态隔离、审计脱敏测试 |
| 用户禁用启用需确认和原因 | 禁用影响登录和企业空间访问，不删除公开作品 | UserAdminService、SetUserStatus preview/confirm | 业务服务责任域 | 禁用确认、禁用后登录拒绝、审计测试 |
| 模型配置和用户积分单价 | 业务服务保存模型目录、默认模型和价格 | ModelConfigService | 业务服务责任域 | 模型停用、默认模型保护、价格缺失测试 |
| Tool 白名单和风险等级 | Agent 调用前必须由业务服务校验 Tool 策略 | ToolCapabilityService | 业务服务责任域 | Tool 停用、白名单、风险确认测试 |
| Skill 创建、审核、发布、回滚 | Skill 发布状态决定 Agent 可路由能力 | SkillCatalogService 与 Skill 审核业务能力 | 业务服务责任域 | 生命周期、审核通知、版本回滚测试 |
| 生成前积分预估和确认 | 积分预估是 preview，冻结是 confirm 后写入 | CreditService Estimate/Freeze | 业务服务责任域 | 余额不足、0 单价、幂等冻结 contract test |
| 资产保存成功后才扣费 | 资产保存和冻结扣减必须在业务事务中保证顺序 | AssetCreditCommitService | 业务服务责任域 | 保存失败不扣费、部分完成扣费释放测试 |
| 上传素材校验和资产权限 | 上传登记和预览下载由业务服务控制 | AssetService 与业务授权访问能力 | 业务服务责任域 | 类型大小、无权限引用、签名访问测试 |
| 站内信审核通知 | 通知写入失败不影响审核事实，但需可补偿 | NotificationService、通知幂等键 | 业务服务责任域 | 通知幂等、已读未读、跳转权限测试 |
| 分享作品前生成公开快照 | 公开快照隔离私有资产和源创作过程 | WorkService、WorkShareService、PublicContentService | 业务服务责任域 | 分享安全证据、公开快照、取消分享测试 |
| 首页公开内容免登录 | 首页公开作品来自公开快照，不能读取用户私有作品或资产权限 | PublicContentService | 业务服务责任域 | 匿名访问、隐私字段缺失、下架不可见测试 |
| 点赞需要登录且同一用户一次 | 点赞是公开作品交互事实，需要唯一约束和幂等 | 业务 HTTP API | 业务服务责任域 | 未登录拒绝、重复点赞测试 |
| 平台下架公开作品 | 高风险运营写操作，需要确认和审计 | FeaturedWorkAdminService preview/confirm | 业务服务责任域 | 下架确认、公开链接不可访问、审计测试 |
...
本节原为早期 Kitex server 能力需求清单，已废弃。当前 RPC 方法、字段、timeout/retry、幂等、错误码和 fixture 映射不在本文维护。

| 当前事实 | 位置 |
| --- | --- |
| Thrift 字段级事实源 | `api/thrift/business_agent_service.thrift` |
| RPC 方法级策略 | `docs/contracts/rpc/业务微服务RPC契约草案.md` |
| 字段级契约索引 | `docs/contracts/字段级契约索引.md` |
| RPC fixture | `tests/contract/fixtures/business-rpc/**` |

如本文历史设计与当前契约冲突，以当前事实源为准。

## Agent 服务 智能体侧需要调用的 RPC

| Agent 场景 | 业务 RPC 能力 | 是否写业务事实 | 是否必须人工确认 |
| --- | --- | --- | --- |
| 进入工作台并确定上下文 | `ResolveCurrentSpaceContext` | 否 | 否 |
| 创建或校验当前项目 | `CreateProject`、`CheckProjectAccess` | 创建项目时写 | 创建项目无需确认；归档不由 Agent 发起 |
...
- 当前 PRD 套件已确认 Done；后续由文档与契约责任域确认契约、开发计划和责任域需求映射矩阵。
- 平台后台是直接调用业务服务，还是通过独立后台 API 适配层调用业务 RPC，需要文档与契约责任域和前端/Agent 架构确认。
