...
本草案基于 `docs/product/prd/README.md` 和 `00` 到 `12` 全部 PRD，先输出智能体微服务工程前置设计。当前 PRD 套件已确认为 `product_status：Done`，本文从工程前置设计升级为契约先行与开发输入，可作为 Go 代码、IDL、migration 和测试任务拆分的依据。
...
- 当前空间决定 Skill 池、积分账户、项目、资产归属和历史可见范围，但最终权限和业务事实由业务微服务确认。
- 项目是业务事实；Agent Runtime 只保存 `project_id` 引用，不保存项目标题、归档状态、封面等业务事实。
...
| 产品补充 项目与资产归属 | 项目是会话、资产、黑板和作品的创作容器 | session/run/event/artifact 保存 project_id，项目事实和权限由业务 RPC 确认 |
...
| --- | --- | --- | --- | --- |
| 统一 Agent 覆盖创作场景 | 第一版只有一个 Agent Runtime，根据当前空间和 Skill 池动态路由 | Agent Runtime 总体架构、统一 Agent 配置 | Agent 服务责任域 | Agent 行为测试、Skill 路由测试 |
| 当前空间决定 Skill/积分/资产/历史 | Agent 不自行判断最终权限，每次 run 携带空间上下文并调用业务服务校验 | Agent API 上下文字段、RPC 权限上下文 | Agent 服务责任域 + 业务服务责任域 | RPC contract test、权限边界测试 |
| 项目承载会话/资产/黑板/作品 | Agent 创建会话和 run 时必须绑定业务项目，只保存 project_id 和 Runtime 过程 | Agent API 项目上下文、Project RPC 调用需求、Agent DB project_id 普通字段 | Agent 服务责任域 + 业务服务责任域 | 项目权限 contract test、跨空间拒绝测试 |
| 模型选择当前对话生效且确认后锁定 | 生成类 run 保存模型选择快照，确认后拒绝修改 | `agent_runs.model_selection_snapshot`、`chat.controls.locked` 事件 | Agent 服务责任域 | 模型锁定事件测试 |
| Skill 仅 Published 参与路由 | Skill 路由前读取当前空间可执行 Skill 池，Draft/Pending/Rejected/Deprecated 不参与 | Skill Registry/Retriever 调用需求、路由决策记录 | Agent 服务责任域 + 业务服务责任域 | Skill 池过滤 contract test |
| Tool 是执行边界 | Tool 统一通过 Tool Registry 和执行器封装，不开放任意 HTTP Tool | Tool Runtime、Tool 调用事件、风险策略 | Agent 服务责任域 | Tool 权限/超时/取消测试 |
| 高风险、扣费、业务写入确认 | TurnLoop 进入 waiting_confirmation，保存 interrupt 和恢复上下文 | Interrupt/Resume API、`confirmation.required`、`interrupt.required` | Agent 服务责任域 + 前端责任域 | Interrupt/Resume 测试 |
| 内容安全前置 | Safety step 在预估和冻结前执行，失败或超时阻断 | Safety Workflow 节点、`safety.prompt.*` 事件 | Agent 服务责任域 | 安全阻断顺序测试 |
| 积分预估/冻结/扣费/释放 | 积分事实完全由业务 RPC 维护，Agent 只保存调用记录和事件 | Credit RPC client 需求、Tool call 记录 | 业务服务责任域确认语义 | RPC mock、幂等和异常测试 |
| 生成进度和取消 | 长任务状态持久化，不依赖内存；取消后不发起新 Tool | `agent_tasks`、preempt/cancel API、`generation.progress` | Agent 服务责任域 | 长任务取消/部分完成测试 |
| 资产保存成功后扣费 | 资产事实由业务服务保存，Agent 根据保存结果触发扣费或释放需求 | Asset RPC client 需求、资产引用元数据 | 业务服务责任域确认语义 | 保存失败释放测试 |
| 黑板和草稿元素 | Agent 保存草稿态黑板、分镜、脚本、提示词和过程快照 | `agent_artifacts`、`workspace.blackboard.updated` | Agent 服务责任域 + 前端责任域 | 黑板事件和快照恢复测试 |
| AG-UI 断线重连 | SSE 支持 Last-Event-ID 和 run_id+sequence 补偿，失败后快照恢复 | Event Store、SSE API、Snapshot API | Agent 服务责任域 | 事件重放和缺口测试 |
| 会话历史恢复 | 从历史会话恢复消息、资产引用、黑板和 run 快照 | Session API、Snapshot API、Agent DB 查询 | Agent 服务责任域 | 会话恢复测试 |
| 作品分享前安全评估 | 可复用安全评估服务能力，但作品事实和分享状态由业务服务维护 | Safety RPC/API 调用需求或内部能力复用建议 | 业务服务责任域确认入口 | 分享安全 contract test 由业务侧主导 |
| 站内信通知 | Skill 审核通知不是 Agent Runtime 责任 | 无 Agent 表；仅保留 trace 关联 | 业务服务责任域 | 通知业务测试 |
...
- 通过 Kitex client 调用业务微服务，创建或校验项目，绑定资产和项目归属。
- 保存 Agent Runtime 数据：session、run、message、event、tool_call、task、interrupt、artifact、memory、runtime_config 以及 `project_id` 引用。
- 生产 AG-UI/A2UI 事件，并支持幂等、排序、断线重连和快照恢复。

### 智能体微服务不负责

- 不直接写业务数据库。
- 不保存项目标题、项目归档状态、积分账户、积分流水、业务资产、Skill 审核结果、模型供应商密钥、作品分享状态、企业成员关系等业务事实。
- 不解释业务权限最终语义。
...
| API | 方法 | 用途 | 幂等/分页 |
| --- | --- | --- | --- |
| `/api/agent/sessions` | `POST` | 创建或恢复会话入口，绑定当前登录身份、空间和项目 | `idempotency_key` |
| `/api/agent/sessions` | `GET` | 查询当前用户有权访问的会话列表，可按项目过滤 | 默认 `page_size=10`，定义上限 |
| `/api/agent/sessions/{session_id}` | `GET` | 获取会话摘要、所属项目、最新 run 和快照状态 | 权限由业务 RPC 校验 |
| `/api/agent/sessions/{session_id}/messages` | `GET` | 分页查询会话消息 | 默认 `page_size=10` |
| `/api/agent/runs` | `POST` | 创建一次 Agent run，提交项目上下文、用户输入、模型选择、素材引用 | `idempotency_key` |
...
内部事件保存为 Agent Runtime 数据，再映射为 AG-UI 事件。事件统一包含 `event_id`、`type/event_type`、`project_id`、`session_id`、`run_id`、`sequence`、`timestamp`、`payload`、`trace_id`。当前规范使用 `type`，PRD 09 使用 `event_type`，需要正式契约统一命名。
...
以下为智能体微服务调用业务微服务的能力需求，最终服务名、方法名、DTO、错误码、权限、超时、重试、幂等和 preview/confirm 语义由业务服务责任域确认。

| 业务能力域 | 调用需求 | 读/写 | 关键要求 |
| --- | --- | --- | --- |
| Identity/Space | 解析当前登录身份、当前空间、租户、用户状态、企业成员状态 | 读 | 权限上下文、禁用用户阻断、被移出企业后拒绝访问 |
| Project | 创建项目、校验项目访问权限、读取项目摘要、绑定资产到项目、查询项目会话/资产/作品摘要 | 读/写 | 项目是业务事实；写操作幂等；企业空间第一版只返回本人项目 |
...
- 日志字段：`trace_id`、`service`、`env`、`tenant_id`、`user_id`、`space_id`、`project_id`、`session_id`、`run_id`、`turn_id`、`tool_call_id`、`task_id`、`interrupt_id`、`event_id`、`rpc_method`、`error_code`、`latency_ms`。
...
| 边界测试 | Agent 不写业务库、不保存项目和其他业务事实、不泄露敏感字段 |
...
- 当前 PRD 套件已确认 Done；后续由文档与契约责任域同步契约、开发计划和 责任域需求映射矩阵。
- AG-UI 标准使用 `type`，PRD 09 使用 `event_type`；正式契约需要统一字段名。
- AG-UI 标准基础事件与 PRD 09 扩展事件命名不同，例如 `agent.started` 和 `agent.run.started`；需要文档与契约责任域统一兼容策略。
- 积分、资产、模型、Skill、Tool、权限等 RPC 最终语义必须由业务微服务确认。
- 项目创建、项目归档、项目标题和项目封面语义必须由产品和业务微服务确认。
- 内容安全评估是否完全由 Agent 内部执行，还是由业务服务提供安全评估 RPC，需要文档与契约责任域确认跨服务 责任域。
