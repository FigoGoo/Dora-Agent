...
| 项目承载会话/资产/黑板/作品 | Agent 创建会话和 run 时必须绑定业务项目，只保存 project_id 和 Runtime 过程 | Agent API 项目上下文、Project RPC 调用需求、Agent DB project_id 普通字段 | Go Eino 智能体微服务架构工程师 + 业务微服务后端工程师 | 项目权限 contract test、跨空间拒绝测试 |
...
- 当前 PRD 套件已确认 Done；后续由主控同步契约、开发计划和 subagent 需求映射矩阵。
