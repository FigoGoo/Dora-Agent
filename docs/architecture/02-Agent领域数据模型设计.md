...
Agent 领域数据库不保存订单、支付、积分账户、积分流水、资产事实、Skill 审核结果、企业成员关系、业务权限、作品分享状态、模型供应商密钥、业务主数据等业务事实。业务事实必须通过 RPC 调用业务微服务产生并维护。
...
- `session_id`、`run_id`、`task_id`、`user_id`、`tenant_id`、`space_id` 等只作为普通字段、查询条件、审计字段或应用层一致性校验依据。
...
- 会话列表：按 `updated_at desc`，默认 10 条。
