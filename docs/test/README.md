...
更新时间：2026-06-28  
适用范围：Dora-Agent 第一版用户端、管理端、业务后端和 Agent 微服务的服务端能力测试用例  
相关代码路径：`tests/contract/**`、`tests/business/**`、`tests/agent/**`、`tests/e2e/**`、`api/thrift/**`、`api/openapi/**`、`api/agui/**`、`db/migrations/iterations/**`  
相关契约：`docs/contracts/**`、`docs/standards/测试规范.md`、`code-plan/tests/00-服务级测试计划与验收矩阵.md`

## 背景

本目录承接 `docs` 和 `code-plan` 下第一版产品、架构、契约和开发设计，沉淀可执行的服务端测试用例。测试范围只覆盖 HTTP API、Agent API、RPC、AG-UI 事件生产、数据库、事务、权限、幂等、日志和服务级链路，不覆盖页面视觉、组件渲染、浏览器点击或 CSS 布局。
...
- 不替代实现阶段的真实测试报告；实现后仍需按命令输出和证据补充测试结果。
...
本文档集是设计阶段服务端测试用例清单。实现完成后，每条用例应补充到对应自动化测试或人工服务级验证记录中，并在测试报告中标注通过、失败、阻塞、未执行原因和证据位置。
