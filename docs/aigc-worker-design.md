...
| Candidate/Approval | 已实现但投影不完整 | Job Finalization 仍将 Asset、Storyboard Candidate/Active Binding 和每项 durable Approval 在一个领域事务中提交；素材预览与 Candidate 统一确认只在左侧故事板。Spec/Storyboard 系统 Approval 在同一 Session 唯一且串行，系统卡存在时抑制模型重复预览。 | 候选批量结果和其他 Capability/Approval Decision 仍有直发窗口，没有全域 Outbox→Inbox Projector。 |
...
当前 Worker 不直接调用 Agent，也不决定下一 Skill Stage。Generation 的 Outbox Publisher 从 Operation/Batch/Job durable truth 构造单张高层 Capability ToolRun，并在终态发出 `refresh_resources`；这不等同于 Worker 本身运行 Agent，也不等同于已经具备通用 Domain Projector。
...
- **尚未实现**：上述领域提交与 Generation Job terminal/Outbox 不是同一个事务。领域事务成功后再提交 Job terminal + `job.succeeded` Outbox；该 Outbox 重读 durable Job/Operation 状态后更新单张高层 Capability ToolRun，并用 `refresh_resources` 刷新左侧资源，不投影 Job/Operation 明细、素材或 Candidate Approval chat 卡。投影失败保持 Outbox pending，不回滚领域结果或触发错误退款；跨事务恢复仍依赖稳定 Asset、Command、Approval、Billing 和 Outbox 幂等键。
...
Review-required Job 的 Finalization 仍会为每个 Candidate Binding 创建独立 durable Approval；这些 Approval 是领域命令、幂等和崩溃恢复的边界，不是一项一张的 chat A2UI 交互边界。`job.succeeded` 之后只更新当前高层 Capability ToolRun 并发出 `refresh_resources`，Candidate 预览与选中状态留在左侧故事板。Spec Revision 和 Storyboard Revision 仍使用系统 chat Approval，但同一 Session 同一时刻最多存在一张可操作卡，必须串行；系统卡存在时抑制模型生成的同候选预览。
...
这里的异步结果不会恢复已经结束的 Go 函数或原 Graph 调用栈。当前可信费用快照保存在 Batch `CostSummary` 和完整 `PostBatchPayload` 中，后者同时写入 Operation.Result、terminal outbox 和 `BatchContinuationResult.Result`；用户侧只把必要汇总投影到对应高层 Capability ToolRun，完整事实仍通过 Store/read model 查询。代码中尚无持久化 ToolOperationResult。目标 PostBatchContinuation 再用它补全同一个 `operation_id/tool_call_id/batch_id` 的 ToolOperationResult。
...
operation.accepted / terminal job / terminal batch / billing event
    → SessionSignalPublisher 从 durable Operation/Batch/Job 聚合状态
    → 更新当前 generate_media/assemble_output 的单张高层 ToolRun
    → DurableEventBroker AppendOnce SessionEventLog

job.succeeded
    → generation outbox 重读已提交 Job
    → 更新同一张高层 ToolRun；终态附加 refresh_resources
    → DurableEventBroker AppendOnce SessionEventLog
...
候选素材批量 Decision
    ...
    → 全批处理后 direct publish 一次 refresh_resources + batch summary
...
当前没有统一 Projector Inbox；`job.started/waiting_provider/finalizing/retry_scheduled` 由 Generation Outbox 分别聚合更新 `tool_run:generate_media` 或 `tool_run:assemble_output` 高层 Capability 卡，不创建 Operation/Job/Stage 多卡或逐 Job Node。卡片用 Batch/Operation 版本门禁拒绝乱序回滚；逐 Job `status_version` 仍由 Store/read model 保留。同一 Turn 已成功投影该权威卡且没有其他公开 Tool 时，模型进度复述不发布、不持久化。Finalization projector 的失败 Row 按 generation outbox AvailableAt 退避、10 次后 dead，Dispatcher 继续同批后续到期 Row；Spec/Storyboard Capability/Decision 和候选批量结果直发仍有事务窗口。Approval Decision 事件使用稳定 decision version，只投影冻结 status/version；可变 Storyboard snapshot 使用独立 Aggregate Version 事件。DurableEventBroker 保证外部 Row 持久化和去重，但目前 publisher 默认使用 `agent_action` producer kind；这与目标 `domain_projector` producer 分类仍有差距。

A2UI 线协议只接受 `a2ui_version="1.0"` 与 `append_card/update_card`；后端、前端实时 SSE 和历史回放都对未知版本/Action 或无效 Card 整包 fail closed。Generic required TextInput、SingleChoice、MultiChoice、FileUpload 在提交前统一校验。成功 Job 的 `result_asset_ids` 只作为 Store/read model 的资源刷新依据；Batch 终态通过 `refresh_resources` 让左侧重新加载 Storyboard、Assets 和 Jobs，ToolRun 不展示本地图片/视频/音频或 Assembly manifest。
...
目标通用 Projector 应根据已持久化领域状态生成唯一投影。当前 generation outbox 从 Job/Operation truth 更新同一张高层 Capability ToolRun，并在终态发出 `refresh_resources`；Candidate Approval、素材预览和 Job/Operation 明细不生成 chat 卡。Capability 创建的 Spec/Storyboard 系统 Approval 及其 Decision 仍是事务后直发，同一 Session 必须唯一且串行，存在系统 Approval 时抑制模型预览。Candidate 则由故事板统一动作创建冻结批次，服务端再逐项执行或恢复 durable decision。Agent 不得自行猜测 Batch 是否完成、自行计算积分或重复创建 Approval；它应直接使用 `BatchContinuationResult.Result` 中的可信净积分和逐 Job 结果。
...
| Worker/领域逻辑直接发易失 UI | DurableEventBroker 持久化外部 Row；generation 状态聚合更新单张高层 Capability ToolRun，终态发 `refresh_resources`。Operation/Job 明细、素材与 Candidate Approval 不发 chat 卡。 | Spec/Storyboard Capability/Decision 与候选批量结果仍直发；全域 Outbox/Inbox Projector 待实现。 |
...
16. Review-required 结果只创建 Candidate Binding 和每项 durable Approval，用户确认前不切换 Active Asset；素材预览与 Candidate 统一确认只在左侧故事板，相关 Job 全终态后仅提交一次候选批量 Decision。Spec/Storyboard Approval chat 卡仍保留，并必须携带 `approval_id`，以 `SingleChoice + Card submit` 调用 Decision API；同一 Session 只允许一个可操作系统 Approval 且必须串行，存在时抑制模型预览。聊天文字“确认”仅是 UserMessage，不能推进审核，模型也不得提示该伪入口。
...
9. 正常媒体/装配 Batch 使用 `on_failure`；每次 Capability 的成功由同一张高层 ToolRun + `refresh_resources` 展示。Batch terminal 直接 UPSERT Durable BatchContinuationResult，其 Result 完整携带 Barrier 的 PostBatchPayload；成功直接提交，只有失败类终态由 Session Runtime 串行调用 Runner。
...
11. Generation 使用专用 A2UI 投影，从 Operation/Batch/Job durable truth 维护单张高层 Capability ToolRun，终态发 `refresh_resources`；不发布 Job/Operation 明细、素材预览或逐项 Candidate Approval chat 卡。Spec/Storyboard Approval 卡及其 Decision、候选批量结果仍事务后直发，系统 Approval 在同一 Session 唯一且串行。DurableEventBroker 将外部 Row AppendOnce 到 SessionEventLog，SSE Relay 按 cursor Tail。
...
13. 高层 Capability ToolRun 通过 `operation_id` 发起 control；cancel 作用于原 Batch，`retry_failed` replay-first，保留原终态并仅为语义仍有效的 provider/transient 失败 Job 创建新的 Recovery Operation/Batch/Jobs。
