...
> 文档定位：本文保留历史 `main` 单体 Generation Worker 实现的迁移快照和仍有效的目标契约；上方 `internal/aigc/**` 路径仅是历史关联模块。正文中的“当前实现”一律指本文日期对应的历史快照，不是当前独立 `worker/` Module 事实。
>
> 权威口径：当前阶段、已通过 Evidence 和下一步只以[功能优先开发与试跑计划](./requirements/full-function-smoke-development-plan.md)为准；Worker 与异步 Tool 获批范围以[Graph Tool 设计索引](./design/agent/graphtool/README.md)、当前 Module 规范和独立设计为准。本文的实施顺序仅保留目标依赖，不再维护独立排期。
>
> 维护规则：不得在本文增量维护当前 Worker Runtime、Job 闭环或试跑状态；这些状态变化只更新上述唯一计划、Graph Tool 索引和对应独立设计。本文中的旧 `internal/aigc/**` 路径、Worker 计费/退款和历史 Provider 验收描述均不得作为当前工作树证据。

## 1. 文档目标

本文档同时记录历史 `main` 单体 Generation Worker 的**实现快照**和**目标架构**，覆盖职责边界、数据模型、状态机、队列与恢复、Provider 执行、媒体收尾、Batch Barrier、领域事件以及 Session Runtime 唤醒机制。

阅读约定：第 2 节是截至 2026-07-11 的历史实现基线；第 3 节之后保留完整目标契约。章节中标为“当前”的内容只描述该历史快照，标为“目标”或“建议”的内容不应被理解为已经落地。该历史实现面向受信本地 Demo，未提供公网生产所需的真实登录鉴权、租户授权和全局/用户/Provider 限流。历史本地验收 profile 保留 DeepSeek 和由 `DORA_IMAGE2_API_KEY` 启用的真实 Image2；Seedance、Audio 和 Assembly 可使用本地占位实现。其验收要求 Worker 生命周期、媒体落盘、Finalization、Barrier、Session Signal 和 A2UI 投影连通，不要求 Seedance/Audio/Assembly 的真实生产，也不执行真实合成或转码。
...
### 2.4 历史 `main` 本地验收 Worker 配置与输出契约
