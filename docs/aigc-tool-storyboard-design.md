...
### 1.1 已确认目标边界（覆盖旧 Demo 假设）

1. Storyboard、Revision、Element、Prompt/Asset Slot、Asset 和 Binding 全部归 Business 所有；Agent 和 Worker 均不得拥有对应表、Repository 或写模型。
2. Agent 只拥有 Graph、Session、Operation、Batch、Job 和 continuation 状态。Graph 的 Query/Command Node 通过正式 Business RPC 读取或修改 Storyboard，不导入 Business `internal` 包。
3. Worker 只负责队列、并发/lease、Provider 生成、重试、TOS 上传和执行回执。Worker 可请求绑定资产或故事板，但只能调用 Business `FinalizeGeneration` RPC，且 `binding_mode=none` 时不绑定。
4. `generate_media` 必须先调用 Business `PrepareGeneration` 完成生成前原子直接扣费，并取得 `preparation_id + element_ids + asset_ids + ledger_entry_ids`，成功后才创建 Agent Job 并派发 Worker。
5. 前置扣费不是 Reservation，成功状态即 `charged`；不支持退款。生成、上传、绑定失败和用户取消都不创建 refund/compensation。
6. Worker 批次终态通过 Outbox/消息投递 Agent Inbox；Agent 以 `(batch_id,result_version)` 幂等写 `BatchContinuationResult`，唤醒 Session Lane。Worker 不直接调用 Agent，不恢复旧 Graph 栈。
7. A2UI 只投影高层 Tool/Status 和必要交互，完整协议与组件见 [AIGC A2UI 全栈详细设计](./aigc-a2ui-design.md)。

### 1.2 跨 Module 所有权

| 契约 | 调用方向 | 责任 |
| --- | --- | --- |
| `GetCreationContext/GetStoryboardScope` | Agent → Business | 读取已授权的 Storyboard、Binding、Asset 摘要和版本 |
| `Plan/Update/PromoteStoryboard` | Agent → Business | 由 Business 校验并写入 Storyboard 聚合 |
| `PrepareGeneration` | Agent → Business | 校验、创建元素/资产占位、直接扣费并返回冻结派发引用 |
| `FinalizeGeneration` | Worker → Business | 保存 TOS Asset metadata，并按策略执行可选 Binding |
| `GenerationBatchTerminal` | Worker → Agent | 传递批次终态及 Business 结果引用，触发 durable continuation |

具体 IDL、超时、鉴权和写入事务在实现设计阶段固化；本需求设计先固定所有权、调用方向、幂等和成功边界。
...
| Skill 会话快照 | 目标变更 | 产品只保留 draft/published；Session 创建时冻结 Business 当前 published snapshot，新发布只对新会话生效。当前按 run 动态加载属于待迁移实现。 |
...
| 动态 Storyboard | 已实现、待迁移所有权 | Active/Pending Revision、动态 Module/Element/数量、Dependency DAG、PromptSlot、AssetSlot、Binding 和稳定 ID；目标迁入 Business，Agent/Worker 仅通过 RPC 使用。 |
| 局部操作 | 已实现 | Prompt 直接替换、上传资产填充、局部重生成、候选资产审核、整板 Replan；AI Prompt Rewrite 尚未接入 UI Command。 |
| Operation/Batch/Job | 已实现、待迁移计费 | Agent 持有 Operation/Batch/Job；目标链路在 Business 前置扣费成功后才创建，与 Agent Outbox 同事务。 |
...
8. Postgres 是业务真源，但按 Module 分治：Business Postgres 持有 Storyboard/Prompt/Asset/Binding/积分；Agent Postgres 持有 Approval/Operation/Batch/Job/Session/Event。不得跨库直写或共享 Repository。
9. Storyboard 的语义模块、元素类型、字段和数量由 LLM 根据场景推理；后端只固定最小协议、生命周期和确定性校验。
10. 所有目标使用稳定 ID。Job、Approval、UI Command 不得长期引用数组下标。
11. 所有写操作必须经过版本校验和幂等。Business 和 Agent 各自在本地事务内写聚合与 Outbox；跨 Module 通过稳定事件 ID 和 Inbox 衔接，任何服务不得假设跨库事务。
...
flowchart LR
    U["User"] --> FE["Frontend"]
    FE -->|"chat / new background / new goal"| API["Agent Message API"]
    API --> APG["Agent Postgres"]
    APG --> DSI
    DSI --> TL["Durable Session lane"]
    TL --> A["ChatModelAgent"]
    A --> CT["Agent-facing Capability Graph Tools"]

    FE -->|"prompt edit / bind / read storyboard"| BAPI["Business API"]

    CT --> QN["Graph Query/Command Nodes"]
    QN -->|"RPC"| BAPI
    CT --> CM["Graph-internal ChatModel Nodes"]
    BAPI --> BPG["Business Postgres\nStoryboard / Asset / Binding / Points"]

    CT --> APG
    Q["Redis List wake-up queue"] --> W["Generation Workers"]
    W --> P["Provider Adapters"]
    W --> OS["TOS Uploader"]
    W -->|"FinalizeGeneration RPC"| BAPI

    APG --> GOB["Agent Generation Outbox"]
    GOB --> Q
    W --> WOB["Worker Terminal Outbox"]
    WOB --> AIN["Agent Inbox"]
    AIN --> DSI
    AIN --> EL["A2UI EventLog"]
    A --> EL
...
- Graph 负责上下文加载、LLM 推理和确定性编排；StoryBoard 查询/命令、权限、版本和生成前扣费通过 Business RPC 完成。
- Worker 只负责 Provider、下载校验、TOS 上传、调用 Business Finalize 和 Worker Job/Batch 执行终态，不拥有 Asset/Binding/积分真值。
- Batch Barrier 决定一组 Job 何时达到业务终态并产生唯一 terminal outbox；Agent Inbox 幂等写入 `BatchContinuationResult` Durable SessionInput，再按需启动一次 Agent 解释。
...
目标生产 Agent Registry 必须精确包含六个高层 Graph Tool。当前代码只注册五个，Runner 仍按五个固定 key 启动校验；缺少独立 `write_prompts` 是明确实施差距，不能用 `plan_storyboard` 的内部 Prompt Node 或旧 `write_the_prompt` 冒充已完成。迁移时应把启动校验、用户工具箱和 Definition Registry 同步切换为下面六个稳定 key，缺失或混入未发布 Tool 都 fail closed。

| name | description | 目的 | 执行方式 | 实施状态 |
| --- | --- | --- | --- | --- |
| `analyze_materials` | 分析用户文字、文件名、MIME、URL 和可信 metadata，保存版本化摘要；当前不读取图片、PDF、音视频二进制内容。 | 建立可追踪的素材清单事实 | 有界同步 | 当前已注册 |
| `plan_creation_spec` | 根据用户背景、创作目标、受众、时长、画幅、叙事、视觉、声音和模型偏好创建或修订完整创作规范。 | 维护全局创作约束 | 有界同步 + 审核 | 当前已注册 |
| `plan_storyboard` | 根据已确认创作规范创建完整动态故事板 Revision；元素规划只输出 Module/Element、数量、内容、PromptSlot purpose、AssetSlot 与 Dependency，不输出 Prompt 文本。领域层修复缺失/重复标识后，独立内部 ChatModel 节点为所有 Provider-backed 槽统一生成 Prompt；响应必须 exact-set 覆盖后才提交审核。仅用于首次规划或整体重规划，不能用于局部 Prompt 或资产编辑。 | 动态规划模块、元素数量、内容、Prompt 和依赖 | 有界同步 + 审核 | 当前已注册 |
| `generate_media` | 推进当前已确认故事板的正常生产，对 Active Revision 再做缺失 Prompt 安全补齐，并为下一可执行阶段创建图片、关键帧、视频或音频任务。 | 正常生产推进 | 异步派发 | 当前已注册，待迁移 Business 前置扣费/Finalize |
| `write_prompts` | 在独立模式创建/改写 Prompt，或在故事板模式批量补齐指定 Prompt Slot；保留用户锁定内容，不自动调用媒体 Provider。 | 用户可直接使用的提示词写作能力 | 有界同步，可审核/保存 | 目标新增 |
| `assemble_output` | 校验当前故事板和已确认资产，生成装配计划，并按输出类型创建预览或最终导出任务。 | 素材合成与导出 | 计划同步；渲染异步 | 当前已注册，待迁移 Business Finalize |

部分旧 Tool 和 Media Graph 实现只保留为隔离测试素材；生产入口、HTTP 路由和 Redis 队列不再装配它们，也不存在把旧 Provider、CRUD、Prompt Tool 装配进 Runner 的旧 Agent Registry 工厂。旧 `write_the_prompt`、`image2_generate_image` 和 `seedance_generate_video` 不属于生产 Agent Registry；其中 `write_the_prompt` 只能作为迁移参考，不能替代新的版本化 `write_prompts` Graph Tool Definition。
...
→ Business GetCreationContext / GetStoryboardScope RPC
...
→ Business Command RPC re-checks user locks before each CAS update
...
→ Business PrepareGeneration(idempotency_key, expected_versions, targets, model_config)
→ Business atomically validates, creates element/asset placeholders and directly charges points
→ require charged + preparation_id + element_ids/asset_ids + ledger_entry_ids
→ create Agent Operation + Batch + Jobs + generation Outbox transactionally
...
```json
{
  ...
  "charged_points": 120,
  "ledger_entry_ids": ["ledger_xxx"],
  "element_ids": ["element_hero"],
  "asset_ids": ["asset_xxx"],
...
Business 未确认 `charged` 时不得创建 Job。Graph 返回后即结束；Provider 和 TOS 上传由 Worker 完成，Asset metadata 与可选 Storyboard Binding 由 Worker 调用 Business `FinalizeGeneration` RPC 提交。

### 7.5 `write_prompts`

`write_prompts` 是用户工具箱和 Agent 都可选择的独立 Graph Tool，不是 `plan_storyboard` 内部 Prompt ChatModel Node 的别名。

业务输入：

```go
type WritePromptsIntent struct {
    Mode         string   `json:"mode"` // standalone, storyboard
    TargetIDs    []string `json:"target_ids,omitempty"`
    MediaKind    string   `json:"media_kind"`
    Instruction  string   `json:"instruction"`
    Variables    []string `json:"variables,omitempty"`
    PreserveLock bool     `json:"preserve_lock"`
}
```
...
```text
validate requested_tool_key / permission / budget
→ standalone: load authorized Business Asset/Project context
→ storyboard: Business GetStoryboardScope and resolve requested Prompt Slots
→ reject locked target when preserve_lock=true; freeze exact target_id/purpose set
→ Business billing RPC atomically freezes model points and directly charges before model execution
→ bounded Prompt ChatModel subgraph
→ require exact one-to-one outputs; reject missing/duplicate/extra target
→ Business Command RPC saves standalone Prompt Artifact or Storyboard Prompt Revision
→ return prompt IDs, target mapping, charged points and editable Markdown
```

约束：

1. 独立模式不要求 Storyboard，结果保存到当前 Project 的 Prompt Artifact；故事板模式只修改明确授权且版本匹配的 Prompt Slot。
2. 用户锁定 Prompt 不得被批量覆盖。Prompt 保存、复制、查看和确认已有结果不重复扣费；只有新的模型执行在开始前按冻结配置扣费。
3. Tool 完成后不自动派发 `generate_media`。需要生成媒体时必须由用户后续动作或新的 Agent 决策调用对应 Graph Tool。
4. 模型返回集合与冻结目标不一致时整次失败，不写部分 Prompt；已发生的正常扣费不退款。
5. A2UI 使用 Card Markdown 展示 Prompt，并可组合目标选择、输入、单选/多选和提交按钮；保存动作携带 expected revision 和幂等键。
...
手工绑定和 Worker 回填必须经过 Business 的同一 Storyboard Command Service；前端调用 Business API，Worker 调用 Business Finalize RPC，不能分别实现不同版本规则。
...
| name | description |
| --- | --- |
| `LoadCreationContext` | 组合 Agent Session/Operation 摘要与 Business RPC 返回的 Spec、Active/Pending Storyboard 和 Asset 摘要。 |
| `LoadStoryboardScope` | 通过 Business RPC 按稳定 ID 和 expected version 加载 Module、Element、Prompt、Asset Slot 和依赖。 |
| `FindReusableAssets` | 通过 Business RPC 在用户和租户权限范围内按 requirement fingerprint 查找可复用资产。 |
...
| `CheckEntitlementsAndEstimateCost` | 计算派发意图的估算参数；权限、模型积分、余额和最终扣费由 Business `PrepareGeneration` 再校验。 |
...
| `SaveCreationSpecRevision` | 调用 Business Command RPC 保存 Spec candidate/active Revision。 |
| `SavePendingStoryboardRevision` | 调用 Business Command RPC 保存完整 Pending Storyboard Revision 和 RequirementDiff，不覆盖 Active。 |
| `PromoteStoryboardRevision` | 调用 Business Command RPC 切换 Active Revision。 |
| `AttachPromptRevision` | 调用 Business Command RPC 保存 Prompt 并更新目标 Prompt Slot。 |
| `MarkDerivedOutputsStale` | 调用 Business Command RPC 标记依赖闭包 stale，保留旧资产。 |
| `ApplyAssetRequirementDiff` | 由 Business 在 Revision Promote 或已批准 Active Mutation 的本地事务中执行。 |
| `PrepareGeneration` | 调用 Business RPC 原子校验、创建元素/资产占位并直接扣费，返回冻结派发引用。 |
| `CreateGenerationOperation` | 仅在 `PrepareGeneration=charged` 后，于 Agent 本地事务原子创建 Operation、Batch、Jobs 和 dispatch Outbox。 |
...
```text
BusinessCreationRPCClient
BusinessStoryboardRPCClient
BusinessGenerationRPCClient
AgentGenerationOperationService
ApprovalService
AssemblyService
```
...
| `TOSUploader` | 按确定性 object key 上传媒体并保存上传 receipt。 |
| `BusinessFinalizationClient` | 调用 Business `FinalizeGeneration` 幂等保存 Asset metadata，并按 Job 策略执行可选 Binding。 |
| `BatchFinalizer` | Batch Barrier、已扣积分引用汇总和唯一 Worker terminal outbox。 |
...
内部 Graph 节点、Operation、Batch 和 Job 不逐一暴露到 chat。所有 `generate_media` 调用归并到同一 Operation 的稳定 Tool Card，后续 Worker terminal event 经 Agent Inbox 更新该卡。卡片只呈现能力名称、总体状态、完成/总数、前置已扣积分、简短失败摘要和必要操作，不把 Provider、上传或 Finalize 等内部 Stage 展开为多卡或逐 Job 节点。

| 高层 ToolRun | 内部 durable truth | chat 展示 |
| --- | --- | --- |
| `generate_media` | Agent Operation/Batch/Jobs、Business preparation/ledger/Asset refs、Worker terminal event | 素材生成中/完成/部分失败/失败/已取消、总体计数、已扣积分和简短错误；`operation_id` 支持取消/失败重试 |
...
```text
Business PrepareGeneration transaction
...
→ Worker submit/poll/upload TOS/Business Finalize RPC
→ Batch Barrier
→ Worker terminal Outbox → Agent Inbox → single Session Signal
```
...
两者必须在 Business 本地事务内提交，并写 Business Outbox。Agent 不保存该快照；Worker Finalization 也只能通过 Business RPC 更新。Business 事件进入 Agent Inbox 后再投影聊天状态。
...
- Token 匹配：允许 Business Finalize 执行 Asset 写入和可选绑定。
...
这是对 `aigc-worker-design.md` 中 `GenerationJob` 的必需扩展。Job 必须持久化 Business `preparation_id`、element/asset IDs、ledger refs、完整 BindingToken、`result_disposition`、`binding_mode` 和 `approval_policy`；不得包含退款或 Worker 计费状态。普通媒体生成幂等主键使用 SpecVersion、TargetRevision、PromptRevision、GenerationEpoch 和 InputFingerprint；Assembly 显式加入 AggregateVersion 全局栅栏。
...
→ save immutable upload/provider receipt on Worker Job
→ call Business FinalizeGeneration(preparation_id, asset_id, metadata, binding_mode, expected_version)
→ Business transactionally sets Asset available
   + optionally creates candidate/active Binding according to binding_mode
   + emits Business domain event
→ Worker records Job terminal + terminal Outbox + batch.finalize_requested
→ Batch terminal Outbox enters Agent Inbox
→ Agent Projector updates Tool/Status Card and refresh_resources
→ best-effort TryFinalizeBatch; durable trigger recovers failures/replay
```

生成前积分已经由 Business 直接扣除。Provider、上传和 Business Finalize 尚未完成时，Job 不能标记 `succeeded`；`binding_mode=none` 时不要求 Storyboard Binding。
...
`review_required` 时 Worker 请求 Business 把 Asset 置为 `available` 并创建 Candidate Binding；ApprovalContinuation 后续通过 Business Command RPC 切换 Active Binding 并传播下游 stale。只有明确冻结为 `auto_approve` 时，Business 才能直接激活。`binding_mode=none` 仅保存 Asset，不写 Storyboard。`assemble_output` 仍只接受 required active Binding；stale Slot 阻止派发。

费用与失败规则：

1. 扣费只发生在 Business `PrepareGeneration`；Worker 不计算实际费用、不调用 Charge/Refund，也没有 Billing retry phase。
2. Provider usage 只用于平台成本核算和审计，不得反向改写用户已扣积分。
3. Provider、TOS 或 Finalize 失败按各自 receipt 和幂等键恢复；达到永久失败时 Job 正常收敛为 failed，扣费流水保持 charged。
4. Binding CAS 冲突时 Asset 可保持 available/unbound，`result_disposition=superseded`，Job 按冻结交付策略确定 succeeded-unbound 或 failed；无论哪种都不退款。
5. Business Finalize RPC 结果未知时，Worker 先按 `(preparation_id,asset_id,job_id)` 查询 receipt，不能重复创建 Asset/Binding。
6. 领域 Commit 返回错误时保留原 Job ID 和错误分类，不得用派生 `job not found` 掩盖根因。
7. 所有 Job 终态与 `batch.finalize_requested` 同 mutation；Barrier 事件持续恢复，普通 Outbox 才可在达到预算后进入 dead-letter。
...
→ aggregate immutable preparation_id / ledger_entry_ids / charged_points references
...
→ Worker writes exactly one batch terminal Outbox
→ Agent Inbox consumes the event and UPSERTs durable BatchContinuationResult by stable InputID
...
目标 `PostBatchPayload` 包含 SessionID、WorkflowRunID、StageRunID、OperationID、ToolCallID、BatchID/Version、终态、`preparation_id/ledger_entry_ids/charged_points`、逐 Job 的 Business element/asset ID、target/slot/status/disposition/error code、`NeedsAgentExplanation` 和 CreatedAt。它是 Barrier 事务生成的可信快照，不是 Publisher 或 Agent 从 Provider Payload 拼接的轻量摘要。
...
Candidate Asset 由 Business `FinalizeGeneration` 创建 Candidate Binding。左侧 Storyboard 等相关 Job 全终态后调用 Business API，Business 冻结并执行当前 pending Candidate 集合；Agent 只接收结果事件并投影，不拥有 Binding。取消意图使用独立 `batch.cancel_requested` 持久化；取消事务和每次 Job settlement 后重新触发 Barrier。
...
| --- | --- | --- | --- |
| `completed` | `generation.batch.completed` | `completed` 或 `waiting_user` | `succeeded` 或 `waiting_user` |
| `partial_failed` | `generation.batch.partial_failed` | `partial` 或 `waiting_user` | `partial_failed` |
| `failed` | `generation.batch.failed` | `failed` | `failed` |
| `cancelled` | `generation.batch.cancelled` | `cancelled` | `cancelled` |
...
目标 UI 写入 SessionEventLog 的来源有三类，完整合同以 [AIGC A2UI 全栈详细设计](./aigc-a2ui-design.md) 为准：

1. ChatModelAgent 输出的聊天类 A2UI Action。
2. Worker terminal event 进入 Agent Inbox 后，Projector 从 Operation/Job/Batch 与 Business 引用计算单张高层 Capability ToolRun；终态发布 `refresh_resources`。
3. Business Storyboard/Spec/Approval 事件进入 Agent Inbox 后，由统一 Projector 发布 A2UI Action。
...
目标必须提供通用 Agent Domain Inbox/Projector，并以稳定 EventID、CardID 和 CardVersion 避免重复卡。投影失败不回滚 Business/Worker 结果；Inbox 保持 pending 并可恢复。`batch.finalize_requested` 是 Worker 内部 Barrier 事件，不直接进入 UI。
...
8. 费用值由 Business 模型积分配置计算并在生成前直接扣除，客户端和 LLM 只能看到估算或最终可信汇总。
9. Session Jobs 和 Operation detail 对外返回前移除 Job Payload/Result、BindingToken、Provider task/request、lease、User/幂等键与账务流水 ID、内部错误正文；失败 Job 不暴露未授权 ResultAssetIDs。
...
| Worker terminal event 到 Agent 投影失败 | Agent Inbox 保持 pending 并重试，不回滚 Business Finalize 或 Worker Job；Candidate 不存在 chat 卡投影。 |
...
| Generation | 当前 Operation/Batch/Job、五种 receipt-backed no-op、Barrier、Outbox、PostBatchPayload、BindingToken/InputFingerprint、对象存储 | 目标需迁移为 Business 前置扣费/Asset/Binding、Worker terminal Outbox、Agent Inbox，并补独立 PostBatchContinuation/Stage Ledger |
...
```text
business/internal/
  storyboard/        # Aggregate、Revision、Module、Slot、Dependency、Command/Query
  asset/             # Asset metadata、availability、Binding
  generation/        # PrepareGeneration、FinalizeGeneration、receipt
  billing/           # model points、account、immutable charge ledger
  transport/rpc/     # 对 Agent/Worker 发布的 DTO/handler
...
agent/internal/
  graphtool/
    ...
    write_prompts/
    assemble_output/
    businessclient/  # 正式 RPC client，不导入 Business internal
  operation/         # Operation、Batch、Job、Agent Outbox
  sessionruntime/    # Inbox、BatchContinuationResult、Session Lane
  a2ui/              # protocol/component/action/projector/publisher

worker/internal/
  generation/        # claim、lease、provider、retry、batch barrier
  storage/tos/       # TOS upload 与 receipt
  businessclient/    # FinalizeGeneration RPC client
...
1. 新增独立 `write_prompts` Graph Tool，并把 Registry、工具箱、权限、计费、A2UI 和启动 fail-closed 校验统一切换到六个稳定 key。
2. 为仍直发的 Capability/Approval Decision 补 Transactional Outbox，并把现有 `job.succeeded` 专用投影收敛到统一 Domain Projector/Inbox。
3. 聚合持久化 Capability ToolRun/ToolOperationResult，并增加独立 PostBatchContinuation。
4. 将 Finalization 的 generation Job、Asset、Storyboard/Approval commit 收敛到可证明的一致性边界。
5. 为当前 `retry_failed` Recovery Operation 增加运营审计，并实现只续跑 Finalization 的独立恢复模式；provider/transient 错误分类和 replay-first 已完成。
...
3. 在独立 `write_prompts` 基线完成后，增强 Prompt Artifact 历史和回滚。

## 30. 测试设计

### 30.1 Tool Registry

- 目标 Agent Registry 精确包含六个 Graph Tool；迁移前五个 Registry 必须被测试明确标记为未达到目标，不得误报通过。
- 只允许独立 `write_prompts`，不包含旧 Prompt Tool、Provider、余额、权限和资产 CRUD Tool。
...
- Provider、上传和 Business Finalize 的每个阶段可独立重试；扣费只在 Job 创建前由 Business 幂等执行。
...
- 同一 IdempotencyKey 不重复生成、上传、前置扣费或绑定。
...
10. Business 前置扣费并返回 element/asset IDs 后才能创建 Job；Provider、TOS 上传和 Business Finalize 完成后 Job 才成功，Binding 按策略可选。
11. 多 Job 只在 Batch 终态由 Worker 发布一次 terminal event；Agent Inbox 幂等创建一个完整 `BatchContinuationResult`，并按需向 Session Lane 发送最多一次解释输入。
...
21. Skill 只有 draft/published；Session 创建时冻结发布快照，新发布只对新会话生效，loader progress/error 不投影到用户 ToolRun。
...
24. Storyboard/Binding 只存在于 Business；Agent Graph Node 和 Worker Finalization 都通过正式 Business RPC 访问。
25. A2UI Card 支持安全 Markdown，首期组件覆盖单选、多选、提交、输入、多媒体、纵向步骤、Tool 和 Status，未知组件不得执行交互。
...
Business 决定：能不能写、哪些依赖失效、版本是否匹配，并在生成前直接扣费。
Worker 决定：如何可靠调用 Provider、上传 TOS，并通过 Business RPC 完成 Asset 与可选 Binding。
Batch Barrier 决定：何时产生一次 Worker terminal outbox；Agent Inbox 决定何时向同一 Session 投递一次 continuation。
```
