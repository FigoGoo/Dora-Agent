...
- [x] **Step 1: 写失败测试（覆盖全表七行 + 匹配顺序 + 表自身合法性）**
...
- [x] **Step 3: 最小实现（表数据从旧 switch 逐字迁移，instruction 用 Sprintf 展开 ToolKey 常量保持与旧代码同源）**
...
- [x] **Step 1: 改写 instruction 壳（守卫与吞错策略保持逐行为等价）**
...
- [x] **Step 2: 改写 directive 壳（错误传播策略保持逐行为等价；SourceID 运行时身份留在 server）**
...
**提交序列**（分支 `cc/core-version`）：

- 前置修复 `ed3c506` — merge fe47cef 遗留的旧 HTTP 面死代码（server 三组文件 + a2ui/events_test.go）拖垮全仓构建，Step 1 的 TDD 无法在编译失败的包上进行；按"零引用"事实删除后全仓首次 build/test/vet 全绿。删除内容可从 git 历史（ff5c71a 等）找回，须 Figo 知悉。
- Task 1 `d41d5ad` — orchestration 包 + PlanNode schema（kind 两态，atomic 预留仅 schema 级校验）。
- Task 2 `e0a878e` — 审批续作决策表七行 + ValidateApprovalContinuationRules 自校验 + DecideApprovalContinuation 纯查表。
- Task 3 `6074bcb` — runtime_processor 两函数薄壳化；黄金测试文件 `git diff --stat` 确认零改动，14 子案例 + 3 端到端全绿。
- Task 4 — 全仓 build/vet/test 全绿（DB 包 -count=1 对本地 Postgres/Redis 真跑）。

**事实清单**：

- 已验证：黄金测试逐字不动全绿 = 零行为变化；决策表自校验过（七行节点全部通过 PlanNode schema）；全仓回归绿。
- 未验证：真实会话下审批续作链路（需要 DeepSeek key + 前端手工回归，属验收范围）。
- 设计取舍（已在计划自审记录）：instruction 文本 Sprintf 展开 ToolKey 构造；表进 DB 后置；谓词求值留 server。
