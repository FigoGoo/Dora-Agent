...
- [x] **Step 1: 先读 adapters_test.go 既有测试的 Assets fake 构造方式（约 50 行内即可找到），复用同一构造；然后写失败测试**
...
- [x] **Step 1: 写失败测试（骨架，store/job 构造按上述既有 helper 落实）**
...
- [x] **Step 2: 跑测试确认失败（在 Task 3 未合入的分支上应报 storyboard services required；合入后首跑若因 job 构造缺字段失败，按报错补齐构造）**

Run: `go test ./internal/aigc/generationruntime -run TestFinalizeSessionDeliverableEndToEnd -v`

- [x] **Step 3: 无实现改动预期——本任务是集成验证；若暴露 Finalize 主流程里其他 storyboard 硬依赖（如 Inspector/Discarder 路径），逐一按 Task 3 的分派模式补分支，每处补一个对应断言**
...
- [x] **Step 2: 有本地基础设施则起 docker compose 跑 DB 相关测试**
...
- [ ] **Step 4: 请 Figo 评审本计划全部 diff（底座保护面变更），评审通过后才进入第 1 步（推进映射数据化）计划**（待 Figo——评审材料见下方执行记录）

---

## 执行记录（2026-07-12，实现完成，待 Figo 评审）

**提交序列**（分支 `cc/core-version`，每个 task 一个提交）：

- Task 1 `959c66a` — `generation/models.go`：`TargetKind` 两态常量 + `NormalizedKind()`（空值回退 storyboard_slot，兼容全部存量 token）+ `Validate` 分派（deliverable 不要求 StoryboardID，未知 kind 拒绝）+ `Equal` 按归一化 kind 比较。测试 `binding_token_test.go`。
- Task 2 `cb775f8` — `generationruntime/adapters.go`：`StoryboardBindingAdapter.Check` 顶部对 deliverable 短路（恒 TargetExists+Matches；漂移防护由 job 级 request_fingerprint 承担）。`BindingGuard.Check → CheckWithFingerprint → Check` 纯委托已核实，单点覆盖全部入口。测试 `adapters_deliverable_test.go`。
- Task 3 `4627c65` — `commit`：`isDeliverable` 分派（storyboard 校验段跳过、资产 available 循环后与 assembly 同型短路 return）；`IsCommitted` 同型短路。**计划外但按 Task 4 Step 3 授权补的一处**：`publishChanges` 对 deliverable 短路——outbox 的 `PublishFinalized` 对每个 finalized job 投影 storyboard，deliverable 无聚合可投，不短路会对 `StoryboardID=""` 反复 GetAggregate 造成 outbox 重试死循环。测试含 `PublishFinalized` 断言（Events 非 nil 必失败 + Repository nil，不短路即报错/panic）。
- Task 4 `32863fe` — 端到端 `TestFinalizeSessionDeliverableEndToEnd`：memory workflow store 建 queued job（active+auto_approve+postpaid）→ 推到 running → `Finalize`（实际签名三参 `(ctx, jobID, ProviderResult)`，计划骨架两参有误，以代码为准）→ 断言 succeeded + bound_active + 资产 available。Repository/Approvals/Specs 全 nil 通过全链 = 物理隔离证明。合入 Task 3 后一次通过，无需新实现（计划 Step 3 预期成立）。

**回归事实清单**：

- 已验证：`go vet` 干净 + 全仓测试 PASS（27 个包，含对本地 Postgres/Redis `-count=1` 真跑 generation/generationruntime/approvalruntime/sessionruntime/capabilityruntime/storyboard/asset）。
- 既有破损（**先于本计划、非本次引入、未修**）：`internal/aigc/server`（编译失败：`documents.go`/`media_generate.go`/`skill_router.go` 引用新 `server.Config` 已不存在的字段 `MediaDispatcher`/`SkillSelector`/`Publisher` 等）、`internal/aigc/a2ui`（仅测试编译失败：`events_test.go` 引用不存在的 `EventSkillSelected`）、`internal/aigc/integration`（因依赖 server 连带 build failed）。已在基点 `9b99fd2`（本计划开工前）复现同样失败，来源是 `fe47cef` merge 远端 main 时旧 HTTP server 面与新底座并存的遗留。**须 Figo 裁决**：删旧路由还是补新 Config 字段——超出 Step 0 范围且属其正在改造的面，未擅动。
- 未验证：生产 Postgres 事务路径下 deliverable commit（测试走 memory/plain 路径；事务包装对 deliverable 无害的推理见 Task 3 header 注）；deliverable 的前端事件形态（Step 2 范围）。
