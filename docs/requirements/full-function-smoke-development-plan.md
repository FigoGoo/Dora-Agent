...
| D0-04B | Validator transitive build-input closure | 把 R01/R04 verifier 迁入独立 entrypoint package，拆除共享 `contract_test` 包的传递缠绕；冻结 import/embed/外部模块/toolchain/workflow/action 构建输入 exact-set | 进行中、formal Freeze blocker。批次一`19236995`已建立仅允许`candidate_unactivated`的optional Schema。批次二已迁移R04到stdlib-only `w2r04approvalconsumption`并在manifest登记未激活closure；R01迁到六源码`w2r01`、拆除embed/internal/google uuid，只保留受管`x/text/unicode/norm`，R03经窄facade复用且R03 manifest字节不变。批次三A新增显式Module lock+go.sum输入、只读`.zip/.mod` loader、canonical h1重算及路径/重复/大小写/vendor/workspace/zip-bomb失败关闭的zip-backed静态候选；它不信任`.ziphash`或解压cache，但`go/build`交叉选择不等于真实`go list`，R01 manifest继续不声明closure。批次三B必须以v2/独立attestation冻结zip/.mod raw SHA与大小、`GOAMD64/GOEXPERIMENT/GOFIPS140`等完整环境、`transform`/`unicode/norm`真实`go list -deps -test -compiled -json`投影及typecheck/compile；批次三C再建立versioned base-owned Action/workflow release、helper exact-set与handoff，修复“文件存在即激活/永久不可升级”。两Gate blocker均保留，全部闭合前不得formal Freeze |
...
| W2-R01 | [`graph-tool-result-receipt-contract-v1.md`](../design/agent/graph-tool-result-receipt-contract-v1.md)、[`runner-session-lane-review-v1.md`](../design/agent/runner-session-lane-review-v1.md)、[`w2-r01-owner-decision-matrix-v1.md`](../design/agent/w2-r01-owner-decision-matrix-v1.md) 与 [`aigc-contract-catalog.md`](../design/cross-module/aigc-contract-catalog.md) | `expansion_frozen / partial_candidate`；8个合法Result、40个Result拒绝、26个Receipt迁移和13个跨状态evidence共87条可靠子集。Validator已迁入六源码独立`w2r01` package，拆除embed/internal/google uuid并保留唯一受管`x/text/unicode/norm`；manifest冻结direct source和Module metadata但有意不声明build closure。zip-backed resolver已校验给定archive的raw SHA/Go h1、根go.mod与静态source选择；compile-attestation当前仅建立单clean-builder run statement、typed content ref、`go list` projection Schema和双run claim配对判定的test-only候选。输入快照已能复核自身canonical metadata JSON及14项repository/13项module metadata exact-set，但尚不读取这些文件的原始字节；其余content ref和`go list` raw也未解引用/重算，不证明archive acquisition、真实builder执行、builder独立性或issuer authority。failed-after/outcome-aware、Billing authority、exact-version、card owner、Registry/ordinal scope、Owner exact-set及toolchain/action仍未关闭；未创建生产DTO/Repository/Migration | 下一步完成D0-04B批次三C的内容寻址material bundle、真实Go 1.26.3 raw `go list` golden/canonicalizer、BuildInfo/SBOM parser、真实离线builder adapter、每run独立签名envelope、双clean-build trust-domain策略、versioned trust-root release与base-owned workflow/action，并按决策矩阵关闭六项语义决定与受信Owner authority。仅按明确结论补删最小向量，再恢复完整`gate_candidate/awaiting_review`，不得以当前87向量或自报attestation直接生成Freeze |
| W2-R02/R03 | [`runner-session-lane-review-v1.md`](../design/agent/runner-session-lane-review-v1.md)、[`session-lane-runtime-contract-v1.md`](../design/agent/session-lane-runtime-contract-v1.md)、[`session-lane-ingress-command-contract-v1.md`](../design/agent/session-lane-ingress-command-contract-v1.md)、[`session-lane-postgresql-design-v1.md`](../design/agent/session-lane-postgresql-design-v1.md)、[`session-lane-legacy-upgrade-contract-v1.md`](../design/agent/session-lane-legacy-upgrade-contract-v1.md)、[`session-event-foundation-marker-v1.md`](../design/agent/session-event-foundation-marker-v1.md)、[`immutable-turn-context-design-v1.md`](../design/agent/immutable-turn-context-design-v1.md)、[`immutable-turn-context-decision-review-v1.md`](../design/agent/immutable-turn-context-decision-review-v1.md)、[`approval-continuation-cross-object-evidence-v1.md`](../design/agent/approval-continuation-cross-object-evidence-v1.md)、[`approval-consumption-receipt-contract-v1.md`](../design/agent/approval-consumption-receipt-contract-v1.md)、[`continuation-child-tool-receipt-contract-v1.md`](../design/agent/continuation-child-tool-receipt-contract-v1.md) 与 [`aigc-contract-catalog.md`](../design/cross-module/aigc-contract-catalog.md) | R02已形成Agent-owned Executable Draft：60条Lane、42条Ingress、17条legacy Authority、90条upgrade blocker、19条Marker，以及Message Set/Turn Context联合8 fixture、70向量和27目标测试。R03以1 fixture、20跨对象向量和13目标测试绑定Receipt/Approval/Decision/Pin/Context；根测试现只经窄facade调用R01父Receipt evaluator，R03 manifest/Corpus字节不变。另有4 fixture、111向量、11测试的activation Consumption Core candidate。child仍是Design Draft/Corpus Pending；生产/认证Consumption、child、Business Decide/Query、真实PG原子性不存在。10项P0仍为`awaiting_owner_approval`，PostgreSQL 16 Migration 005只到`REAL PG PRECONDITION READY` | 保持TC-P07全部unmet；先关闭W2-ADR-001～004与W2-ADR-011，再按决议迁移或保留projection。只补Owner明确要求的child exact-set最小Corpus，随后联合审批Turn Context、Marker/Retention、DDL、升级Ledger、Approval/Consumption与Claim/Fence/Ref；Approved后先实现Lane/PG Kernel，SMK-018 Worker留在W3 |
...
| W2-R04 | [`plan_creation_spec-design.md`](../design/agent/graphtool/plan_creation_spec-design.md)、[`plan_creation_spec-w2-r04-gap-review.md`](../design/agent/graphtool/plan_creation_spec-w2-r04-gap-review.md)、[`approval-consumption-receipt-contract-v1.md`](../design/agent/approval-consumption-receipt-contract-v1.md) 与 [`continuation-child-tool-receipt-contract-v1.md`](../design/agent/continuation-child-tool-receipt-contract-v1.md) | Draft；`creation_spec_activation` Consumption unsigned core已有111条test-only向量，Validator迁入两源码stdlib-only `w2r04approvalconsumption`并登记`candidate_unactivated` build closure；受信toolchain/workflow/action未闭合，blocker保留。child root/parent/双guard/prepared/unknown为Design Draft，child与failed-after Corpus pending；Intent/Candidate、逐模型计费、immutable attribution、Business RPC、typed fan-in/Output等P0仍未关闭，A2UI由R08门禁 | 先补D0-04B锁定toolchain compile与base-owned workflow/action，再关闭activation key、Action/Effect、Consumption Query鉴权、唯一执行授权模式与最小计费ADR；只补Owner明确要求的最小缺口并联合审批共享R01/R02/R03与本Tool，R08在W2-C1前独立Approved。Approved后它是唯一首批可执行Tool |
...
D0-04B Batch 3B 当前只新增 `w2_validator_compile_attestation_run_statement.v1` 的纯Schema/Validator、双run pair evaluator、typed input-snapshot metadata contract与对抗矩阵：每份statement只声明一个clean builder run，并绑定repository/base commit+tree、manifest与build-closure投影、validator源码、`go.mod/go.sum`、`x/text v0.34.0` raw zip/`.mod`/zip根go.mod/h1/HashDir/selected source、Go 1.26.3 linux/amd64隔离环境、124项stdlib名称exact-set及闭合import图；typed content ref只允许schema/kind/media type/SHA-256/有界大小，不允许URI或路径。输入快照从已注册manifest四组引用派生14项repository metadata，并交叉绑定13项module-cache metadata、五条命令、sandbox policy和typed SBOM generator binary ref；其raw JSON必须canonical、before/after同ref且摘要/大小按传入字节重算，但当前仍不复核每个文件的raw bytes、Git blob/base tree成员、symlink或额外workspace material。单run校验还要求记录绝对路径`go list`、`go test -c`、同一已哈希test binary执行、固定BuildInfo投影和项目自有确定性SBOM命令；pair evaluator要求同一challenge/policy/subject/投影下的`builder_a`与`builder_b`具有不同builder/workspace/module cache/build cache标识并声明相同artifact、BuildInfo、test和SBOM。现有`go list`正例是专用于Schema/图规则的合成fixture，不是真实Go 1.26.3 golden；调研确认真实命令应产生129个对象（124项stdlib、2项`x/text`、target regular/test variant和synthetic test main），Batch 3C必须用两个fresh GOCACHE的raw stream分别重算同一固定projection。statement严格拒绝`status/candidate_unactivated/active/trusted/authoritative/result/issuer/signature`类字段，8 MiB/64层JSON上限、重复key/null/缺字段/未知字段和大小写alias也失败关闭；当前候选没有签名、可信runner或activation语义。Batch 3C必须补齐内容寻址material bundle及其逐字节复核、真实raw→projection canonicalizer、BuildInfo/SBOM parser、真实离线builder和每run独立envelope，再由pair policy校验不同issuer key/app/trust domain/runner group与instance，最终由versioned trust-root release决定issuer资格和受保护base authority；禁止把statement、snapshot metadata或pair验证通过解释成Formal Freeze。
