...
| D0-02 | 受限契约缺口、Owner 联合审批与 Contract Review Freeze | 按 ADR 结果迁移现有 canonical projection；只允许关闭 R01 failed-after/outcome-aware 与 child exact-set 等已知审批缺口；为 R00～R08 分别生成 machine-readable freeze baseline 与 CI guard | 待开始；10 项 P0 必须由 Owner 显式 Approved；缺少 `freeze_id/contract_manifest_sha256/vector exact-set/target tests/owner approval manifest` 时不得标记 Review Frozen |
| D1-01 | 结构化 Smoke Harness 前移 | Shell 只编排 Runtime；独立 `smoke/` Node/TypeScript + YAML Registry 承担步骤、断言、故障注入和 Evidence；先迁移 1～2 条 W0 场景 | 可与 D0-01/D0-02 并行开始；只依赖 `W2-ADR-009` 与冒烟工程设计，不依赖 R00～R08、领域 IDL 或 Tool 设计；不得新增根 Go Module或生产测试后门 |
...
| 顺序 | 评审项 | 必须冻结的结论 | 关闭证据 |
| --- | --- | --- | --- |
| W2-R00 | Graph Execution Billing v1 | `Prepare/Get/FinalizeBillableExecution`、Charge/Ledger/ModelReceipt 关联、计价版本、唯一授权模式、Unknown Outcome、失败不退款、invoke 前/dispatch 后崩溃边界与 W4 冲正边界 | Business/Agent/财务/安全联合 Approved；IDL、状态机、幂等键、真实 PG/RPC/故障测试方案与 machine-readable Review Freeze 齐备 |
...
| W2-R00 | 尚无独立 Graph Execution Billing v1 设计；目标文件建议为 `docs/design/cross-module/graph-execution-billing-contract-v1.md` | Missing；ADR-005 只有推荐候选，未批准授权模式、IDL、计价、崩溃窗与失败/冲正边界 | Business/Agent/财务/安全先联合起草并 Approved R00；未冻结前 B0a 不得创建生产计费 Handler/Migration |
...
- **Contract Review Freeze（D0-02 按 Gate 生效）**：Integration Owner 在 `docs/design/agent/approvals/w2-review-freeze-manifest.json` 或其 Approved 后继版本中串行维护治理状态；Agent contract tests 只读取并验证，实施 Agent 不得修改自己的批准状态。没有机器基线只能称 Expansion Frozen，不能称 Review Frozen 或 Approved。

每个 Gate 的机器条目至少包含：

| 字段 | 要求 |
| --- | --- |
| `freeze_id` | 稳定 `CF-W2-<gate>-v<n>` |
...
| `frozen_at` | UTC 时间 |
| `status` | `review_frozen / approved / reopened` |
| `owner_approval_manifest_sha256` | 受影响 Owner 的不可变审批清单摘要；不得用文档作者代替 |
...
CI 固定提供 `make check-w2-review-freeze` 与 `make check-w2-tool-release`（实现前先冻结命令契约）。至少拒绝：SHA/vector/target-test 漂移、approval ref 摘要不匹配、无 CFE 的 `approved → reopened`、未重新审批的 `reopened → approved`、R09 引用 stale/reopened Freeze，以及 availability 已切换但 Release Manifest 未通过。
