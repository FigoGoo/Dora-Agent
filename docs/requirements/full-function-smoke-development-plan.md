...
| W2-R02/R03 | [`runner-session-lane-review-v1.md`](../design/agent/runner-session-lane-review-v1.md)、[`session-lane-runtime-contract-v1.md`](../design/agent/session-lane-runtime-contract-v1.md)、[`session-lane-ingress-command-contract-v1.md`](../design/agent/session-lane-ingress-command-contract-v1.md)、[`session-lane-postgresql-design-v1.md`](../design/agent/session-lane-postgresql-design-v1.md)、[`session-lane-legacy-upgrade-contract-v1.md`](../design/agent/session-lane-legacy-upgrade-contract-v1.md)、[`session-event-foundation-marker-v1.md`](../design/agent/session-event-foundation-marker-v1.md)、[`immutable-turn-context-design-v1.md`](../design/agent/immutable-turn-context-design-v1.md)、[`immutable-turn-context-decision-review-v1.md`](../design/agent/immutable-turn-context-decision-review-v1.md) 与 [`aigc-contract-catalog.md`](../design/cross-module/aigc-contract-catalog.md) | R02 已形成 Agent-owned Executable Draft：60 条 Lane、42 条 Ingress、17 条 legacy Authority、90 条 upgrade blocker、19 条 Marker（含 3 个合法 golden）和 32 条 Message Set full-array 向量已固定；后者覆盖空集合、Unicode 强类型正文摘要、User→Assistant ToolCall→Tool Result→Assistant、六 Tool key/Receipt 因果、256 上限、cutoff 隔离、逐字段 sensitivity 与失败关闭，但 58 字段 Turn Context golden 尚未实现。独立 Marker 为 Review Ready 候选，Turn Context 10 项 P0 已收敛为待 Owner 签字的推荐决策，均未 Approved。真实 PostgreSQL 16 已在精确 Migration 005 上逐值通过 V1/V2/Unicode empty 三类 Foundation cohort，并纳入 required-mode，因此状态推进为 `REAL PG PRECONDITION READY`；但 forward Migration/Helper/Verify/Down、Receipt/Message/Event 不可变、生产 Runtime、Keyring/Redis/lost-wake/Drain/SMK Evidence 仍不存在，R03 仍为 Draft | 先联合审批 Turn Context 10 项 P0、独立 Marker/Retention、Receipt/Message/Event 不可变 DDL、升级 Ledger 和 Claim/Fence/Effect Evidence；期间只继续独立 test-only Corpus；再实现 forward PG Migration/Repository/Helper 与真实 upgrade/race/crash/Down matrix；全部真实 PG 证据批准后才接 Lane Capability Readiness、Eino Runner/Checkpoint、Redis/Scanner/Drain；SMK-018 Worker 主体留在 W3 |
