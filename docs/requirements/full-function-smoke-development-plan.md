...
| D0-04B | Validator transitive build-input closure | 把 R01/R04 verifier 迁入独立 entrypoint package，拆除共享 `contract_test` 包的传递缠绕；冻结 import/embed/外部模块/toolchain/workflow/action 构建输入 exact-set | 进行中、formal Freeze blocker。批次一已提交 `19236995`：新增仅允许 `candidate_unactivated` 的 optional build-closure Schema，硬绑定 R01/R04目标 entrypoint/package/dependency policy、Go 1.26.3候选环境、direct source/Test/import/Module selected-source与 aux/vendor静态边界，并对 missing/null/invalid UTF-8、跨 Gate冒用和 formal升级失败关闭；现有 manifest仍未引用扩展，两个 blocker均保留。既定混合方案仍是 R04迁到 stdlib-only `w2r04approvalconsumption`，R01迁到 `w2r01`并保留受管 `x/text/unicode/norm`，R03只经窄 facade复用 Receipt evaluator。下一批原子迁移 package/manifest/摘要；之后实现带 module zip+h1 provenance的真实 resolver、锁定 toolchain下 `go list`/typecheck/compile attestation及 base-owned Action/workflow。全部完成前不得 formal Freeze或移除两个 `VALIDATOR_BUILD_CLOSURE_PENDING` |
