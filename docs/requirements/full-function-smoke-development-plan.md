...
> W2 当前新增评审输入：[Graph Execution Billing v1](../design/cross-module/graph-execution-billing-contract-v1.md)、[全功能冒烟工程设计 v1alpha2](../design/testing/full-function-smoke-engineering-design.md) 与 [W2 Review Freeze 机器治理清单](../design/agent/approvals/w2-review-freeze-manifest.json)。三者均不得被解释为生产实现或 Owner Approved。
...
| D0-02 | 受限契约缺口、Owner 联合审批与 Contract Review Freeze | 按 ADR 结果迁移现有 canonical projection；只允许关闭 R01 failed-after/outcome-aware 与 child exact-set 等已知审批缺口；为 R00～R08 分别生成 machine-readable freeze baseline 与 CI guard | 治理骨架已落地：R00～R08 exact-set、状态、候选摘要、required Owner、正式 Freeze/CFE 形状和只读 Agent guard 已机器化；R01 仅 `awaiting_review`，R04 仅 partial candidate，其余 Expansion Frozen，当前没有 Gate Review Frozen/Approved。跨 commit transition-diff 与 R09 release guard 仍待后续批次 |
...
| W2-R01 | [`graph-tool-result-receipt-contract-v1.md`](../design/agent/graph-tool-result-receipt-contract-v1.md)、[`runner-session-lane-review-v1.md`](../design/agent/runner-session-lane-review-v1.md) 与 [`aigc-contract-catalog.md`](../design/cross-module/aigc-contract-catalog.md) | Agent-owned Review Ready 基线仍未 Approved；8 个合法 Result、40 个 Result 拒绝、24 个 Receipt 迁移和 13 个跨状态 evidence 向量已由 Agent `GOWORK=off` 契约测试消费，覆盖 Unicode/UUID、JS safe integer、稳定错误优先级、pinned slot/ref/effect-class、Fence/CAS 及既有状态 evidence。R03 child 复核新增 `permanent_failure_after_side_effects_resolved` / outcome-aware negative authority 设计候选，但尚无 R01 Corpus 正反向向量；未创建生产 DTO/Repository/Migration | 先关闭 W2-ADR-001/002，明确 canonical projection 与物理表、三类 Digest 的边界；仅当 ADR/Owner 仍要求时补 failed-after/outcome-aware 与 exact refs 的最小向量，再联合审批 ResultCode、slot/CAS 和跨 Module 版本策略 |
...
| `frozen_at` | UTC 时间 |
| `owner_approval_manifest_sha256/approval_summary_sha256` | 受影响 Owner 的不可变审批清单摘要；summary 必须绑定 status、Freeze、path、时间、manifest、vector/test exact-set 与可选 CFE，不得用文档作者代替 |
...
当前已提供 `make check-w2-review-freeze`，拒绝 Gate/Owner exact-set、SHA/vector/target-test、formal Freeze、approval summary、自批、CFE/reapproval 和路径漂移。跨 commit 的历史 transition-diff 仍需独立 guard，当前不能仅凭单文件 shape test 宣称已覆盖删除旧 Freeze 后降级的历史篡改。`make check-w2-tool-release` 在 R09 Schema/首个 release manifest 获批后实现；届时必须拒绝 R09 引用 stale/reopened Freeze，以及 availability 已切换但 Release Manifest 未通过。
