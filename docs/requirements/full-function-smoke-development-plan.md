...
| 本地基础设施 | PostgreSQL、Redis、etcd Compose、三独立数据库、独立 `_test` 契约库、Schema Migration、`foundation-smoke`、`w0-smoke`、`w05-browser-smoke` 与 W1 canonical 门禁已可执行；`w1-smoke` 强制执行 `@w1-real-review` 真实 Chromium 链路。W1 Foundation canonical `v3` 固定 47 项 assertions/42 项布尔门禁；同次独立发布 Governance `v1` 五项、Market `v2` 六项和 Public Market Binding `v1` 七项闭集 sidecar。四份均校验顶层/assertions exact-set、共用 run/source digest，全部通过才原子发布 passed | 继续以真实基础设施运行 canonical + 三 sidecar 四 Evidence 门禁；既有 Project Binding v2 Feature Flag 在普通配置中默认关闭，只由确认同批 Agent capability 的 Smoke/部署显式开启，W1-F 不新增 public-market 专用 Flag |
...
- **M1.6 W1-D Skill Governance 后端纵切与四角色冒烟（已通过）**：Business 已实现 `skill_governor -> skill.govern` 动态投影、治理列表/详情/决定 HTTP、Strong ETag、幂等回执、append-only 审计，以及 `active -> suspended -> active -> offline` 状态机。`make w1-smoke` 的独立 `w1.skill-governance.smoke.evidence.v1` 五项闭集 sidecar 已用 Governor 真实 Login、正式 CLI 撤权、QuickCreate/既有 Session 和数据库事实证明同义重放只产生一次迁移、暂停/下架不产生部分 Project、恢复后的新 Resolution 固化 epoch 3、offline 终态不可恢复、offline 后批准不改发布指针。治理页面、Owner 原因安全投影、申诉、在线角色管理和完整 `SMK-031` 仍未完成。
...
同日，`SMK-005/006` 的 W1 API/数据库和页面子集已经由 `w1.skill-foundation.smoke.evidence.v3` 真实黑盒通过：Skill 草稿/更新/审核、Owner-safe 读取与修改、持久化 Reviewer 角色分配与动态 Session 权限投影、真实 HTTP 队列/冻结详情/批准发布、同义回执重放、同 Cookie 撤权即时失效、`public_tool_refs` 失败关闭、100 并发非空绑定 QuickCreate v2、Business→Agent Snapshot、三方摘要与数量逐值一致、正式 Load 解密重验、密文清理、V1/V2 Session 隔离，以及不依赖 API mock 的 Creator → Reviewer → Published Skill 选择 → Workspace 单一 Chromium 全链。真实浏览器还证明 Reviewer 对 Creator Owner 草稿的路由 GET 和带合法 CSRF/ETag/Definition 的显式 PUT 都返回严格防枚举 404，不显示编辑表单或当前草稿事实，并由 Creator 后置 Owner GET 证明草稿 ETag 与 Definition 未变化；这加强 W1-C1 跨用户隔离，但不替代完整管理员数据范围与越权审计。W1-B2/W1-C1 的六 Tool 静态目录也已由同一 Evidence 的 API exact-set、跨 Owner 404 和真实浏览器六项禁用投影通过；这只是 `SMK-007/008` 的契约准备，不能宣称 Tool 可执行。独立 Governance `v1` sidecar 又真实覆盖了 Governor 后端 active/suspended/offline 状态机、QuickCreate 门禁、offline 终态和撤权即时失效，但没有治理 Chromium 页面。完整 `SMK-005/006` 仍保持 `待实现`，因为 Skill 测试调用、治理管理页面及完整浏览器失败矩阵尚未覆盖。
...
| D0-02 | 受限契约缺口、Owner 联合审批与 Contract Review Freeze | 按 ADR 结果迁移现有 canonical projection；只允许关闭 R01 failed-after/outcome-aware 与 child exact-set 等已知审批缺口；为 R00～R08 分别生成 machine-readable freeze baseline 与 CI guard | 本地候选门禁已覆盖 Corpus 子文件 SHA/count、设计/Validator/build 源摘要、CFE 摘要、正式状态逐步迁移、旧 Freeze/Approval/CFE append-only、两阶段 CFE diff 和签字 commit ancestry；verifier 独立信任根与 Git `100644 blob` 检查用于失败关闭。R01/R04 都只保留 partial candidate，R00～R08 全部 `expansion_frozen`，当前没有 Gate Review Frozen/Approved。D0-04A 已完成 direct package source + Module metadata closure，但 embed、内部/第三方传递依赖、受信 toolchain/action 尚未闭合；GitHub enforcement 也仍须合入默认分支 `main`、完成真实 PR 验证与 Ruleset 配置，Owner role 到受信 GitHub identity/review 的映射仍未建立。此前不得创建正式 Freeze。R09 release guard 仍待后续批次 |
| D0-03 | R01 候选一致性、验证器 Source Closure 与 Owner Decision Matrix | 修正非法测试 Tool Key、replay 夹带字段短路、设计源摘要缺失；绑定 target test/validator 源码摘要；允许 pre-formal Owner exact-set 按决议受控调整；把 exact-version、authority_outcome、card owner、Registry 范围、Owner exact-set 与 GitHub identity/review authority 收敛为可逐项签字的决策矩阵 | 已把非法 `storyboard_generation` 收敛为 `plan_creation_spec`，补齐 replay piggyback 两条拒绝向量，将 R01 可靠候选子集从 85 条修正为 87 条，并建立 [`W2-R01 Owner 决策矩阵 v1`](../design/agent/w2-r01-owner-decision-matrix-v1.md) 与文件级 source closure；D0-04A 已继续补齐直接包源码与 Module 元数据。R01 仍是 partial candidate，下一步并行关闭 D0-04B、受信 GitHub Owner authority 和矩阵中的语义决定，不自行补写身份、角色或产品结论；完成前所有 Gate 必须保持 pre-formal |
| D0-04A | Validator direct source + Module metadata 闭包 | 冻结共享 `contract_test` 包全部直接 Go 源与 Agent Module 元数据，阻止未登记源码、build-tag/TestMain 测试选择、`go.mod/go.sum` 漂移和显式 dependency replacement 注入 | 已完成：R01/R04 manifest 都绑定 `agent/tests/contract` 全部 11 个直接 `.go` 文件及 `agent/go.mod`、`agent/go.sum` 的 raw SHA，校验 direct package exact-set、Git 普通文件模式，拒绝 modern/legacy build constraint、package-level `TestMain` 与任何 `replace`；直接源码增删、模式变化、重绑后的 TestMain/build constraint、Module 漂移与 re-bound replace 对抗测试均失败关闭。该结论不等于完整 build closure |
| D0-04B | Validator transitive build-input closure | 优先把各 Gate verifier 迁入独立、stdlib-only 且无未登记 embed 输入的包；若保留共享包，则冻结 embed、内部 Go 依赖、第三方模块、受信 toolchain 与 workflow/action digest 的完整传递闭包 | 待完成、formal Freeze blocker：当前共享 `contract_test` 包含多组 `//go:embed`，并导入 `agent/internal/event`、`agent/internal/session`、`agent/internal/skill` 与 `github.com/google/uuid`；这些输入及 toolchain/action 尚未由 R01/R04 manifest exact-set 绑定。优先实施独立 stdlib-only verifier，避免继续扩大跨 Gate 重审面 |
| D1-00 | W2-S0-G0 Smoke 治理前置 | 独立 W2-ADR-009、Context/Registry 契约、SMK-004A shadow baseline 与七方 approval manifest；未批准时机器拒绝 `smoke/**` Harness | D0-03 后可与 D0-04B、Owner authority 并行；全程不修改既有 4457 行 Shell/Playwright、不创建 `smoke/**`，先关闭当前纯 Smoke 文件可绕过治理 CI 的缺口 |
| D1-01 | 结构化 Smoke Harness 前移 | Shell 只编排 Runtime；独立 `smoke/` Node/TypeScript + YAML Registry 承担步骤、断言、故障注入和 Evidence；先迁移 1～2 条 W0 场景 | 冒烟工程设计已收敛为 v1alpha2 Review Ready，首切候选固定 `SMK-004A api/ui shadow_parity`；但 `W2-ADR-009` 与设计尚未 Approved，因此本批没有创建 `smoke/**`。获批后才能进入实现，不依赖 R00～R08、领域 IDL 或 Tool 设计 |
| D2-01 | Agent Session Lane Kernel | Forward Migration、Claim/Renew/MarkRunning/Retry/Expire/Release/Fence、HOL、Crash Recovery、`TurnExecutor` 端口及真实 PostgreSQL 竞争测试；不提交 resolved、不创建 Receipt、不注册业务 Graph | `W2-R02 + W2-ADR-001/002` Approved 后开始；确定性 Executor 只通过依赖注入存在于测试/本地 Adapter |
...
近期编号与长计划唯一映射为：`D0-00` 是 W2-D0 的 evidence-only 前置而非实现包；`D0-01/02/03/04A/04B → W2-D0`、`D1-01 → W2-S0`、`D2-01 → W2-A1`、`D2-02 → W2-A2`、`D3-01A → W2-B0a`、`D3-01B → W2-B0b`、`D4-01 → W2-B1`、`D5-01 → W2-C1`、`D6-01 → W2-C2`、`D7-01 → W2-D1/D2a/D2b`、`D8-01 → W3`、`D9-01 → W4～W6`。审批、Evidence 与提交信息必须同时写入这组映射，禁止创造第三套工作包编号。
...
| W1-D | Skill Governor 治理后端与四角色冒烟（当前纵切已完成） | W1-C2、Authorization Service、Project Skill Binding v2 | 四身份/双角色严格隔离；暂停/恢复/offline、QuickCreate 零部分事实、发布指针不变、同义重放与同 Cookie 撤权由独立五项 Evidence 通过；治理 UI 与完整 `SMK-031` 不在本项 |
...
| W2-A1 | 实现 Session Lane Kernel | W0 Session/Input，W2-R02 与 W2-ADR-001/002 Approved | 真实 PostgreSQL Claim/Renew/MarkRunning/Retry/Expire/Release、HOL、Fence、Crash Recovery 和 Redis 关闭兜底；Processor 只依赖 `TurnExecutor` 端口，本阶段不提交 resolved、不创建 Receipt、不注册业务 Graph |
...
| W2-A1 | R02、W2-ADR-001/002 | R03～R07、业务 Tool |
...
| W2-R00 | [`graph-execution-billing-contract-v1.md`](../design/cross-module/graph-execution-billing-contract-v1.md) | Draft / awaiting joint approval；已明确 Business Owner、`preauthorized` 推荐候选、唯一 `tr:` command key + `(logical_tool_execution_id, model_call_ordinal)` domain backstop、Prepare/Get/Finalize、NOT_FOUND 与 NOT_CHARGED 区分、Charge-before-dispatch、两个独立 billing ref slot、post-dispatch quarantine、失败不退款、correction 禁用和 W4 reversal 边界；尚无 Approved IDL/corpus/真实 PG/RPC Evidence | Business、Agent、产品、财务、安全关闭 `BILL-OPEN-001`～`012`，同步 R04 primary ordinal 与 retained slot Registry，冻结 currency/policy/cap/Provider Query/ModelReceipt 及 `authority_outcome` 映射后生成 canonical manifest、向量和 Owner approvals；此前 B0a 继续禁止生产 Handler/Migration |
...
| W2-R01 | [`graph-tool-result-receipt-contract-v1.md`](../design/agent/graph-tool-result-receipt-contract-v1.md)、[`runner-session-lane-review-v1.md`](../design/agent/runner-session-lane-review-v1.md)、[`w2-r01-owner-decision-matrix-v1.md`](../design/agent/w2-r01-owner-decision-matrix-v1.md) 与 [`aigc-contract-catalog.md`](../design/cross-module/aigc-contract-catalog.md) | `expansion_frozen / partial_candidate`；8 个合法 Result、40 个 Result 拒绝、26 个 Receipt 迁移和 13 个跨状态 evidence 向量，共 87 条已校验可靠子集，覆盖 Unicode/UUID、JS safe integer、稳定错误优先级、pinned slot/ref/effect-class、Fence/CAS、replay 命令 shape 及既有状态 evidence。D0-04A 又冻结共享 `contract_test` 包 11 个直接 `.go` 文件与 `agent/go.mod/go.sum` exact-set，并拒绝 `replace`。但 embed、内部/第三方传递依赖、受信 toolchain/action 仍未闭合；failed-after/outcome-aware、Billing authority projection、exact-version、card owner、Registry 范围、Approval/Consumption ordinal scope 和 Owner exact-set 也尚未关闭；未创建生产 DTO/Repository/Migration | D0-03 owner-free 一致性和 D0-04A direct source/Module metadata closure 已收口；下一步优先完成 D0-04B 独立 stdlib-only verifier（或完整传递闭包），并按决策矩阵关闭六项语义决定与受信 Owner authority。仅按明确结论补或删除最小向量，再恢复完整 `gate_candidate/awaiting_review`，不得以当前 87 向量直接生成 Freeze |
| W2-R02/R03 | [`runner-session-lane-review-v1.md`](../design/agent/runner-session-lane-review-v1.md)、[`session-lane-runtime-contract-v1.md`](../design/agent/session-lane-runtime-contract-v1.md)、[`session-lane-ingress-command-contract-v1.md`](../design/agent/session-lane-ingress-command-contract-v1.md)、[`session-lane-postgresql-design-v1.md`](../design/agent/session-lane-postgresql-design-v1.md)、[`session-lane-legacy-upgrade-contract-v1.md`](../design/agent/session-lane-legacy-upgrade-contract-v1.md)、[`session-event-foundation-marker-v1.md`](../design/agent/session-event-foundation-marker-v1.md)、[`immutable-turn-context-design-v1.md`](../design/agent/immutable-turn-context-design-v1.md)、[`immutable-turn-context-decision-review-v1.md`](../design/agent/immutable-turn-context-decision-review-v1.md)、[`approval-continuation-cross-object-evidence-v1.md`](../design/agent/approval-continuation-cross-object-evidence-v1.md)、[`approval-consumption-receipt-contract-v1.md`](../design/agent/approval-consumption-receipt-contract-v1.md)、[`continuation-child-tool-receipt-contract-v1.md`](../design/agent/continuation-child-tool-receipt-contract-v1.md) 与 [`aigc-contract-catalog.md`](../design/cross-module/aigc-contract-catalog.md) | R02 已形成 Agent-owned Executable Draft：60 条 Lane、42 条 Ingress、17 条 legacy Authority、90 条 upgrade blocker、19 条 Marker（含 3 个合法 golden），以及 Message Set/Turn Context 联合 8 个 fixture、70 条向量和 27 个目标测试已固定。R03 以 1 个 fixture、20 条跨对象向量和 13 个目标测试绑定 frozen Receipt/Approval/Decision/Pin/Context，并新增 4 个 fixture、111 条向量、11 个目标测试的 `creation_spec_activation` unsigned Consumption Core candidate evidence，覆盖显式 intent/first-write、stored core/index、重复集合失败关闭、scope authorization、Query/expected-binding 与 single-use。child 只有 Design Draft / Corpus Pending；生产/认证 Consumption、child、Business Decide/Query、真实 PG 原子性仍不存在。10 项 P0 继续由机器 manifest 固定为 `awaiting_owner_approval`；真实 PostgreSQL 16 Migration 005 仍只达到 `REAL PG PRECONDITION READY` | 保持 TC-P07 全部 unmet；先关闭 W2-ADR-001～004 与 W2-ADR-011，再按决议迁移或保留现有 projection。只补 Owner 明确要求的 child exact-set 最小 Corpus，随后联合审批 Turn Context、Marker/Retention、DDL、升级 Ledger、Approval/Consumption 与 Claim/Fence/Ref；Approved 后先实现 Lane/PG Kernel，SMK-018 Worker 仍留在 W3 |
...
| W2-R04 | [`plan_creation_spec-design.md`](../design/agent/graphtool/plan_creation_spec-design.md)、[`plan_creation_spec-w2-r04-gap-review.md`](../design/agent/graphtool/plan_creation_spec-w2-r04-gap-review.md)、[`approval-consumption-receipt-contract-v1.md`](../design/agent/approval-consumption-receipt-contract-v1.md) 与 [`continuation-child-tool-receipt-contract-v1.md`](../design/agent/continuation-child-tool-receipt-contract-v1.md) | Draft；`creation_spec_activation` Consumption unsigned core 已有 111 条 test-only 向量，child root/parent/双 guard/prepared/unknown 形成 Design Draft，但 child 与 failed-after Corpus pending；Intent/Candidate、逐模型计费、未来收益所需 immutable attribution、Business RPC、typed fan-in/Output 等 headless P0 仍未关闭，A2UI 由 R08 单独门禁 | 先关闭 activation key、Action/Effect 映射、Consumption Query 鉴权、`preauthorized` 或完整 billable Approval 的执行授权模式与最小计费 ADR，消除两套激活身份和两套 normal topology；再补 Owner 明确要求的最小缺口并联合审批共享 R01/R02/R03 与本 Tool，R08 在 W2-C1 前独立 Approved。Approved 后它是唯一首批可执行 Tool |
...
当前分支提供两层本地候选门禁：`make check-w2-review-freeze` 在独立 `agent/tests/reviewfreeze` package 校验 HEAD shape、Corpus manifest 及其子文件 SHA/count、Gate/Owner exact-set、formal Freeze、approval summary、自批、CFE/reapproval 和路径；`make check-w2-review-freeze-transition` 使用明确 base/head commit 校验逐 Gate 历史迁移、旧正式产物 append-only、CFE 两阶段 `allowed_files` 真实 diff、签字 commit ancestry 与 Git tree 普通文件模式。base-owned workflow 限定 `main`，只 checkout base、读取 PR head Git objects 并直接执行 base verifier，不执行 PR 代码；workflow 与 verifier 目录一旦在 base 激活即构成不可同 PR 修改的 v1 信任根，后续升级必须走显式版本化治理变更。

远端默认分支当前是 `main`，但它尚不包含该 workflow，因此本分支上的本地测试不等于 GitHub enforcement 已激活。D0-04A 只关闭 direct package source 与 Module metadata 注入；必须先完成 D0-04B transitive build-input closure 和 Owner identity/review authority，再将最终信任根合入默认分支。随后用真实 PR 验证 workflow 的事件、head/check 关联与失败关闭语义，并用 Ruleset 固定 required workflow、required Owner review、strict up-to-date、merge-commit-only、禁止 direct push/bypass。未完成这些代码与外部设置前状态保持“CI 候选”，任何自报 role/URL 都不得解释为真实联合批准。`make check-w2-tool-release` 在 R09 Schema/首个 release manifest 获批后实现；届时必须拒绝 R09 引用 stale/reopened Freeze，以及 availability 已切换但 Release Manifest 未通过。
