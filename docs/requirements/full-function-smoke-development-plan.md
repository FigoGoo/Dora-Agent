...
| 本地基础设施 | PostgreSQL、Redis、etcd Compose、三独立数据库、独立 `_test` 契约库、Schema Migration、`foundation-smoke`、`w0-smoke`、`w05-browser-smoke` 与 W1 canonical 门禁已可执行；`w1-smoke` 强制执行 `@w1-real-review` 真实 Chromium 链路。W1 Foundation canonical `v3` 固定 47 项 assertions/42 项布尔门禁；同次独立发布 Governance `v1` 五项、Market `v2` 六项、Public Market Binding `v1` 七项和 Republish Session Isolation `v1` 三十三项闭集 sidecar。五份均校验顶层/assertions exact-set、共用 run/source digest，并由同一 release 目录和 `current.json` 单提交点组原子发布 | 继续以真实基础设施运行 canonical + 四 sidecar 五 Evidence 门禁；既有 Project Binding v2 Feature Flag 在普通配置中默认关闭，只由确认同批 Agent capability 的 Smoke/部署显式开启，W1-F 不新增 public-market 专用 Flag |
...
- **M1.5 W1 Skill Foundation、Reviewer 纵切与 Session Snapshot（W1-A1/W1-B1/W1-C2 主路径已通过）**：Business 已实现结构化 Skill 草稿、Owner 读取、审核提交、持久化 `skill_reviewer` 分配、登录与每次 Session Resolve 的动态权限投影、Reviewer 队列/冻结详情/当前发布对比/批准发布、正式发布事务、Project Binding/Resolution、完整加密 Bootstrap v2 和 active/previous keyring；Agent 已实现数据库级 append-only Session Snapshot Header/Item、专用用途派生 AES-GCM、Ensure/Query v2、Receipt 语义重验和 V1/V2 command 隔离；前端已实现 Owner Skill Builder、Reviewer 管理入口与 QuickCreate Skill Picker，空选择严格保持 v1、非空选择显式进入 v2，并在第 17 项前失败关闭。W1-C2 canonical 门禁命令 `make w1-browser-smoke`（以及其兼容别名 `make w1-smoke`）已用真实 PostgreSQL/Redis/etcd、当前 worktree 构建的真实 Business/Agent Runtime 和 Chromium 证明 100 并发幂等、发布/治理审计、Business/Agent/verifier 三方 digest/count 一致、Agent 正式 Load 解密重验、Outbox 密文清理、旧 V1 Session 不回写，以及单一真实浏览器中的 Creator 直达 Reviewer 管理路由失败关闭且不隐式读取队列、同 Cookie 显式读取审核 API 得到严格 403 并关联结构化拒绝审计、Reviewer 对 Creator Owner 草稿的真实 GET/PUT 均防枚举 404 且不泄漏当前草稿、Creator 提交 → Reviewer 批准 → Creator 选择已发布 Skill → QuickCreate v2。2026-07-16 新增的 `w1.skill-republish-session-isolation.smoke.evidence.v1` 又以 33 项闭集证明同一 Skill 发布 Runtime Content 不同的 A、B 后：两次 Review→Content Revision→Published Snapshot 血缘逐值一致且 Revision 不同；旧命令在 B 发布后仍重放原 A Project/Session，新命令冻结 B；两侧各只有一套九类 Business 创建事实和一套 Agent Session 事实；Business/Agent/verifier 摘要链一致且 A/B content/runtime/set digest 不同；Owner list/detail 仍为 10 字段安全投影且无版本/历史/回滚 UI。五份 Evidence 先进入同一 staging/release 目录，只有 `current.json` 最终 rename 成功才被消费者认可。脚本对任何缺少 `@w1-real-review` 的 W1 调用失败关闭，禁止生成 canonical W1 v3 passed Evidence。Smoke 还通过部署控制的窄权限 CLI 撤销 Reviewer 分配，并证明同一 Cookie 再次 Resolve 后角色/capability 为空且管理 API 返回 403。Creator 负向链记为 `SMK-001A` Reviewer capability isolation 子集；Reviewer 对 Creator 草稿的链只加强 W1-C1 跨用户隔离。完整 ADM-RBAC、在线角色管理、数据范围、敏感字段/导出越权与审核驳回/撤回仍是后续工作。
...
| P1-01 | 三 Module 和本地基础设施骨架 | 独立构建、Migration、Compose、CI、健康检查 | M1.1、W0/W0.5 与 W1-A1～W1-G 已完成并真实黑盒通过；Graph/Worker/财务域仍按后续波次推进 |
| P1-02 | 前端接入底座 | API Client、认证会话、错误映射、SSE 游标重连、开发代理和 CI | 正式鉴权、QuickCreate、Workspace/SSE、Skill Owner/Reviewer、公开市场读取及跨发布者登录预选已接线并通过真实 Chromium；完整治理和后续业务页面继续按领域波次推进 |
...
| W2 同步创作能力 | `SMK-007`～`SMK-012` | Session Lane、Runner、Executable Tool、Creation Spec、素材分析、Storyboard、Prompt 与正式 Approval | 共享 Runtime/A2UI 契约和四个同步 Graph Tool 设计审核、Compile、状态机、Receipt、真实 Run 与前端恢复全部通过 |
...
| W1-G | Skill A→B Republish Session Isolation（当前纵切已完成） | W1-C2、Project Binding、Session Snapshot v2 | 同一 Skill 二次审核发布与修订血缘、旧/新 QuickCreate replay、旧 Workspace 回访、Owner 无版本 UI，以及 Business/Agent/verifier 的 A/B content/runtime/set digest 隔离由独立 33 项 Evidence 通过；完整 `SMK-006` 仍待 Skill 测试调用与失败矩阵 |
| W2-A | 实现 Runner、Session Lane、Turn/Receipt/A2UI 共享基础 | W0 Session/Input，W2-R01/R02/R03/R08 Approved | HOL 串行、冻结输出重放、Checkpoint 恢复、未知 A2UI 失败关闭；Executable Registry 尚不开放业务 Tool |
| W2-B | 评审并实现 `plan_creation_spec`、`analyze_materials` | W2-A、两份独立 Graph Tool 设计 Approved | Graph Compile、确定性 Validator、Receipt、partial/failed |
| W2-C | 评审并实现 `plan_storyboard`、`write_prompts`，接通显式 Tool Pin 与真实 Run | W2-A、W2-B、Storyboard Owner/RPC、Approval 契约和两份 Tool 设计 Approved | Pending/Active 隔离、正式审批、锁定项、原子写入，并以正式 A2UI/前端恢复 Evidence 关闭 `SMK-007/008` |
...
| W2-R01 | [`graph-tool-result-receipt-contract-v1.md`](../design/agent/graph-tool-result-receipt-contract-v1.md)、[`runner-session-lane-review-v1.md`](../design/agent/runner-session-lane-review-v1.md) 与 [`aigc-contract-catalog.md`](../design/cross-module/aigc-contract-catalog.md) | Agent-owned Review Ready 基线仍未 Approved；9 个合法 Result、41 个 Result 拒绝、24 个 Receipt 迁移、13 个跨状态 evidence，以及 failed-after/outcome-aware 的 3 个合法、8 个拒绝向量已由 Agent `GOWORK=off` 契约测试消费，覆盖 Consumption committed exact refs、Business `not_committed`、prepared unknown、漏 ref 和 outcome 严格校验；未创建生产 DTO/Repository/Migration | 由 Agent/Business/安全/运维/财务审核 Corpus、摘要域、slot/CAS，并联合评审/确认已由 172 条向量固定的 child Decision/action slot exact-set；各 Tool canonical request fixture、跨 Module exact-version 例外与 W2-R08 `card_id` 仍需继续冻结 |
| W2-R02/R03 | [`runner-session-lane-review-v1.md`](../design/agent/runner-session-lane-review-v1.md)、[`session-lane-runtime-contract-v1.md`](../design/agent/session-lane-runtime-contract-v1.md)、[`session-lane-ingress-command-contract-v1.md`](../design/agent/session-lane-ingress-command-contract-v1.md)、[`session-lane-postgresql-design-v1.md`](../design/agent/session-lane-postgresql-design-v1.md)、[`session-lane-legacy-upgrade-contract-v1.md`](../design/agent/session-lane-legacy-upgrade-contract-v1.md)、[`session-event-foundation-marker-v1.md`](../design/agent/session-event-foundation-marker-v1.md)、[`immutable-turn-context-design-v1.md`](../design/agent/immutable-turn-context-design-v1.md)、[`immutable-turn-context-decision-review-v1.md`](../design/agent/immutable-turn-context-decision-review-v1.md)、[`approval-continuation-cross-object-evidence-v1.md`](../design/agent/approval-continuation-cross-object-evidence-v1.md)、[`approval-consumption-receipt-contract-v1.md`](../design/agent/approval-consumption-receipt-contract-v1.md)、[`continuation-child-tool-receipt-contract-v1.md`](../design/agent/continuation-child-tool-receipt-contract-v1.md) 与 [`aigc-contract-catalog.md`](../design/cross-module/aigc-contract-catalog.md) | R02 已形成 Agent-owned Executable Draft：60 条 Lane、42 条 Ingress、17 条 legacy Authority、90 条 upgrade blocker、19 条 Marker（含 3 个合法 golden），以及 Message Set/Turn Context 联合 8 个 fixture、70 条向量和 27 个目标测试已固定。R03 以 1 个 fixture、20 条跨对象向量和 13 个目标测试绑定 frozen Receipt/Approval/Decision/Pin/Context，并新增 4 个 fixture、102 条向量、11 个目标测试的 `creation_spec_activation` unsigned Consumption Core candidate evidence。独立 child 又以 1 个 fixture、172 条向量和 11 个目标测试固定 52 个 canonical 叶子字段、root/parent/current、Decision、approve/reject exact-set、双 guard、Fence 与 Business unknown/query 恢复，并实际执行 R04/R01 bridge；再加 70 条 BIZ-AIGC-008 Agent consumer-side command/query/authority 向量后，W2 test-only 总量为 760 条。生产/认证 Consumption、child、正式 Business Decide/Query、真实 PG 原子性仍不存在。10 项 P0 继续由机器 manifest 固定为 `awaiting_owner_approval`；真实 PostgreSQL 16 Migration 005 仍只达到 `REAL PG PRECONDITION READY` | 保持 TC-P07 全部 unmet，联合评审 child Corpus、Turn Context、Marker/Retention、不可变 DDL、升级 Ledger、Approval/Consumption、Claim/Fence/Effect 与 BIZ-AIGC-008 consumer-side 候选；之后才可实现 forward PG Migration/Repository/Helper、公共 Decide/Query IDL 与真实 upgrade/race/crash/Down，最终才接 Runner/Checkpoint、Redis/Scanner/Drain；SMK-018 Worker 留在 W3 |
...
| W2-R04 | [`plan_creation_spec-design.md`](../design/agent/graphtool/plan_creation_spec-design.md)、[`plan_creation_spec-w2-r04-gap-review.md`](../design/agent/graphtool/plan_creation_spec-w2-r04-gap-review.md)、[`approval-consumption-receipt-contract-v1.md`](../design/agent/approval-consumption-receipt-contract-v1.md)、[`continuation-child-tool-receipt-contract-v1.md`](../design/agent/continuation-child-tool-receipt-contract-v1.md) 与 [`creation-spec-candidate-decision-contract-v1.md`](../design/cross-module/creation-spec-candidate-decision-contract-v1.md) | Draft；`creation_spec_activation` Consumption unsigned core 已有 102 条 test-only 向量，共享 failed-after/outcome-aware R01 增量已有 11 条向量；child root/parent/双 guard/prepared/unknown 已形成 172 条独立 test-only Corpus 并实际执行 R04/R01 bridge；BIZ-AIGC-008 另有 70 条 Agent consumer-side strict union、authority 与 unknown Query 向量。Intent/Candidate、成功 ResultCode/资源投影、逐模型计费、收益归因、生产 slot ordinal、认证 envelope、Business producer parity/正式 IDL/PG、typed fan-in/Output、A2UI 等 P0 仍未关闭 | 联合评审 child、BIZ-AIGC-008 test-only candidate 与共享 R01/R02/R03/R08 证据，冻结成功 ResultCode、生产 slot Registry 和 Business Decide/Query 公共契约；再逐项关闭 Tool P0 并重画计费/恢复拓扑，Owner Approved 前不实现 Graph |
...
独立 Eino 依赖评审已经完成，但以上共享与 Tool 文档尚未 Approved。契约测试向量可执行且对应文档 Approved 前，不创建 Runner、Graph、Executable Registry、相关 Migration 或前端 Approval Action；六个目录项继续保持 `unavailable / DESIGN_REVIEW_PENDING`。
