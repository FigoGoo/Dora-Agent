...
| D0-04A | Validator direct source + Module metadata 闭包 | 冻结共享 `contract_test` 包全部直接 Go 源与 Agent Module 元数据，阻止未登记源码、build-tag/filename/TestMain 测试选择、`go.mod/go.sum` 漂移和显式 dependency replacement 注入 | 已完成：R01/R04 manifest 都绑定 `agent/tests/contract` 全部 11 个直接 `.go` 文件及 `agent/go.mod`、`agent/go.sum` 的 raw SHA，校验 direct package exact-set、Git 普通文件模式，拒绝 modern/legacy build constraint、Go 忽略/GOOS/GOARCH 平台选择文件名、package-level `TestMain` 与任何 `replace`；直接源码增删、模式变化、重绑后的 TestMain/build constraint/filename、Module 漂移与 re-bound replace 对抗测试均失败关闭。该结论不等于完整 build closure |
| D0-04B | Validator transitive build-input closure | 把 R01/R04 verifier 迁入独立 entrypoint package，拆除共享 `contract_test` 包的传递缠绕；冻结 import/embed/外部模块/toolchain/workflow/action 构建输入 exact-set | 进行中、formal Freeze blocker。已确定“独立包为骨架 + R01 NFC 保留受管 `x/text/unicode/norm` 闭包 + exact file-list 仅作迁移验证”的混合方案：R04 迁到 stdlib-only `w2r04approvalconsumption`，本地持有 strict JSON/digest/UUID/manifest helper；R01 迁到 `w2r01`，以标准库 canonical UUIDv7 校验取代 `google/uuid`，取消 embed，但不手写弱化 NFC，而是绑定 `x/text` module/sum 与 build-selected 源文件；R03 只经窄 facade 复用 Receipt evaluator。按“先扩 validator schema/entrypoint 校验→原子迁移 R01/R04 与 R03 facade/摘要→闭合 Go 1.26.3 与 Action 全长 SHA 并跑 base→head”三批执行；完整验证前保留两个 `VALIDATOR_BUILD_CLOSURE_PENDING` blocker |
| D1-00 | W2-S0-G0 Smoke 治理前置 | 独立 W2-ADR-009、Context/Registry 契约、SMK-004A shadow baseline、七方 approval manifest 与 Go 候选校验器；真实 worktree 和显式 base/head Git object 都可验证 | 当前只是 `candidate_unactivated`：无 Smoke Governance workflow、Ruleset 或 required check，不得声称已建立远程机器门禁。全程不修改既有 4457 行 Shell/Playwright、不创建 `smoke/**`；正式启用必须另走 release/rekey/handoff Bootstrap PR，固定 workflow/Action/Validator/构建输入摘要，配置 source-locked/no-bypass Ruleset 并通过同仓/fork canary |
| D1-01 | 结构化 Smoke Harness 前移 | Shell 只编排 Runtime；独立 `smoke/` Node/TypeScript + YAML Registry 承担步骤、断言、故障注入和 Evidence；先迁移 `SMK-004A` 的 14 项 API 和 12 项 UI shadow assertion | 冒烟工程设计已收敛为 v1alpha2 Review Ready，26 项 shadow + 8 项 canonical-only 候选 exact-set 已由 baseline 绑定；但 `W2-S0-G0` 仍为 awaiting/locked，且 trust root 未激活，因此本批没有创建 `smoke/**`。只能在独立 unlock-only 迁移通过后的下一 PR 进入实现 |
