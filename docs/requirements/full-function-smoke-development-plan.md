...
| D0-02 | 受限契约缺口、Owner 联合审批与 Contract Review Freeze | 按 ADR 结果迁移现有 canonical projection；只允许关闭 R01 failed-after/outcome-aware 与 child exact-set 等已知审批缺口；为 R00～R08 分别生成 machine-readable freeze baseline 与 CI guard | 本地候选门禁已覆盖 Corpus 子文件 SHA/count、设计/Validator/build 源摘要、CFE 摘要、正式状态逐步迁移、旧 Freeze/Approval/CFE append-only、两阶段 CFE diff 和签字 commit ancestry；Git `100644 blob` 检查继续失败关闭。R01/R04 都只保留 partial candidate，R00～R08 全部 `expansion_frozen`，当前没有 Gate Review Frozen/Approved。D0-04B批次二已拆分R01/R04独立package；批次三A～三C形成x/text zip-backed、真实`go list`与direct-material候选；Batch 3D test-only又以同一verified statement/snapshot串联direct material、14项repository blob leaf、15项真实x/text module-cache leaf（4项`0644` download evidence + 11项`0444` materialized tree）与已验ELF BuildInfo，逐叶回绑拒绝stale bundle；module结论仅限leaf identity/internal semantics。以上仍不证明Git base-tree membership/commit ancestry、artifact source derivation、build-closure或SBOM，也没有真实builder、双run pair、signature或trust-root。现有checker仍把workflow“存在”误作安装边界且缺少versioned release/handoff，这是独立Blocker；受信toolchain/action、GitHub enforcement、真实PR/Ruleset、Owner identity/review映射未建立前不得创建正式 Freeze。R09 release guard 仍待后续批次 |
...
| D0-04B | Validator transitive build-input closure | 把 R01/R04 verifier 迁入独立 entrypoint package，拆除共享 `contract_test` 包的传递缠绕；冻结 import/embed/外部模块/toolchain/workflow/action 构建输入 exact-set | 进行中、Formal Freeze blocker。批次一`19236995`建立仅允许`candidate_unactivated`的optional Schema；批次二把R04迁到stdlib-only `w2r04approvalconsumption`，把R01迁到六源码`w2r01`并只保留受管`x/text/unicode/norm`；批次三A完成zip/.mod raw、canonical h1和静态source选择候选；批次三B`a0252cff`建立单run statement、双run claim与input snapshot；批次三C形成7项direct material单次有界读取、真实Go 1.26.3双fresh-cache raw canonicalizer、BuildInfo文本解析与partial semantic admission候选。Batch 3D仍是test-only，但组合admission已落地并通过审计：它以同一verified statement/snapshot串联direct material、14项repository leaf、15项真实x/text module-cache leaf与已验ELF BuildInfo，并在assemble边界逐叶回绑snapshot metadata/bytes以拒绝stale或跨snapshot bundle；14项repository leaf绑定size/SHA-256/Git blob SHA-1，15项module leaf维持12/2/1 purpose exact-set及4项`0644`、11项`0444`，ELF projection与BuildInfo raw/statement三方一致。129对象早停、Deps闭包、HFiles、exact-case及snapshot `.info`→raw Time交叉绑定继续失败关闭。这些结论只关闭repository blob identity、module leaf identity/internal semantics与binary/raw/statement BuildInfo语义，不证明Git base-tree membership/commit ancestry或artifact source derivation。build-closure、SBOM、真实离线builder、双run pair、独立signature envelope、versioned base-owned trust-root Action/workflow release与handoff仍是blocker；两Gate blocker均保留，全部闭合前不得称为Formal Freeze |
...
| W2-R01 | [`graph-tool-result-receipt-contract-v1.md`](../design/agent/graph-tool-result-receipt-contract-v1.md)、[`runner-session-lane-review-v1.md`](../design/agent/runner-session-lane-review-v1.md)、[`w2-r01-owner-decision-matrix-v1.md`](../design/agent/w2-r01-owner-decision-matrix-v1.md) 与 [`aigc-contract-catalog.md`](../design/cross-module/aigc-contract-catalog.md) | `expansion_frozen / partial_candidate`；8个合法Result、40个Result拒绝、26个Receipt迁移和13个跨状态evidence共87条可靠子集。Validator已迁入六源码独立`w2r01` package，只保留受管`x/text/unicode/norm`；zip-backed resolver校验archive raw SHA/Go h1、根go.mod与静态source选择。compile-attestation已从纯statement推进到7项direct refs逐字节SHA/size复核、真实Go 1.26.3双fresh-cache raw→projection、BuildInfo raw文本解析和三项语义绑定的`partial_test_only_admission`。Batch 3D test-only组合admission已用同一verified statement/snapshot串联direct、14项repository leaf、15项真实x/text module leaf与已验ELF BuildInfo，并逐叶回绑拒绝stale bundle；repository leaf重算size/SHA-256/Git blob SHA-1，module leaf维持12/2/1 purpose exact-set、4项download evidence `0644`与11项materialized tree `0444`，ELF projection则与BuildInfo raw/statement三方一致。这只证明repository blob identity、module leaf identity/internal semantics与binary BuildInfo语义：Git base-tree membership/commit ancestry、artifact source derivation/source closure、build-closure、SBOM与generator语义仍未证明，Go archive摘要的golden测试上下文仍是非权威占位；真实builder、双run独立pair、签名/issuer/trust-root与base-owned action/workflow均不存在。failed-after/outcome-aware、Billing authority、exact-version、card owner、Registry/ordinal scope及Owner exact-set仍未关闭；未创建生产DTO/Repository/Migration | 下一步先关闭Git base-tree membership/commit ancestry与artifact source derivation，再完成build-closure/SBOM语义和真实离线builder；之后建立每run独立签名、双clean-build trust-domain pair策略、versioned trust-root与base-owned workflow/action。并行按决策矩阵关闭六项业务语义与受信Owner authority；不得以当前Batch 3D、partial admission或87条Corpus生成Formal Freeze |
