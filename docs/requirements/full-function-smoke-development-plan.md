...
| W2-R02/R03 | [`runner-session-lane-review-v1.md`](../design/agent/runner-session-lane-review-v1.md)、[`session-lane-runtime-contract-v1.md`](../design/agent/session-lane-runtime-contract-v1.md) 与 [`aigc-contract-catalog.md`](../design/cross-module/aigc-contract-catalog.md) | R02 已形成 Agent-owned Executable Draft：60 条纯状态 Corpus 向量固定严格 HOL、Turn/Run 创建决策、Fence ownership epoch、Heartbeat/精确到期、Takeover/Quarantine Effect 防降级、目标 Input/Run 与 Cancel Version first-write-wins/Claim 前取消、PG Scan trigger 与 Drain Handoff 候选；带 Run 摘要固定 effect state，并拒绝跨 Input/Turn Run 引用；Evidence 尚未绑定权威记录，不代表 lost-wake/Drain/SMK 已通过；仍无生产 Runtime，R03 仍为 Draft | 关闭 command receipt/enqueue exact-set、权威 Evidence、PG Repository/Migration/race/crash matrix、Eino Runner/Checkpoint wrapper、Cancel/Redis/Drain 对外契约与跨角色复选项；SMK-018 Worker 主体留在 W3 |
