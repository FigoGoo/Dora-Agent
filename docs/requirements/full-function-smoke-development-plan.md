...
| 前端 | `frontend/` 可以构建且 163 项测试通过；真实 Login/Logout、Auth v1 Provider、QuickCreate/Bootstrap、正式 `/projects/:project_id/workspace`、严格 Snapshot DTO、六态 Input、Snapshot→SSE、Cursor/Reset/Probe、同 Session/跨路由旧连接代次与 Abort 隔离均已接线；真实 Chromium 已验证创建、首 Prompt、`ready/live`、硬刷新同 Session、404 和退出 | W0.5 Workspace Transport 页面已可真实冒烟；下一批补真实 TCP/服务重启、Retention Reset 与跨 owner 浏览器故障注入，并逐域移除其他页面 Mock |
| 服务端 | 三个独立 Module 已建立基础 Runtime；Business 已有 Auth、Project、Outbox/Dispatcher、Agent BFF、owner/ready Binding JOIN 和短期三 Header 身份断言；Agent 已有 Session RPC、身份断言验证/Redis 防重放、DRAE Keyring 解密、REPEATABLE READ Snapshot 与 PostgreSQL poll-first SSE | W0.5 的 RPC、Snapshot、SSE 补读/Ready/Reset 已真实黑盒通过；后续只以前向契约扩展业务资源，不把当前恢复底座误报为 A2UI/Graph Tool 已实现 |
...
| 本地基础设施 | PostgreSQL、Redis、etcd Compose、三独立数据库、独立 `_test` 契约库、Schema Migration、`foundation-smoke`、`w0-smoke` 与 `w05-browser-smoke` 已可执行；Snapshot 固定三查询、EventBatch、owner 隔离、Redis 100 并发 Nonce 和 Migration 重复执行已有真实依赖测试 | 继续以真实基础设施运行门禁；后续波次不得退回进程内或 Mock 权威状态 |
...
| P1-01 | 三 Module 和本地基础设施骨架 | 独立构建、Migration、Compose、CI、健康检查 | M1.1、W0 Auth/Project/RPC/Dispatcher 与 W0.5 Agent BFF/Snapshot/SSE 已完成并真实黑盒通过 |
| P1-02 | 前端接入底座 | API Client、认证会话、错误映射、SSE 游标重连、开发代理和 CI | 正式鉴权、QuickCreate、Workspace Snapshot/SSE、硬刷新、404 与退出已接线并通过真实 Chromium；受控恢复矩阵继续补齐 |
...
| W1 Skill 与 Tool 入口 | `SMK-005`～`SMK-008` | Skill 草稿/发布快照、Session Snapshot、六 Tool 目录与显式选择 | 新旧 Session 快照隔离；无权限 Tool 从前后端共同失败关闭 |
...
| 阶段 | 主要任务 | 关键前置 | 主要验收 |
| --- | --- | --- | --- |
| W1-A | Business Skill 草稿/发布、治理可用性、Tool 引用校验 | Skill 产品字段矩阵、Tool Definition v1 | 草稿与发布快照独立，发布原子替换且失败保留旧快照 |
| W1-B | Agent Session Skill Snapshot 与 Tool Registry | W1-A 契约、六 Tool 审核结论 | 新旧 Session 按创建时快照隔离，目录顺序和不可用原因稳定 |
| W1-C | 前端 Skill 市场/创建/我的 Skill/工具箱接入 | W1-A/W1-B HTTP | 页面不展示版本明细，越权修改请求仍被后端拒绝 |
...
| W2-C | Runner、Session Lane、Turn/Receipt/A2UI 接入 | W0 Session/Input、四 Graph Tool | HOL 串行、冻结输出重放、未知 A2UI 失败关闭 |
