...
| D0-04B | Validator transitive build-input closure | 把 R01/R04 verifier 迁入独立 entrypoint package，拆除共享 `contract_test` 包的传递缠绕；冻结 import/embed/外部模块/toolchain/workflow/action 构建输入 exact-set | 进行中、formal Freeze blocker。批次一`19236995`建立仅允许`candidate_unactivated`的optional Schema；批次二把R04迁到stdlib-only `w2r04approvalconsumption`，把R01迁到六源码`w2r01`并只保留受管`x/text/unicode/norm`；批次三A完成zip/.mod raw、canonical h1和静态source选择候选；批次三B`a0252cff`建立单run statement、双run claim与input snapshot。批次三C现已形成7项direct material单次有界读取、真实Go 1.26.3双fresh-cache raw canonicalizer、BuildInfo文本解析与partial semantic admission候选；129对象早停、Deps闭包、HFiles、exact-case及snapshot `.info`→raw Time交叉绑定均失败关闭。15项module-cache union已分成12项`go_command_input`、2项`acquisition_evidence`和1项`materialization_evidence`，不得统称最小执行输入。下一批必须逐字节解析14+15叶子、从已验ELF用`debug/buildinfo`派生、校验build-closure/SBOM语义并接入真实离线builder；随后才是独立签名envelope、双trust-domain pair、versioned base-owned Action/workflow release与handoff。两Gate blocker均保留，全部闭合前不得formal Freeze |
...
| W2-R01 | [`graph-tool-result-receipt-contract-v1.md`](../design/agent/graph-tool-result-receipt-contract-v1.md)、[`runner-session-lane-review-v1.md`](../design/agent/runner-session-lane-review-v1.md)、[`w2-r01-owner-decision-matrix-v1.md`](../design/agent/w2-r01-owner-decision-matrix-v1.md) 与 [`aigc-contract-catalog.md`](../design/cross-module/aigc-contract-catalog.md) | `expansion_frozen / partial_candidate`；8个合法Result、40个Result拒绝、26个Receipt迁移和13个跨状态evidence共87条可靠子集。Validator已迁入六源码独立`w2r01` package，只保留受管`x/text/unicode/norm`；zip-backed resolver校验archive raw SHA/Go h1、根go.mod与静态source选择。compile-attestation已从纯statement推进到7项direct refs逐字节SHA/size复核、真实Go 1.26.3双fresh-cache raw→projection、BuildInfo raw文本解析和三项语义绑定的`partial_test_only_admission`。input snapshot锁定14项repository与15项module-cache provenance union，其中12项是Go命令输入、2项是acquisition旁证、1项是materialization旁证；目前仍只验证其metadata，未解引用14+15叶子。artifact、build-closure、SBOM与generator仍仅hash/size，BuildInfo未从已验binary派生，Go archive摘要的golden测试上下文仍是非权威占位；真实builder、双run独立性、签名/issuer/trust-root与base-owned action/workflow均不存在。failed-after/outcome-aware、Billing authority、exact-version、card owner、Registry/ordinal scope及Owner exact-set仍未关闭；未创建生产DTO/Repository/Migration | 下一步先实现snapshot两阶段leaf resolver与Git tree/blob/symlink验证，再完成ELF/`debug/buildinfo`、build-closure/SBOM语义和真实离线builder；之后建立每run独立签名、双clean-build trust-domain策略、versioned trust-root与base-owned workflow/action。并行按决策矩阵关闭六项业务语义与受信Owner authority；不得以当前partial admission或87条Corpus生成Formal Freeze |
...
D0-04B Batch 3B 已提交 `w2_validator_compile_attestation_run_statement.v1` 的纯Schema/Validator、双run pair evaluator、typed input-snapshot metadata contract与对抗矩阵：每份statement只声明一个clean builder run，并绑定repository/base commit+tree、manifest与build-closure投影、validator源码、`go.mod/go.sum`、`x/text v0.34.0` archive/module/source claim、Go 1.26.3 linux/amd64隔离环境、124项stdlib名称exact-set及闭合import图。typed content ref只允许schema/kind/media type/SHA-256/有界大小，不允许URI或路径；statement拒绝authority、issuer、signature或activation语义。input snapshot从manifest派生14项repository metadata，并冻结15项module-cache provenance union、五条命令、sandbox policy和typed SBOM generator ref。删除矩阵证明15项不能统称“最小只读cache”：实际Go命令读取12项（download `.mod`、`.ziphash`和10个selected source）；`.info`与`.zip`分别属于raw/acquisition旁证，materialized root `go.mod`属于解包一致性旁证。三类purpose按12/2/1 exact-set锁定，snapshot raw仍只重算metadata JSON及自身ref，尚未读取14+15叶子字节。

D0-04B Batch 3C 当前形成四个相互分层的test-only候选：无URI material descriptor从strict statement派生7项direct refs并对每个digest单次`size+1`有界读取、重算SHA/size和缓存immutable bytes；真实Go 1.26.3 `go list -deps -test -compiled -json`以两个fresh GOCACHE产生129个对象，raw parser在第130个对象前早停，递归拒绝大小写alias/unknown/null，重算regular Deps闭包并把Go/C/汇编/header/embed选择纳入固定projection；BuildInfo文本parser绑定固定探针、zero deps/replace/vcs和完整settings；partial admission只把snapshot JSON、go-list projection与BuildInfo raw文本列为语义已绑定，并额外要求snapshot锁定`.info`时raw Module.Time一致。它仍把artifact、build-closure、SBOM raw和SBOM generator列为hash/size-only，并显式保留14+15叶子、binary→`debug/buildinfo`、toolchain/image、真实builder、双run trust-domain、签名/issuer和trust-root open gaps。golden中的Go archive摘要只是非空测试占位，不是实际archive provenance。下一批先完成两阶段leaf resolver、Git tree/blob/symlink、ELF/BuildInfo、build-closure/SBOM语义和真实离线builder，再进入独立envelope、pair policy与versioned trust-root release；当前任何通过都不得解释成Formal Freeze。
