...
| D0-02 | 受限契约缺口、Owner 联合审批与 Contract Review Freeze | 按 ADR 结果迁移现有 canonical projection；只允许关闭 R01 failed-after/outcome-aware 与 child exact-set 等已知审批缺口；为 R00～R08 分别生成 machine-readable freeze baseline 与 CI guard | 本地候选门禁已覆盖 Corpus 子文件 SHA/count、设计/Validator 源摘要、CFE 摘要、正式状态逐步迁移、旧 Freeze/Approval/CFE append-only、两阶段 CFE diff 和签字 commit ancestry；verifier 独立信任根与 Git `100644 blob` 检查用于失败关闭。R01/R04 都只保留 partial candidate，R00～R08 全部 `expansion_frozen`，当前没有 Gate Review Frozen/Approved。GitHub enforcement 仍须先合入默认分支 `main`、完成真实 PR 验证与 Ruleset 配置；Owner role 到受信 GitHub identity/review 的映射仍未建立，当前 Validator 也只是文件级绑定，同包未登记 `TestMain`/`init` 及 go.mod/go.sum/toolchain/replace/第三方依赖的完整 build closure 仍待独立 stdlib-only 包或 package/build exact-set 关闭。此前不得创建正式 Freeze。R09 release guard 仍待后续批次 |
| D0-03 | R01 候选一致性、验证器 Source Closure 与 Owner Decision Matrix | 修正非法测试 Tool Key、replay 夹带字段短路、设计源摘要缺失；绑定 target test/validator 源码摘要；允许 pre-formal Owner exact-set 按决议受控调整；把 exact-version、authority_outcome、card owner、Registry 范围、Owner exact-set 与 GitHub identity/review authority 收敛为可逐项签字的决策矩阵 | 当前批次已把非法 `storyboard_generation` 收敛为 `plan_creation_spec`，补齐 replay piggyback 两条拒绝向量，将 R01 可靠候选子集从 85 条修正为 87 条，并建立 [`W2-R01 Owner 决策矩阵 v1`](../design/agent/w2-r01-owner-decision-matrix-v1.md) 与文件级 source closure；它仍是 partial candidate。下一步先关闭完整 Go package/build closure 和受信 GitHub Owner authority，不自行补写身份、角色或产品结论；完成前所有 Gate 必须保持 pre-formal |
| D1-00 | W2-S0-G0 Smoke 治理前置 | 独立 W2-ADR-009、Context/Registry 契约、SMK-004A shadow baseline 与七方 approval manifest；未批准时机器拒绝 `smoke/**` Harness | D0-03 后可并行；全程不修改既有 4457 行 Shell/Playwright、不创建 `smoke/**`，先关闭当前纯 Smoke 文件可绕过治理 CI 的缺口 |
...
近期编号与长计划唯一映射为：`D0-00` 是 W2-D0 的 evidence-only 前置而非实现包；`D0-01/02 → W2-D0`、`D1-01 → W2-S0`、`D2-01 → W2-A1`、`D2-02 → W2-A2`、`D3-01A → W2-B0a`、`D3-01B → W2-B0b`、`D4-01 → W2-B1`、`D5-01 → W2-C1`、`D6-01 → W2-C2`、`D7-01 → W2-D1/D2a/D2b`、`D8-01 → W3`、`D9-01 → W4～W6`。审批、Evidence 与提交信息必须同时写入这组映射，禁止创造第三套工作包编号。
...
远端默认分支当前是 `main`，但它尚不包含该 workflow，因此本分支上的本地测试不等于 GitHub enforcement 已激活。必须先关闭 D0-03 的 Owner identity/review authority 与完整 package source closure，再将最终信任根合入默认分支；随后用真实 PR 验证 workflow 的事件、head/check 关联与失败关闭语义，并用 Ruleset 固定 required workflow、required Owner review、strict up-to-date、merge-commit-only、禁止 direct push/bypass。未完成这些代码与外部设置前状态保持“CI 候选”，任何自报 role/URL 都不得解释为真实联合批准。`make check-w2-tool-release` 在 R09 Schema/首个 release manifest 获批后实现；届时必须拒绝 R09 引用 stale/reopened Freeze，以及 availability 已切换但 Release Manifest 未通过。
