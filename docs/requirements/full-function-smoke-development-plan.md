...
| 本地基础设施 | PostgreSQL、Redis、etcd Compose、三独立数据库、独立 `_test` 契约库、Schema Migration、`foundation-smoke`、`w0-smoke`、`w05-browser-smoke` 与 W1-C2 canonical 门禁已可执行；W0.5 Browser Evidence v2 逐值证明 Agent 真实 TERM/同二进制重启、原 Snapshot/Event/Ready 恢复、Chromium 受控断线与跨 Owner UI；`w1-smoke` 作为 `w1-browser-smoke` 的兼容别名，两者都强制执行 `@w1-real-review` 真实 Chromium 链路。W1 Evidence v3 覆盖持久化 Reviewer 授权、真实登录/API 批准、同 Cookie 撤权即时失效、提交 A 后继续编辑 B 仍只发布并冻结 A、100 并发非空 Skill QuickCreate v2、跨 Module Snapshot、密文清理和新旧 Session 隔离 | 继续以真实基础设施运行门禁；W1 Feature Flag 在普通配置中默认关闭，只由确认同批 Agent capability 的 Smoke/部署显式开启 |
...
| 冒烟工程 | Fixture、协议 Adapter、35 条 Scenario、Evidence 与故障注入已形成独立草案；W0.5 已有本地 Seeder、真实 HTTP/RPC/Snapshot/SSE、Agent 重启恢复、真实 Chromium 受控断线/跨 Owner Driver 和失败安全的脱敏摘要 Evidence v2 | W0.5 Transport API/浏览器恢复子集可重复执行并已通过；真实 Retention Window 的浏览器 Reset、完整 A2UI/业务资源及 `SMK-001`～`SMK-004` 整体仍待实现 |
...
- **M1.4 W0 身份与工作台纵切（W0.5 API/浏览器恢复主路径已通过）**：真实黑盒已经覆盖 Auth、100 并发同键创建、重放/冲突、generated Kitex、Prompt 清理、Business BFF owner 门禁、双用户跨 Owner Project/Workspace/Events、Agent Snapshot 解密、SSE 补读/Ready/Reset、Agent 真实 TERM/同二进制重启后的 Snapshot/Event 恢复、空 Prompt 非 null 空数组、页面首 Prompt、硬刷新同 Session、Chromium 受控离线恢复、第二用户跨 Owner UI 404 与退出撤销；组件矩阵覆盖 Reset 完整回源和旧连接隔离。`SMK-004A` 的主体实现已具备，但真实 Retention Window 的浏览器 Reset 尚未进入 canonical Evidence，因此仍不宣称全绿；A2UI/Storyboard/Asset/Run 的 `SMK-004B` 也未关闭。
...
截至 2026-07-14，`SMK-001`～`SMK-003` 的 W0.5 Transport API/浏览器子集已经真实黑盒通过，但完整场景仍保持 `待实现`：`SMK-001` 还缺管理员角色/数据范围越权闭环；`SMK-002`/`SMK-003` 已覆盖 Workspace Snapshot/SSE 水合，但真正 Runner/费用禁止副作用和完整业务 UI 尚未进入本批。`SMK-004A` 已覆盖 Session/Event Snapshot、补读、API Reset、前端 Probe/硬刷新、Agent 真实重启恢复、Chromium 受控断线恢复和第二用户跨 Owner UI；仍缺真实 Retention Window 触发后的浏览器 Snapshot Reset。完整 `SMK-004` 还缺 Chat/A2UI/故事板/资产投影，因此不得整体标记为可执行或通过。
...
`W0-DEC-001`～`W0-DEC-011` 已确认，W0 契约 v1 与 W0.5 Workspace Transport 契约 v1 已冻结。W0-08～W0-19 已完成；W0-20/W0-21 的 W0 Transport 与 W0.5 Workspace API/浏览器恢复主路径均已真实运行并通过。双用户跨 Owner API/UI、Agent 重启、Chromium 断线恢复、Reset/重连组件矩阵和旧连接隔离已经补齐；真实 Retention Window 的浏览器 Reset 仍需故障注入，因此不得把 `SMK-001`～`SMK-004` 整体标记为通过。
...
| Frontend Transport 页面 | 已实现并通过组件/Transport/Playwright 测试 | 真实 Login/Logout、Auth Provider、QuickCreate 稳定意图键、失败重试、正式 Project Workspace 路由、Workspace Snapshot→SSE 水合、Input 六态、刷新保持 Session、Reset 完整回源、同 Session/跨路由旧连接隔离、受控离线恢复和第二用户跨 Owner UI 404 | 补真实 Retention Window 触发后的浏览器 Reset 与更完整故障矩阵 |
...
| W0 Workspace/SSE Smoke | API/浏览器恢复主路径已通过 | 非空与空 Snapshot、未知 Session/双用户 Project/Workspace/Events Owner-safe 404、Agent 直连拒绝、SSE Event/Ready/Reset、Agent 重启恢复、Prompt 与 Input 状态、硬刷新、Chromium 受控断线恢复、跨 Owner UI、退出后工作台 401 | 补真实 Retention Window 浏览器完整回源及完整业务 Driver |

W0.5 三条文件互斥实现线已经汇合并通过交叉评审：Agent Workspace Snapshot/SSE、Business Agent BFF/身份断言、Frontend Snapshot→SSE 水合。真实 Agent 重启、浏览器断线恢复和跨 Owner UI 已进入 canonical 门禁；下一批只补 Retention Window 浏览器 Reset，再进入 `SMK-001`～`SMK-004` 的完整业务断言，不重复实现 Auth、QuickCreate、Ensure RPC、Dispatcher 或基础水合。
...
12. 浏览器工作台显示首 Prompt、唯一 Session 与 Input 状态，硬刷新仍回到同一 Session；Chromium 离线时保留已验证 Snapshot 并进入 `reconnecting`，恢复后回到同一 Session `live`。
13. 首用户退出后，第二个真实用户通过 UI 登录并访问首用户 Workspace；Project Bootstrap 返回 404，页面不显示 Project/Session/Prompt，且不继续请求 Agent Workspace/Events/Tools。
14. 退出为 `204`；旧会话访问会话 API 与 Workspace 均返回 `401`，浏览器只能看到未登录状态。

脱敏汇总证据位于 `.local/smoke/w05-workspace-transport-evidence.json`。非浏览器 `w05-smoke` 保持兼容的 `w05.workspace-transport.smoke.evidence.v1`；canonical `w05-browser-smoke` 当前发布 `w05.workspace-transport.smoke.evidence.v2`，包含 25 项 exact-set assertion，其中 `concurrent_requests=100`，其余 24 项必须逐值为 `true`，并额外记录独立浏览器 Workspace ID。每次成功运行的 Business/Agent/Agent-Restart/Playwright 日志位于 `.local/smoke/w0-transport/runs/<run_id>/`。脚本以 `umask 077` 创建 `0700/0600` Evidence，先有界停止 Runtime 并确认 etcd 摘除，再扫描不会继续追加的闭合 Evidence 集合；失败运行会删除尚未完成脱敏闭环的本地 run bundle。只有结构化 Browser Result、全部断言和泄漏扫描均通过才原子发布汇总；skipped 浏览器用例、缺失结果或任一 false 都不能发布 passed。`.local` 是本地临时证据目录，不作为提交资产；进入 CI 后仍需输出脱敏、可保留的正式 Evidence。

本次通过证明 W0 Transport 与 W0.5 Workspace Snapshot/SSE 的 API、双用户权限、Agent 真实重启恢复、Chromium 断线恢复、跨 Owner UI 和硬刷新主路径，并以组件矩阵证明 Reset 回源与旧连接隔离。它尚不覆盖真实 Retention Window 的浏览器 Reset、完整管理员 RBAC、A2UI/业务资源、Worker、Graph Tool、模型执行、扣费或支付，因此不改变 W1～W6 和完整 `SMK-P0` 的待实现状态。
