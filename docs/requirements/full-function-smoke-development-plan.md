...
| W2-R01 | [`graph-tool-result-receipt-contract-v1.md`](../design/agent/graph-tool-result-receipt-contract-v1.md)、[`runner-session-lane-review-v1.md`](../design/agent/runner-session-lane-review-v1.md)、[`w2-r01-owner-decision-matrix-v1.md`](../design/agent/w2-r01-owner-decision-matrix-v1.md) 与 [`aigc-contract-catalog.md`](../design/cross-module/aigc-contract-catalog.md) | `expansion_frozen / partial_candidate`；8个合法Result、40个Result拒绝、26个Receipt迁移和13个跨状态evidence共87条可靠子集。Validator已迁入六源码独立`w2r01` package，只保留受管`x/text/unicode/norm`；zip-backed resolver校验archive raw SHA/Go h1、根go.mod与静态source选择。Batch 3D test-only组合admission已串联direct、14项repository leaf、15项真实x/text module leaf与已验ELF BuildInfo。Batch 3E以共享body-only raw Git CAS重算commit/tree，证明BaseCommit identity、commit→BaseTree与14项`100644` leaf membership，assurance固定为`raw_object_recomputed_test_only`。Batch 3F-A已删除兼容view并让两个语义验证器直接消费immutable body，补齐tree的body/entry/depth/active-stack/DAG预算；三项raw closed claim不变。Batch 3F-B又建立`w2_github_reported_commit_projection_pair.v1`，严格绑定Git Database Get a commit端点、REST version、Accept/identity encoding、entity size/digest、reported commit/tree和ordered parents，仅关闭`github_reported_commit_projection_stable_double_read_verified`这一test-only投影一致性。它尚未证明真实HTTP双读、repository/observation authority、complete tree projection或可信expected source，也未与raw admission组合；artifact source derivation/source closure、build-closure、SBOM与generator语义、真实builder、双run独立pair、签名/issuer/trust-root与base-owned action/workflow均不存在。failed-after/outcome-aware、Billing authority、exact-version、card owner、Registry/ordinal scope及Owner exact-set仍未关闭；未创建生产DTO/Repository/Migration | 下一步不再横向扩大test-only投影；先显式冻结`transfer_decoded_identity_entity_body_sha256_v1`及adapter attestation缺口，再以独立composite只交叉绑定commit/tree/ordered parents语义，不合并两类assurance。主开发线转向artifact source derivation、build-closure/SBOM和真实离线builder，随后建立每run独立签名、双clean-build trust-domain pair、versioned trust-root与base-owned workflow/action。并行按决策矩阵关闭业务语义与受信Owner authority；不得以当前Batch 3F、partial admission或87条Corpus生成Formal Freeze |
