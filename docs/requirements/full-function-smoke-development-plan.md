...
| 前端 | `frontend/` 可以构建且 302 项测试通过；真实 Login/Logout、Auth v1 Provider、QuickCreate/Bootstrap、Owner Skill 创建/编辑/审核提交、Reviewer 待审队列/冻结详情/当前发布对比/批准发布、最多 16 个 Published+Active Skill 的 QuickCreate v2 选择、正式 `/projects/:project_id/workspace`、严格 Snapshot DTO、Snapshot→SSE 与旧连接隔离均已接线；真实 Chromium 已验证 Creator 提交、Reviewer 批准、Creator 选择已发布 Skill 并创建 Workspace 的单一浏览器全链 | W0.5 与 W1 当前页面子集已可真实冒烟；Skill 市场、完整治理页面、全量 ADM-RBAC 和后续业务域仍按波次补齐 |
...
| W1-B2 | 六 Tool Definition Catalog 与可用性 DTO 评审 | W1-A0、六 Tool 独立设计 | exact-set、顺序和不可用原因稳定；本阶段不创建 Executable Registry |
| W1-C1 | 前端 Skill 创建/我的 Skill 与目录不可用投影 | W1-A1 HTTP、W1-B2 DTO | 页面不展示版本明细，越权修改失败；未就绪 Tool 不被 Mock 成可执行 |
