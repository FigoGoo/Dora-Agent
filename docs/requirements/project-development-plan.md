...
> 更新日期：2026-07-16
>
> 当前实现事实基线（不含本计划文档提交）：`codex/w2-r04-consumption-audit@d4df2e99`
...
| [W2 ADR ballot 准备清单](../design/agent/approvals/w2-adr-ballot-preparation-requests/CPR-W2-ADR-v1.json) | 八项 ADR 的 P4 conflict、65 个跨 Gate 联合裁决引用、唯一 `ADR-005 → ADR-011` 硬前置、未满足证据和失败关闭来源摘要 | canonical ballot/Owner request 已创建、`R02-D19` 自动代表 ADR 接受、最终 Owner roles 已推导或实现已解锁 |
...
| W2 Contract / Review Freeze corpus | `评审中` | 已完成 P4 跨 Module 审计与八项 ADR dependency map；严格 ADR CPR 已固定 8 项 readiness、65 个 joint disposition refs、唯一 `ADR-005 → ADR-011` 硬前置和 7 份来源摘要，状态仅为 `prepared_unregistered / canonical_ballot_status=not_created / implementation_unlocked=false`。R00 已形成 14 项稳定决策，并以严格 CPR 固定六项 incomplete candidate 的缺失输入/证据，但没有提交实际 candidate、Owner option 或 R00 请求。R01 已形成覆盖六项语义与六项治理决定的严格请求，只绑定当前 1 fixture/87 vectors/4 tests partial candidate；R02/R03/R04 分别保留 19/14/20 项严格待决请求。R02 legacy-upgrade v2 已从未修改的 v1 资产机械固定 3 fixtures/107 vectors/14 tests，但仅为 `prepared_unregistered`。ADR/R00 CPR、R01、R02 upgrade v2、R03/R04 使用独立交叉守卫的 stdlib-only validator/guard；R00/R03 的 Gate `candidate_evidence` 为空，R04 仍只绑定 4 fixture/111 vectors/11 tests 的 `partial_candidate / candidate_unactivated`；R00～R04 全部为 `expansion_frozen` | R00 六项 exact candidate、ADR-001/002/003/004/005/008/010/011 与 R00～R04 语义结论、R02 upgrade v2 评审/登记、R03 child/R04 full-gate exact-set、aggregate manifest、最终 Owner exact-set、正式 authority/build closure 与 Formal Freeze |
...
- W2 ADR 准备态契约：`d4df2e99 test(agent): 固定W2 ADR准备态契约`；CPR 原始摘要为 `a65f5b3575b56a1a237c56eca64da15881e526ae8addb8de26105feb8cd84dbd`，固定八项 readiness、65 个 joint disposition refs、唯一 `ADR-005 → ADR-011` hard edge 与 7 份来源摘要。两套独立 stdlib-only validator/guard 拒绝 duplicate/null Gate、第二 ballot/authority、角色/选项/批准、live P4 Gate 和实现解锁；八项 canonical ballot 仍全部 `not_created`。
