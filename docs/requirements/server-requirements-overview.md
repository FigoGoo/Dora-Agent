...
> 更新日期：2026-07-13
>
> 关联文档：[共通业务规则与验收基线](common-requirements-baseline.md)、[用户端需求总览](user-requirements-overview.md)、[管理端需求总览](admin-requirements-overview.md)
...
1. 统一 Project、Skill、Skill 调用、Agent、故事板、资产、积分、发布者收益、精选作品和公告等概念。
...
→ 使用 Graph Tool 和公开 Tool 完成故事板、资产和媒体创作
→ 发布者获得可追溯收益
...
- 同一 Project 能够同时启用多个 Skill，但只对实际执行的 Skill 调用计费。
- Skill 通过结构化字段和受控 `@Tool` 引用执行，不允许模型或用户绕过权限、版本和计费边界。
...
| 个人用户 | 创建 Project、使用 Agent、启用 Skill、管理故事板和资产、消费积分、点赞和提交工单。 |
...
- 结构化 Skill 定义、版本、审核、市场和 `@Tool` 引用。
- Chat/A2UI、动态故事板、资产和异步媒体生产。
- 积分、订单、发布者收益积分、结算、回收和打款。
- 精选作品公开快照、点赞和创作过程查看。
- 平台公告全局弹窗。
- 内容安全、风险控制、审计和生产可观测性。
...
└─ Publisher Profile（可选）
   ├─ Skill → Skill Version → Tool Reference
...
| Enabled Skill Binding | Project/Session 当前允许 Agent 选择的确定 Skill 版本绑定。 |
...
| Graph Tool/Public Tool Run | Skill 调用内部执行的高层 Graph Tool 或公开 Tool 运行记录；Graph Tool 终态汇总其内部可计费执行，不额外产生 Tool 完成费。 |
...
- Skill 发布版本冻结 Tool、权限、计费配置引用和安全策略快照；具体执行价格在可计费执行开始时按已发布配置版本冻结，运行时不得静默替换为未审核 Tool 或未发布价格。
...
### 7.6 Skill 生命周期、测试、审核与市场

- 支持保存草稿、自动保存、复制、测试、预览、提交审核、驳回、发布、版本更新、暂停、恢复、下架和版本回滚。
- 已发布 Skill 不得原地修改；更新必须生成新版本并重新校验和审核。
...
- 用户端“已发布 Skill/未发布 Skill”是细粒度状态的分组投影，不能丢失真实生命周期状态。存在有效线上版本时归入“已发布”；没有有效线上版本时归入“未发布”；旧版本在线且新版本审核中时仍属于“已发布”。

建议状态：

```text
draft → testing → submitted → reviewing → published/rejected
published → suspended → published/offline
published/offline → deprecated
```
...
- Enabled Skill Binding 记录确定 Skill 版本、启用人、启用时间、停用时间和来源。
- 启用、停用、展示、预加载或读取调用规则都不扣除积分。
- Skill 被暂停、下架、Tool 被禁用或用户权限失效后，新调用必须 fail closed；不能静默切换到其他版本或 Tool。

#### 7.7.2 Runtime 选择

- Agent 每轮从服务端读取当前已启用且可执行的 Skill 快照。
...
- Skill Invocation 记录 Project、Session、Run、Turn、Skill、版本、调用序号、选择原因、计费配置与收益规则引用、Tool、权限和安全快照；单项执行价格和账务结果记录在对应可计费执行及账本明细中。
...
- 每个新 Turn 冻结消息边界、Skill、Prompt、Tool Registry、预算和计费配置引用；技术重试和 Resume 不得混入后续消息或重新选择版本。
...
初期 Agent-facing Graph Tool 白名单为：

```text
analyze_materials      素材/资产分析
plan_creation_spec     创作规格规划
plan_storyboard        动态故事板规划
generate_media         图片、视频、音频等媒体生成派发
assemble_output        剪辑、装配与结果导出
```
...
- Worker 负责 Provider Submit/Poll/Cancel、错误分类、有限重试与 Full Jitter、对象存储、可信用量回执、资产登记、绑定和 Batch Barrier；Worker Pool、Claim Batch 和下游并发必须有硬上限。
...
- Job 只有在 Provider、存储、资产、绑定及扣费事实核对完成后才成功；Graph Tool/Batch 终态只汇总已有扣费，不二次收费。
...
- Batch 全部 Job 和必要核对进入业务终态后，只发布一次可信终态及去重费用汇总。
...
- 支付订单和支付回调需要验签、幂等和金额校验；本期不提供充值退款。
...
- 发布时生成不可变 Featured Work Snapshot，包含作品、封面、标题、标签、作者公开资料、媒体信息、实际使用的 Skill 及精确版本、公开创作过程和版权声明。
...
- 选定的故事板版本及关键镜头。
- 被选公开资产的演进和最终结果。
- 实际使用的 Skill、版本和高层 Graph Tool 步骤。
...
- 对象存储、Asset Finalization、绑定和用量回执；涉及 Business 资产或积分事实时通过受控契约写入，不直连 Business 普通业务表。
- Batch Barrier、失败补偿和可靠终态事件。
...
本需求只确定业务职责，不提前固定具体表、Migration Owner、RPC 方法、Event Schema 或 Job Payload。开始对应功能实现前，详细设计必须补齐跨 Module 所有权、唯一写入方、读取方、事务/最终一致性、幂等键、版本兼容和回滚矩阵。
...
- Project 创建、首消息、Skill 启用、Skill Invocation、可计费执行扣费、Graph Tool 费用汇总、账务冲正、收益入账/回收/冲正、点赞、Storyboard Command、Job/Batch、Asset Binding 和公告回执分别具有明确幂等边界。
- 相同幂等标识但业务语义不同必须返回冲突，不能复用第一次结果覆盖新语义。
- SSE 只从持久化 EventLog 分配单调序号；队列、进程内 Channel 和通知不能充当事件真源。
- 读模型允许最终一致，但余额、扣费/冲正账本、Graph Tool 费用汇总、发布者收益积分、Skill/Tool 版本和权限快照必须可强一致验证与审计。
- 技术重试不能产生重复 Project、首消息、Provider 副作用、Asset、点赞、扣费、收益或公告展示回执。
...
| Skill Version | `draft → testing → submitted → reviewing → published/rejected`；`published ↔ suspended → offline → deprecated` |
...
- Skill 选择分布、实际调用数、逐项扣费、Graph Tool 汇总、重复扣费拦截和版本使用分布。
...
| SRV-SKILL-002 | Given Skill 和 `@Tool` 已发布；When 改名、改价、暂停或并发更新 | 已运行 Turn 使用冻结版本；新运行失败关闭或使用新发布版本，历史事实不被改写。 |
...
| SRV-WRK-001 | Given 同一 Job 被重复唤醒且 Worker 竞争；When 执行并提交终态 | 只有匹配 Lease Owner/Version 的提交生效，Provider、Asset、绑定和扣费最多一次。 |
...
| 维度 | 服务端验收要求 |
| --- | --- |
| 可用性 | 核心 API 月可用性 `≥99.9%`，扣费、收益账本和任务受理接口 `≥99.95%`。 |
...
| 幂等 | 相同业务幂等键并发/重放 100 次，仅产生一个业务事实、一次正常扣费和一份收益归属。 |
...
| 财务 | 每日对账覆盖 100% 扣费、冲正、收益和回收记录；不平在 5 分钟内告警并阻止重复结算。 |
...
| 兼容性 | 已发布 DTO/Event/Job Payload/Skill/Tool/Prompt 字段不得复用或静默改义；滚动发布期间新旧版本均可处理存量权威状态。 |
...
| [AIGC Generation Worker 详细设计](../aigc-worker-design.md) | Operation/Batch/Job、异步 Provider、Finalization、Batch Barrier、Outbox、幂等、重试、取消和费用回执。 |
...
- Skill 市场、创建、测试、审核、版本和 Dashboard。
...
现有三份设计文档中的 Reservation、Refund、Compensation 或生成后扣费描述如果与[共通计费真值表](common-requirements-baseline.md#32-计费真值表)冲突，均视为待设计阶段修订的旧假设，不得作为生产需求或当前实现验收依据。
