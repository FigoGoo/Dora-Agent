...
> 更新日期：2026-07-14
>
> 关联文档：[Graph Tool 功能需求总览](graph-tool-requirements-overview.md)、[支付与积分充值需求总览](payment-requirements-overview.md)、[用户端需求总览](user-requirements-overview.md)、[管理端需求总览](admin-requirements-overview.md)、[服务端需求总览](server-requirements-overview.md)

## 1. 文档目的与适用顺序

本文统一 Dora Graph Tool、支付、用户端、管理端和服务端共同依赖的业务术语、计费与收益规则、状态机投影、非功能基线和可执行验收格式。各需求总览不得复制出与本文不同的同名规则。

当文档存在冲突时按以下顺序处理：

1. 已由用户确认并写入本文的共通业务规则。
2. Graph Tool、支付、用户端、管理端和服务端总览中的具体场景规则。
...
| 术语 | 统一定义 |
| --- | --- |
| Skill Invocation | Agent 在一个 Turn 中实际选择并开始执行某个 Session Skill Snapshot 的业务调用，是调用归因、运行追踪和发布者收益归属的最小单位，但不是固定的“一次调用统一扣费”单位。 |
| Graph Tool | 暴露给唯一主 Agent、同时可在用户工具箱发现和显式选择的高层场景能力；v1 至少包含流程规划、素材分析、故事板设计、媒体生成、提示词写法和视频剪辑，旧文档中的“高层 Capability”统一按 Graph Tool 理解。 |
...
| Payment Order | 用户购买消费积分时冻结商品、金额、积分、渠道和用户的支付事实；只有验签通知或受信主动查单可以确认成功。 |
| Points Fulfillment | 支付确认后按冻结商品快照向用户积分账户唯一入账的履约事实，与支付订单状态分离。 |
| 账务冲正 | 仅用于纠正重复扣费、错扣、无对应可计费执行等平台账务差错的反向账本记录，不属于产品退款能力。 |
| 退款 | 用户因失败、取消、不满意、部分成功或申诉而要求返还已正常扣除积分的产品能力；Dora 本期不支持退款。 |

### 2.1 Skill 草稿与发布真值

1. Skill 产品生命周期只展示 `draft` 和 `published`，不提供版本明细、版本列表、版本差异、版本筛选、版本切换或版本回滚。
2. 每个 Skill 保留一个可编辑草稿和一个当前发布快照。发布原子替换当前发布快照；发布失败不影响旧发布内容。
3. 新 Session 创建时冻结当时的 published snapshot；发布、再次编辑或再次发布只影响后续新 Session，已有 Session 的所有 Turn 均保持原快照。
4. 后端允许保留内部 `publication_revision`、内容 digest、发布时间和操作人用于并发控制、审计、幂等和历史事实，但不得向产品包装成“版本明细”。
5. 草稿可以与当前发布快照并存。用户端可展示“已发布”和“有未发布修改”，但不能形成第三种生命周期状态。
6. Skill 被暂停/下架属于治理可用性，不是 Skill 内容版本或用户编辑状态；详细治理状态在管理端独立记录，用户端仍只投影可编辑草稿与当前发布结果。
...
| Payment Order | `created → pending_payment/reconciling`；`pending_payment/reconciling → paid/closed/payment_failed/provider_exception`；仅强制异常允许 `paid → provider_exception`；支付状态与积分履约分离 |
| Points Fulfillment | `unfulfilled → fulfilling → fulfilled/fulfillment_failed`；已支付未履约必须补发积分，不要求重复付款 |
...
正常业务流程不存在 `reserved` 或 `refunded` 状态。执行状态与账务状态必须分离；运行失败不等于未扣费，已扣费也不等于运行成功。监管、司法或支付机构强制资金处置进入 `provider_exception`，通过独立异常流程处理，不形成用户自助退款能力。
...
2. Project 生命周期与最近运行摘要分离；Skill 草稿/发布与治理可用性分离；Skill Invocation 执行与账务状态分离。
...
| Skill 内容 | `draft / published`，允许草稿与当前发布快照并存 | 草稿 / 已发布；可附加“有未发布修改”标记 |
...
| Graph Tool Definition | `draft / testing / published / suspended / offline / deprecated` | 草稿 / 测试中 / 已发布 / 已暂停 / 已下架 / 已废弃 |
...
3. 注册登录、实名、收益回收、打款、账号注销、申诉和数据保留的详细 PRD，以及[支付与积分充值需求总览](payment-requirements-overview.md)要求的支付技术、商户、法务、风控和财务设计。
4. [Graph Tool 功能需求总览](graph-tool-requirements-overview.md)中六个 Agent-facing Graph Tool 的独立设计文档、Graph State、Node/Branch、HITL、幂等、预算和状态机。
...
| 维度 | v1 基线 |
| --- | --- |
| 可用性 | 用户端和管理端核心 API 月可用性不低于 `99.9%`；支付、积分履约、积分扣费、收益账本和可靠任务受理接口不低于 `99.95%`。 |
...
| 幂等 | 同一幂等键并发或重放 `100` 次，只允许产生一个对应业务事实；支付场景至多确认一次支付和一笔正常充值积分，消费场景至多产生一次正常扣费和一份发布者收益。 |
...
| 财务对账 | 每日自动对账覆盖 `100%` 渠道支付、积分履约、扣费、冲正、收益和回收记录；任何不平记录在发现后 `5min` 内告警并阻止相关重复履约或结算。 |
...
下表是可直接转为自动化或人工测试的场景定义。每次验收执行都必须附带对应 Evidence：用户端场景至少包含 UI/无障碍结果、API 响应和关键事件；管理端场景至少包含 API 响应、权限判定和审计记录；Graph Tool 场景至少包含目录/表单投影、Run/Receipt、业务结果、调用次数和费用汇总；支付场景至少包含冻结订单快照、验签通知或主动查单、渠道交易号、积分履约、账本和对账结果；计费与收益场景至少包含幂等键、不可变账本、Graph Tool 汇总和下游回执；Agent/Worker 场景至少包含权威状态、Lease/Fence、Receipt/EventLog 和外部调用次数；非功能场景必须附压测、监控或恢复演练报告。只有 UI 截图、只有接口成功码或只有数据库快照均不能单独作为通过证据。
...
| AGT-005 | Given Session 创建后 Skill 再次发布；When 原 Session 和新 Session 分别执行 | 原 Session 使用冻结快照，新 Session 使用当前发布快照；用户端不展示版本明细。 |
| AGT-006 | Given A2UI 收到未知组件/Action 或含危险 Markdown；When 前端实时或回放渲染 | 不执行未知交互，Markdown 被清洗，整包失败关闭或渲染不可交互降级 Card。 |
| WRK-001 | Given 同一 Job 被重复唤醒且多个 Worker 竞争；When Claim 执行 | 只有有效 Lease/Fencing Owner 可以提交状态，Provider、TOS 和 Business Finalize 副作用最多一次；Worker 不扣费/退款。 |
| WRK-002 | Given Provider 或 Business Finalize 请求结果未知；When Worker 恢复 | Job 进入 `reconciling` 并按原幂等键/receipt 核对，不立即重跑 Provider、重复上传或重复绑定。 |
| WRK-003 | Given Redis 不可用；When Job 已到 `available_at` | PostgreSQL 轮询在 30 秒 P95 内恢复 Claim，Redis 恢复后不重复执行。 |
| WRK-005 | Given Worker terminal event 重复投递；When Agent Inbox 消费 | 只创建一个 BatchContinuationResult 并唤醒一次 Session Lane，不直接调用 Agent 或恢复旧 Graph。 |
