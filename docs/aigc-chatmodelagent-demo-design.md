...
> 文档定位：本文保留历史 `main` 单体 AIGC 实现的迁移快照和仍有效的目标契约。正文中的“当前实现”一律指本文日期对应的历史快照，不是当前 `business/`、`agent/`、`worker/` 工作树事实。
>
> 权威口径：当前阶段、已通过 Evidence 和下一步只以[功能优先开发与试跑计划](./requirements/full-function-smoke-development-plan.md)为准；各 Tool 获批范围以[Graph Tool 设计索引](./design/agent/graphtool/README.md)及独立设计为准。本文的 Phase、后续阶段和实施顺序仅保留目标依赖，不再维护独立排期。
>
> 维护规则：不得在本文增量维护当前 Runtime、Tool 数量、Worker 闭环或试跑状态；这些状态变化只更新上述唯一计划、Graph Tool 索引和对应独立设计。本文中的旧路径、五 Tool、真实 Image2 与历史 Demo 验收描述均不得作为当前工作树证据。

本文档记录历史 `main` 快照中的 ChatModelAgent、Runner、持久化 Session Runtime、Capability Tool、A2UI，以及仍需迁移核验的目标契约。Tool 暴露面、动态故事板和局部操作以 `aigc-tool-storyboard-design.md` 为参考；异步 Batch、Job、Worker 和 Finalization 以 `aigc-worker-design.md` 为参考。历史快照的产品定位是受信本地 Demo，没有面向公网的真实登录鉴权、租户授权或接口限流。
