...
| Spec/Storyboard/Candidate 审核 | 已实现 | Spec/Storyboard 保留 chat approval 卡；Candidate 每项仍是 durable Approval，但不逐项发 chat 卡。左侧 Storyboard 等相关 Job 全终态后一次提交冻结候选批次，服务端可逐项恢复 deterministic decision。 |
...
| Generation 回流 | 部分实现 | 正常媒体/装配 Batch 使用 `on_failure`；成功由 durable generation outbox/A2UI projection 展示，不调用模型解释。Barrier 把完整不可变 PostBatchPayload 写入 Operation/terminal outbox 和 durable BatchContinuationResult.Result；只有 partial/failed/cancelled 需要可信 system Turn。无独立 PostBatchContinuation/Stage Ledger/ToolOperationResult。 |
| 前端定向操作 | 已实现 | 槽位本地上传后直接绑定、音频原生试听、Operation 取消、带 dispatch snapshot 的局部重生成，以及 replay-first、只恢复 provider/transient 失败的 `retry_failed` 均不经过 Agent。 |
| A2UI/Event | 部分实现 | 严格 A2UI 1.0/known-action parser 与整包 fail-closed 不变；模型外层 normalizer 只清理单对象格式噪声，不放宽协议。generic required 表单、状态版本门禁、EventLog/Tail 已实现；无全域 Projector/Inbox。 |
...
| 单个 `job.status` | 否 | generation outbox publisher 更新 Job 卡 |
...
Candidate Asset Approval 同样是 durable continuation，但不再逐项发布 chat A2UI 卡。Job Finalization 只投影最新 Storyboard；左侧 Storyboard 等该板相关 Job 全终态后，一次调用 `POST /api/aigc/sessions/:id/storyboards/:sid/candidate-approvals/decision`。首次请求校验 expected Storyboard version，并按幂等键持久冻结当时所有 pending Candidate 的 `(approval_id, binding_id, expected_decision_version)`；每个子项再使用稳定 child key 调用现有 durable Decision。部分失败返回逐项结果和 HTTP 207；同键重试只续作冻结集合，不会混入点击后新增 Candidate。Spec/Storyboard 仍使用原 chat approval 卡和单项 Decision API。

Spec/Storyboard 的 chat Approval Card 是唯一审核入口。它由系统在领域提交后发布，`data.approval_id` 绑定冻结 Approval，Markdown 仅展示候选详情；当前 A2UI 没有独立 Button 组件，因此决策 UI 固定为必选的“确认/拒绝” `SingleChoice` 加卡片统一“提交”控件。前端仅对携带 `approval_id` 的卡片调用 Approval Decision API。用户在聊天框键入“确认”、提交普通无 `approval_id` 表单，或点击模型仿制的确认文案，都只产生 UserMessage，绝不能被解释成 Approval Decision。Capability 返回 `waiting_user` 后，模型不得输出“请回复确认/输入确认”等伪入口或仿制 Approval 表单；发布前的协议校验必须拒绝这类最终输出并进行有界纠错重试。
...
Generation outbox 的 Job 事件更新独立 Job 卡。Batch 达到 `completed/partial_failed/failed/cancelled` 后，Barrier 写唯一 terminal outbox，并同步完成 Operation 状态与费用汇总。

正常 `generate_media` 和 `assemble_output` 创建的 Batch 固定为 `WakePolicy=on_failure`。`completed` 不启动 Agent Turn：generation outbox 展示 Operation/Job 状态，`job.succeeded` 的 Finalization 投影只刷新 Storyboard，不发布逐项 Candidate chat Approval。左侧 Storyboard 在相关 Job 全终态后开放一次统一确认。只有 `partial_failed/failed/cancelled` 把 `NeedsAgentExplanation` 置为 true，由模型解释已经持久化的失败事实；成功路径不依赖模型复述 Tool Result。
...
2. Generation Outbox 的 `SessionSignalPublisher` 投影 Operation、Job、Batch、Billing；`job.succeeded` 后的 Finalization 投影只发布已提交的 Storyboard，不发布 Candidate chat Approval。
...
Generation 投影同时更新 Operation/Job 卡和 `tool_run:<tool_call_id|stage_run_id>` Public Stage 卡；Operation/Stage 顶层状态使用 Batch/Operation 版本门禁，每个 Job Node 使用自己的 `status_version`。只含 nodes 的 Worker patch 跳过顶层比较，但每个 node 仍拒绝旧 Job 版本。成功 Job 的 `result_asset_ids` 会刷新 available Asset，并在 ToolRun 中显示图片/视频/音频预览或 Assembly JSON manifest 链接。Public Stage 仍是 UI projection，不是持久化 Stage Ledger。
...
- 普通消息和信息收集卡。
- Operation/Job 进度卡。
- 成功 Job 的本地媒体预览或 Assembly manifest 链接。
- Operation 卡对非终态提供取消，对 `failed/partial_failed` 提供“重试失败项”；重试创建新的 recovery workflow。
- 整体 Spec/Storyboard Revision 审核；Candidate 审核不进入右侧 chat。
...
当前每个 Job 有独立 `job:<job_id>` 卡，并同时按 `tool_run:<tool_call_id|stage_run_id>` 聚合到 Public Stage UI projection；独立持久化 Stage Ledger 仍是后续目标。
...
当前 generation `SessionSignalPublisher` 承担局部 projector 职责：除 Operation/Job/Batch 外，`job.succeeded` 还投影 Finalization Storyboard，但不投影 Candidate chat Approval。Capability 创建的 Spec/Storyboard 卡和对应 Approval Decision 仍在事务后直发；Candidate 批量决定发布 Storyboard 与批次摘要。目标 Projector 是确定性适配器，不调用 LLM，并以 Inbox 与业务提交解耦。
...
6. Job 只在 Asset、费用和 Candidate/Active Binding 完成后进入 succeeded；当前 generation Job/outbox 与 Asset/Storyboard/Approval 使用幂等的多事务收尾，并非一个跨聚合数据库事务。领域 Commit 不发布 UI，随后 `job.succeeded` outbox 只投影 Storyboard；Candidate durable Approval 保留在服务端供 Storyboard 批量入口消费。投影失败不会回滚 Job 或触发补偿。
...
11. Job 状态只更新 UI；一个 Batch terminal 只创建一个带完整 PostBatchPayload 的稳定 `BatchContinuationResult`，并且最多创建一条解释用 SessionInput。
...
19. 当前 Spec/Storyboard Approval 只产生普通 durable `a2ui.action` 卡，不产生 interrupt 卡；generic Runner interrupt 则使用 durable `/messages/resume`。
...
24. 槽位本地上传后按精确版本直接绑定，动态 Active/Candidate 音频和显式 AudioPreview 可试听。
...
27. Worker 领域 Commit 后不直接发布 UI；`job.succeeded` outbox 可重试投影 Storyboard，不发布 Candidate chat Approval。
...
41. 正常媒体和装配 Batch 使用 `on_failure`；成功由 generation outbox 的 durable A2UI projection 展示，不依赖模型解释。
