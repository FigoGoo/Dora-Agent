...
遗留（后续子切片，不阻塞）：
- **created_by/updated_by operator 回填继续逐域推进**：账户/Skill/积分/资产/通知等表按领域操作者字段和权限语义单独映射。
...
- `created_by/updated_by operator 回填` 已启动项目/作品域、Tool 策略域与模型配置域；GORM 未映射公共列不能用简单 `SetColumn` callback 粗暴补齐，剩余表继续逐域设计。
