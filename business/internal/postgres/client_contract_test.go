...
// TestMigratedSchemaContract 使用真实 PostgreSQL 验证 Business Migration 的 Schema 契约。
func TestMigratedSchemaContract(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("连接 Business 契约测试数据库失败: %v", err)
	}
	t.Cleanup(func() {
		if err := client.Close(); err != nil {
			t.Errorf("关闭 Business 契约测试数据库失败: %v", err)
		}
	})
	if err := client.VerifySchema(context.Background(), 5*time.Second); err != nil {
		t.Fatalf("Business Migration Schema 契约无效: %v", err)
	}
}
