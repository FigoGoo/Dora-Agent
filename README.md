...
| 审批与续作 | 已实现 | Spec、Storyboard 保留 chat approval 卡；Candidate Asset 仍逐项创建 durable Approval，但不逐项发 chat 卡，由左侧 Storyboard 在相关 Job 全终态后一次冻结并提交候选批次。冻结命令、版本栅栏、Continuation 与 Command Ledger 继续作为逐项业务真相 |
...
| A2UI/SSE | 部分实现 | 严格 A2UI 1.0/known-action parser 与整包 fail-closed 不变；外层 normalizer 不放宽协议，只清理单对象格式噪声；required 表单校验、状态 `status_version` 门禁、SessionEventLog/cursor 回放和 Tail Relay 已实现 |
| Stage/Event Projector | 待实现 | 没有独立持久化 Stage Ledger，也没有覆盖所有领域的 Outbox→Inbox→Projector；当前卡片按 ToolCall、Operation、Job、Storyboard、Approval 分别更新 |
...
保存候选状态
  ...
  │   → 用户逐张提交带 decision_version/idempotency_key 的决定
...
正常 `generate_media` 与 `assemble_output` Batch 固定使用 `wake_policy=on_failure`：成功结果不再唤醒模型解释。Generation outbox 持久投影 Operation/Job 状态，Job Finalization 只刷新 Storyboard；Candidate Approval 不进入 chat，统一确认入口由左侧 Storyboard 在相关 Job 全终态后开放。只有 partial/failed/cancelled 才创建需要 Agent 解释的 Turn。
...
Generation UI 会更新 `operation:<operation_id>`、`job:<job_id>`，并把同一 Operation/Job 投影到 `tool_run:<tool_call_id|stage_run_id>` Public Stage 卡。Operation 和 Stage 顶层状态使用 Batch/Operation 版本门禁，每个 Job Node 使用自己的 `status_version`；只更新 nodes 的 Worker patch 不参与顶层版本比较。这里的 Public Stage 仍只是 A2UI 投影，不是独立持久化 Stage Aggregate/Ledger。
...
- Generation 状态先写 Generation Outbox，再投影为 operation/job A2UI Action、Worker Finalization 后的 Storyboard 和 BatchContinuationResult；Candidate Approval 不投影到 chat。
...
浏览器只执行 `/events/stream` 中的 `a2ui.ready`、`a2ui.action`、`a2ui.interrupt_request`、`a2ui.interrupt_resolved` 和 `a2ui.error`；错误事件使用命名空间，避免与 EventSource 的连接级 `error` 事件冲突。正常五 Tool 审批使用 `a2ui.action` approval card。A2UI 支持 Image/Video/Audio Preview，其中 Audio Preview 使用原生可控播放器。成功 Job 的 `result_asset_ids` 会触发 available Asset 刷新，并在 ToolRun 卡展示本地图片、视频、音频预览或可打开的 Assembly manifest 链接。用户侧 REST/SSE 使用公开视图：Storyboard 隐藏 Command ledger/fingerprint，Generation 隐藏 Provider Payload/Result、BindingToken、lease、内部 request/task ID、账务交易和原始错误正文，Approval Decision 响应隐藏冻结命令/Continuation/Outbox，Asset 只暴露当前 Session 中 `availability=available` 的记录。
...
2. 确认 `generate_media` 返回 `accepted` 后，Operation/Job 状态能够经 Worker 和 SSE/A2UI 持续更新并到达终态。
