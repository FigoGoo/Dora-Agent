...
| 范围 | 当前事实 | 下一门槛 |
| --- | --- | --- |
| 前端 | `frontend/` 已接入真实登录/退出、QuickCreate、Project Workspace、Snapshot→SSE、Owner Skill Builder、Reviewer 管理入口、最多 16 项 Published+Active Skill 选择，以及匿名 Skill 市场列表/详情；W0.5/W1 Chromium 主路径通过 | 冻结 Public Market Binding v1 后再开放跨发布者使用；治理页面与后续业务继续按纵切推进 |
| Business Service | `business/` 已具备真实 Auth/Session、Project QuickCreate/Bootstrap、Skill 草稿/审核/不可变发布、Reviewer/Governor 动态权限、治理状态机、匿名 Market 白名单读取、加密 Outbox/Dispatcher、Agent Workspace BFF 与内部身份签名 | 设计并评审 Public Market Binding；完整管理员角色/数据范围 RBAC 仍留在 W5 |
...
| 本地基础设施 | `go.work`、三 Schema Migration、PostgreSQL/Redis/etcd Compose、跨服务 Probe、Schema Catalog 契约测试，以及 W0/W0.5/W1 真实 API/浏览器 Smoke 已建立；W1 另有四角色治理与公开 Market 独立 Evidence | 把脱敏 Evidence 与故障注入矩阵纳入持续 CI |
...
成功运行会同时发布权限为 `0600` 的 `.local/smoke/w1-skill-foundation-evidence.json`、`.local/smoke/w1-skill-governance-evidence.json` 与 `.local/smoke/w1-skill-market-evidence.json`。三者分别固定为 canonical v3 的 47 项 assertions/42 项布尔门禁、治理 v1 的五项闭集布尔门禁，以及 Market v1 的六项闭集布尔门禁；sidecar 不扩容 canonical 合同。该命令已经覆盖 Creator/Reviewer Chromium 主链、Governor 真实 Login 与暂停/恢复/offline，以及游客 Market 列表/详情、21 条 keyset、发布/草稿隔离和跨 Owner 使用零部分写入；不表示治理页面、跨发布者使用或完整 ADM-RBAC 已实现。
