...
def main() -> int:
    ...
    cmd = [
        ...
        "-run",
        "TestM6(IndependentToolCharge|SkillTestConsumesReviewCandidateRPC)",
    ]
    completed = subprocess.run(cmd, cwd=ROOT, env=env, text=True)
    if completed.returncode != 0:
        return completed.returncode
    print("M6 service e2e checks ok: Agent -> BusinessGateway mock covers model list, review skill spec, tool estimate/freeze/charge/release")
    return 0
