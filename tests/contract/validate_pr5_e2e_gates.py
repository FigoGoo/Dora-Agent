#!/usr/bin/env python3
"""Validate PR-5 E2E fixture and release gate artifacts without external packages."""
...
def validate_fake_provider() -> None:
    ...
    print("pr5 fake provider artifacts ok")
...
def validate_suite_indexes() -> None:
    ...
    for path in required_suites:
        ...
        for item in suite.get("fixtures", []):
            ...
            if item.get("required") is not True:
                fail(f"{path}: all PR-5 fixtures must be required")
    ...
    print("pr5 e2e suite indexes ok")
...
def validate_e2e_fixtures() -> None:
    for relative_path, expected in REQUIRED_E2E_FIXTURES.items():
        ...
        depends_on = set(data.get("depends_on_pr", []))
        missing_pr = expected["depends_on_pr"] - depends_on
        if missing_pr:
            fail(f"{path}: missing dependencies {sorted(missing_pr)}")
