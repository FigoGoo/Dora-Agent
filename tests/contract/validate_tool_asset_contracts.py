#!/usr/bin/env python3
"""Validate tool asset Tool/Credit/Asset contract artifacts without external packages."""
...
def validate_json_artifacts() -> None:
    ...
    for path in paths:
        if not path.exists():
            fail(f"missing {path}")
        load_json(path)
    print(f"tool asset json ok {len(paths)} files")
...
def validate_thrift() -> None:
    ...
    print("tool asset thrift surface ok")
...
def validate_toolplan_fixture() -> None:
    ...
    print("tool asset toolplan fixture ok")
...
def validate_asset_and_tool_fixtures() -> None:
    ...
    print("tool asset asset/tool fixtures ok")
...
def validate_migrations() -> None:
    ...
    for path in (MIGRATION_DIR / "business").glob("*.down.sql"):
        if "drop table if exists" not in path.read_text(encoding="utf-8").lower():
            fail(f"{path}: missing drop table")
    print("tool asset migration static guard ok")
...
def main() -> None:
    ...
    validate_migrations()
    print("tool asset contract validation ok")
