...
本次继续补齐 M1 语义门禁：Agent 安全证据结果只接受 `passed/blocked/failed`，拒绝旧 `allow`；业务幂等不再依赖客户端 `request_hash` 或 raw body hash helper，而由业务应用层生成冲突指纹后交给幂等 guard；幂等 Begin 覆盖并发同 key 场景，唯一冲突收敛为 `processing/replay/conflict` 语义。
...
| M1 语义对齐 | passed | `scripts/validate-engineering-baseline.sh` 校验业务幂等 tenant 唯一键、业务层冲突指纹、并发同 key 测试、审计模型字段、Agent 状态机和安全证据枚举。 |
