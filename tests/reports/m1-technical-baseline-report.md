...
| 检查项 | 状态 | 说明 |
| --- | --- | --- |
| M0 基线 | passed | `scripts/validate-m1.sh` 已先执行 `scripts/validate-m0.sh`。 |
...
| M1 语义对齐 | passed | `scripts/validate-m1.sh` 校验业务幂等 tenant 唯一键、canonical request hash、并发同 key 测试、审计模型字段、Agent 状态机和安全证据枚举。 |
...
- 后续修改 migration、配置键、repository 或契约时必须重新执行 `scripts/validate-m1.sh`。
