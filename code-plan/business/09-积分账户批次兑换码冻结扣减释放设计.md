...
| `CreditEstimateDTO` | `estimate_id`、`estimate_kind`、`estimate_points`、`available_points`、`expires_soon_points`、`insufficient`、`line_items[]`、`expires_at` |
...
请求字段：`project_id`、`resource_type`、`model_id`、`pricing_snapshot_id`、`quantity`、`duration_seconds`、`tool_usage_items[]` 非必填、`auth_context`、`safety_evidence`。响应：`estimate_id`、`estimate_points`、`available_points`、`expires_soon_points`、`insufficient`、`account_type`、`line_items[]`。
...
用于纯 Tool 运行、不产生生成资产的计费预估。请求字段：`project_id`、`tool_usage_items[]`、`auth_context`、`request_meta`。响应同 `CreditEstimateDTO`。
...
| --- | --- | --- | --- | --- |
| 安全通过后预估 | `EstimateGenerationCredits` | `resource_type`、`model_id`、`pricing_snapshot_id`、数量/秒数、`safety_evidence` | `estimate_points`、`available_points`、`estimate_id` | 积分不足不进入确认 |
| 非生成 Tool 预估 | `EstimateToolCredits`；生成计划内 Tool 通过 `EstimateGenerationCredits.tool_usage_items[]` | `tool_name`、`tool_type`、`billing_unit`、`quantity`、`metadata_summary` | `line_items[]`、`estimate_points` | Agent 不计算单价，只展示业务返回 |
