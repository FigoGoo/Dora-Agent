...
| 模型 | 可选模型、默认模型、价格快照 | `/api/models/generation`、`/api/admin/models/**` | 停用默认模型保护 | model/default/price |
...
| Skill | runtime spec DTO、审核错误码、Memory 默认值 | `/api/skills/**`、`/api/admin/skills/**` | 发布、审核、回滚、通知、Memory 默认开启 | skill/version/review |
...
| 资产 | upload intent、batch access、元素类型同步审计 | `/api/assets/**`、`/api/asset-element-types` | TOS 确认、权限、预览下载、内置元素类型发布/变更审计 | asset/object/elements/change_records |
| 资产扣费 | commit DTO | 无直接 HTTP，验证禁止前端直连 | 保存成功扣费、失败释放 | asset + freeze + ledger |
| 作品 | public snapshot DTO、可复制分享链接 | `/api/public/**`、`/api/works/**`、`/api/admin/works/public/**` | 分享、取消、点赞、下架、公开列表详情返回 share_url | work/snapshot/like |
...
| 模型选择 | `ListAvailableGenerationModels`、`ResolveDefaultModel` | 只返回展示名和价格快照 |
| 安全前置 | 业务写 RPC 接收 `SafetyEvidenceDTO` | 缺失/过期/摘要不匹配拒绝 |
| 积分闭环 | `EstimateGenerationCredits`、`EstimateToolCredits`、`FreezeCredits`、`ChargeToolUsageCredits`、`ReleaseFrozenCredits` | 安全通过后预估，确认后冻结，独立 Tool 成功扣费，失败释放 |
| 资产保存扣费 | `CommitGeneratedAssetAndCharge` | 保存成功才扣费，按 `estimate_item_id` 防重复扣费，部分完成部分扣费 |
| 资产引用 | `BatchCheckAssetAccess` | 批量校验，跨空间拒绝 |

## 验收 Done Gate

- [ ] 所有文档中的业务表都有 migration 计划。
- [ ] 每张业务表都有字段名、类型、必填、默认值、索引/约束和说明。
- [ ] 所有写 RPC 有 `idempotency_key`。
- [ ] 所有写 HTTP API 通过 `Idempotency-Key` 进入 `RequestMeta.idempotency_key`。
- [ ] 所有列表 RPC 默认 `page_size=10`，最大 50。
- [ ] 所有列表 HTTP API 默认 `page_size=10`，最大 50。
- [ ] 所有业务写入有权限校验和审计或明确不审计原因。
- [ ] 用户端、管理端、公开端接口都落在对应业务领域文档，不存在脱离领域状态机的单独对接设计。
- [ ] 所有 Agent 依赖都有【Agent开发】参数说明。
- [ ] 空库初始化能创建且仅创建一个初始平台管理员，并强制首次密码轮换；轮换成功后 bootstrap 状态为 `rotated`。
- [ ] Skill `memory_policy` 未传时默认 `enabled=true`，Published runtime spec 返回同一策略。
- [ ] 兑换码绑定用户、企业、渠道的成功和失败路径都有 contract fixture。
- [ ] 公开作品列表、详情和分享结果都返回可复制 `share_url`，且不泄露私有 object key。
- [ ] 内置资产元素类型发布或变更写 `asset_element_type_change_records` 和后台审计日志。
- [ ] Tool 积分表 `tool_pricing_policies`、`credit_estimate_items`、`credit_tool_charge_batches`、`credit_tool_charge_items` 均有 migration、repository test 和 contract fixture。
- [ ] contract test 覆盖正常、业务错误、权限错误、幂等冲突、超时。
- [ ] 集成测试覆盖关键事务回滚。
- [ ] migration 不包含数据库级外键。
- [ ] 日志脱敏测试通过。
- [ ] Agent DB 与业务 DB 边界测试通过：业务库不出现 Agent session/run/message/event/tool_call/blackboard/memory 表。
- [ ] 进入生产前必须通过 [15-生产级闭眼开发门禁与Agent对齐验收设计](./15-生产级闭眼开发门禁与Agent对齐验收设计.md)。
