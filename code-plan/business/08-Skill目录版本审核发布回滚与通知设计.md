...
| Memory 默认开启 | 未传策略时业务写入默认开启 | `skill_versions.memory_policy` | Agent 按 runtime spec 使用 Memory |
...
| `memory_policy` | jsonb | 是 | `{"enabled":true}` |  | Agent memory 策略摘要；未显式关闭时默认开启 |
...
| `status` | varchar(32) | 是 | `running` | idx | `running`、`passed`、`failed` |
...
| `CreateSkillDraftRequest` | `scope_type`、`display_name`、`description`、`intent_descriptions[]`、`input_schema`、`output_schema`、`memory_policy` 可选、`tool_bindings[]`、`output_element_schemas[]`、`test_cases[]` |
| `UpdateSkillDraftRequest` | `display_name`、`description`、`intent_descriptions[]`、`input_schema`、`output_schema`、`memory_policy` 可选、`tool_bindings[]`、`output_element_schemas[]`、`test_cases[]`、`base_version_id` |
...
| `PublishedSkillSpecDTO` | `skill_id`、`version_id`、`display_name`、`intent_descriptions[]`、`behavior_instruction_ref`、`input_schema`、`output_schema`、`output_element_schema[]`、`tool_bindings[]`、`memory_policy` |
| `SkillTestResultDTO` | `test_run_id`、`status` passed/failed/running、`actual_elements[]`、`error_summary`、`agent_trace_id` |
...
| `memory_policy` | object | Memory 策略 |
...
请求字段：`auth_context`、`request_meta`、`skill_id`、`version_id`、`test_run_id`、`test_case_id` 可选、`status=passed|failed`、`actual_elements_json`、`error_code` 可选、`error_summary` 可选、`safety_evidence_json` 可选、`agent_trace_id`。
...
- Skill Memory 默认开启；创建或更新未传 `memory_policy` 时写入 `{"enabled":true}`，`GetPublishedSkillSpec` 必须原样返回发布版本的 `memory_policy`。
...
| 事务 | 原子写入 | 回滚条件 |
| --- | --- | --- |
| 创建/编辑 Skill | `skills`、`skill_versions`、bindings、output schemas、test cases、审计、幂等记录 | Tool 不可绑定、元素类型非法、版本冲突 |
...
| 执行 Skill | `GetPublishedSkillSpec` | `skill_id`、`space_id`、`actor_user_id` | runtime spec、Tool 绑定、输出元素结构 | 按 spec 组装执行，不自造 Tool/元素 |
...
- 未传 `memory_policy` 创建 Skill 时，发布后的 `PublishedSkillSpecDTO.memory_policy.enabled=true`。
