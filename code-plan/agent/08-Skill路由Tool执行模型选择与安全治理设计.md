...
路由结果写入 `agent_runs.skill_selection`，字段包含 `skill_id`、`skill_version`、`matched_reason`、`fallback_reason`、`tool_refs_digest`。无 Skill 命中时进入 `free_chat` 或直接 Tool 意图，但仍必须经过安全评估和权限策略。
...
| 场景 | 验证 |
| --- | --- |
| Skill 命中 | `agent_runs.skill_selection` 保存命中信息，AG-UI 输出 `agent.skill.selected`。 |
...
| Skill runtime spec | `skill_id`、`version`、`auth_context` | 返回意图、步骤、Tool refs、输出元素结构、Memory 设置。 |
| Tool 执行策略 | `tool_name`、`tool_type`、`auth_context`、`risk_context` | 返回是否可用、风险等级、是否确认、超时、重试和取消策略。 |
| 模型选择与计价快照 | `resource_type`、`model_id`、`auth_context` | 返回用户可见模型名、默认模型、`pricing_snapshot_id`、是否停用。 |
