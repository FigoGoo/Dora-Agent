...
| `AccountSpaceService.ResolveAuthContextFromToken` | Agent HTTP middleware 鉴权时 | `authorization`、`request_meta`、`expected_space_id` 可选 | `auth_context`、`space_context`、`session_id`、`expires_at` 可选 | 2s；可重试 1 次；读无幂等键；Agent 不信任 `X-Actor-User-Id` / `X-Space-Id` 身份头 | `UNAUTHENTICATED`、`PERMISSION_DENIED`、`CROSS_SPACE_DENIED` |
...
```mermaid
flowchart TD
    A["Agent HTTP Authorization"] --> B0["ResolveAuthContextFromToken"]
    B0 --> A1["Agent 创建 run"]
    A1 --> B["ResolveCurrentSpaceContext"]
...
type BusinessGateway interface {
    ResolveAuthContextFromToken(ctx context.Context, req ResolveAuthTokenRequest) (AuthContextDTO, SpaceContextDTO, error)
