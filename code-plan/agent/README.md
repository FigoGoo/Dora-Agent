...
相关契约：`api/thrift/**`、`api/openapi/**`、`api/agui/**`、`docs/architecture/**`、`docs/standards/**`

## 说明

本目录是 Eino 智能体微服务的生产级系统设计开发文档，用于和业务开发、前端开发、测试对齐范围、顺序和职责边界。当前文档体系已经包含架构、模型、函数出入参、RPC/AG-UI 边界、流程闭环、日志和测试设计。
...
- Agent 数据库建表不添加数据库级外键约束，不写 `FOREIGN KEY` 或 `REFERENCES`。
...
| `auth_context.login_identity_type` | 业务开发确认 | 是 | `personal`、`enterprise_member`、`admin`。 |
...
| `request_meta.source` | Agent 传入 | 是 | Agent 调用固定为 `agent`。 |
| `pagination.page_size` | Agent 传入，业务限制 | 列表必填 | 默认 10，最大值由业务契约声明。 |
| `safety_evidence` | Agent 生成，业务校验 | 写入公开/资产场景必填 | 只传脱敏证据摘要，不传原文和内部策略。 |

业务开发重点确认：

- `07` 中业务 RPC 能力、DTO、错误码、幂等和权限上下文是否符合业务语义。
- `09` 中积分、资产保存和扣费释放闭环是否与业务事务边界一致。
...
- `06` 中 AG-UI 事件类型、payload、sequence、补偿和快照是否满足前端 reducer。
