...
func TestPR2BoardGraphHTTPRoutes(t *testing.T) {
	db := testdb.StartPostgres(t, "dora_agent_pr2_http")
	...
	if len(initialEvents) != 2 || initialEvents[0].(map[string]any)["type"] != "graph.plan.created" || initialEvents[1].(map[string]any)["type"] != "board.snapshot.updated" {
		t.Fatalf("unexpected initial PR-2 event replay: %#v", initialReplay)
	}
	...
	patchedElement := pr2.CreativeElement{
		SchemaVersion: pr2.SchemaVersionCreativeElement,
		ElementID:     "elem_patch_note_001",
		BoardID:       creationResult.Board.BoardID,
		ElementType:   pr2.BoardElementTypeTextNote,
		Source:        pr2.BoardElementSourceUser,
		Status:        pr2.BoardElementStatusReady,
		Position:      pr2.ElementPosition{X: 0, Y: 760, Width: 640, Height: 160, Order: 3},
	...
	afterElements := append(append([]pr2.CreativeElement{}, creationResult.Elements...), patchedElement)
	...
	patch := pr2.BoardPatch{
		...
		BaseVersion:    1,
		TargetVersion:  2,
		Operation:      pr2.BoardPatchOperationAddElement,
	...
	if len(eventsAfterPatch) != 4 || eventsAfterPatch[2].(map[string]any)["type"] != "board.patch.applied" || eventsAfterPatch[3].(map[string]any)["type"] != "board.snapshot.updated" {
		t.Fatalf("unexpected PR-2 event replay after patch: %#v", replayAfterPatch)
	}
	...
	if len(busEventsAfterPatch) != 2 || busEventsAfterPatch[0].EventType != pr2.EventTypeBoardPatchApplied || busEventsAfterPatch[1].EventType != pr2.EventTypeBoardSnapshotUpdated {
	...
	if run.Status != pr1.RunStatusPlanning {
	...
	if len(eventsAfterApproval) != 6 || eventsAfterApproval[4].(map[string]any)["type"] != "board.patch.applied" || eventsAfterApproval[5].(map[string]any)["type"] != "board.snapshot.updated" {
		t.Fatalf("unexpected PR-2 event replay after approval: %#v", replayAfterApproval)
	}
	...
	if len(busEventsAfterApproval) != 2 || busEventsAfterApproval[0].EventType != pr2.EventTypeBoardPatchApplied || busEventsAfterApproval[1].EventType != pr2.EventTypeBoardSnapshotUpdated {
...
func testDigest(t *testing.T, value any) string {
	t.Helper()
	digest, err := pr1.CanonicalDigest(value)
	if err != nil {
