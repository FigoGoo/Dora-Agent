...
func TestPR5AgentHTTPRedisRuntimeGate(t *testing.T) {
	db := testdb.StartPostgres(t, "dora_pr5_agent_http_redis")
	...
	creationResult, err := runtime.ExecuteGenericCreation(t.Context(), creation.GenericCreationInput{
		RunID:                "run_pr5_http_redis_001",
		ProjectID:            "prj_pr5_http_redis_001",
		SessionID:            "sess_pr5_http_redis_001",
		SpaceID:              "sp_personal_1001",
		ActorUserID:          "usr_1001",
		TraceID:              "trace-pr5-http-redis",
	...
	extraEvent := pr5RedisOnlyEvent(t, creationResult, now.Add(time.Minute), 3)
	...
	if err := snapshotCache.Set(t.Context(), snapshotKey, []byte(`{"source":"pr5"}`), time.Minute); err != nil {
		t.Fatalf("set redis snapshot cache: %v", err)
	}
	if value, ok, err := snapshotCache.Get(t.Context(), snapshotKey); err != nil || !ok || string(value) != `{"source":"pr5"}` {
...
func pr5RedisOnlyEvent(t *testing.T, result creation.GenericCreationResult, createdAt time.Time, seq int64) pr1.AGUIEnvelope {
	t.Helper()
	payload := map[string]any{
		"board_id": result.Board.BoardID,
		"source":   "pr5_http_redis_gate",
		"seq":      seq,
	}
	...
	event, err := pr1.BuildAGUIEnvelope(pr1.AGUIInput{
		EventID:       "evt_pr5_http_redis_003",
...
func agentJSON(t *testing.T, router nethttp.Handler, method string, path string, body any) map[string]any {
	...
	req.Header.Set("X-Trace-Id", "trace-pr5-http-redis")
