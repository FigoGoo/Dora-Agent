...
func TestPR5FullHTTPServiceE2ESmoke(t *testing.T) {
	businessDB := testdb.StartPostgres(t, "dora_pr5_full_business_http")
	...
	agentDB := testdb.StartPostgres(t, "dora_pr5_full_agent_http")
	...
	businessBinary := buildPR5Binary(t, repoRoot, "dora-business-full-http", "./services/business/cmd/business")
	agentBinary := buildPR5Binary(t, repoRoot, "dora-agent-full-http", "./services/agent/cmd/agent")
	...
	business := startPR5Process(t, repoRoot, businessBinary, map[string]string{
	...
	logPR5ProcessOnFailure(t, "business", business)
	...
	agent := startPR5Process(t, repoRoot, agentBinary, map[string]string{
		"DORA_CONFIG_SOURCE":                    "env",
		"APP_ENV":                               "test",
		"LOG_LEVEL":                             "debug",
	...
	logPR5ProcessOnFailure(t, "agent", agent)
	...
	token := loginPR5User(t, "http://"+businessHTTPAddr)
	session := postPR5AgentJSON(t, "http://"+agentHTTPAddr, token, "/api/agent/sessions", "idem-pr5-full-session", map[string]any{
		"project_id":    "prj_active_1001",
		"initial_title": "PR-5 full HTTP smoke",
	})
	sessionID := stringField(t, session, "session_id")

	guideRun := postPR5AgentJSON(t, "http://"+agentHTTPAddr, token, "/api/agent/runs", "idem-pr5-full-guide-run", map[string]any{
		"session_id": sessionID,
		"project_id": "prj_active_1001",
		"run_intent": "entry_guide",
		"user_input": map[string]any{
			"client_message_id": "cm_pr5_guide",
			"content_type":      "text",
			"text":              "",
		},
	})
	...
	guideReplay := getPR5AgentJSON(t, "http://"+agentHTTPAddr, token, "/api/agent/runs/"+stringField(t, guideRun, "run_id")+"/events?after_sequence=0&limit=50")
	assertPR5EventTypes(t, guideReplay, "creative.guide.presented", "agent.run.completed")

	normalRun := postPR5AgentJSON(t, "http://"+agentHTTPAddr, token, "/api/agent/runs", "idem-pr5-full-normal-run", map[string]any{
		"session_id": sessionID,
		"project_id": "prj_active_1001",
		"run_intent": "normal",
		"user_input": map[string]any{
			"client_message_id": "cm_pr5_normal",
			"content_type":      "text",
	...
	normalReplay := getPR5AgentJSON(t, "http://"+agentHTTPAddr, token, "/api/agent/runs/"+stringField(t, normalRun, "run_id")+"/events?after_sequence=0&limit=100")
	assertPR5EventTypes(t, normalReplay, "creative.router.decided", "agent.message.completed")
}

type pr5Process struct {
	cmd     *exec.Cmd
	done    chan struct{}
	waitErr error
	output  bytes.Buffer
...
func buildPR5Binary(t *testing.T, repoRoot, name, packagePath string) string {
	t.Helper()
...
func startPR5Process(t *testing.T, repoRoot, binary string, env map[string]string) *pr5Process {
	t.Helper()
	ctx, cancel := context.WithCancel(t.Context())
	cmd := exec.CommandContext(ctx, binary)
	cmd.Dir = repoRoot
	cmd.Env = testprocess.EnvWith(env)
	process := &pr5Process{cmd: cmd, done: make(chan struct{})}
...
func logPR5ProcessOnFailure(t *testing.T, service string, process *pr5Process) {
	t.Helper()
	t.Cleanup(func() {
		if t.Failed() {
...
func loginPR5User(t *testing.T, businessBaseURL string) string {
	t.Helper()
	resp := postPR5JSON(t, businessBaseURL+"/api/auth/login", "", "idem-pr5-full-login", map[string]any{
		"login_type": "personal",
...
func postPR5AgentJSON(t *testing.T, agentBaseURL, token, path, idempotencyKey string, body any) map[string]any {
	t.Helper()
	return postPR5JSON(t, agentBaseURL+path, token, idempotencyKey, body)
}

func getPR5AgentJSON(t *testing.T, agentBaseURL, token, path string) map[string]any {
	t.Helper()
	...
	req.Header.Set("X-Trace-Id", "trace-pr5-full-http")
	req.Header.Set("X-Space-Id", "sp_personal_1001")
	return doPR5JSON(t, req)
}

func postPR5JSON(t *testing.T, url, token, idempotencyKey string, body any) map[string]any {
	t.Helper()
...
func doPR5JSON(t *testing.T, req *http.Request) map[string]any {
	t.Helper()
...
func assertPR5EventTypes(t *testing.T, replay map[string]any, required ...string) {
	t.Helper()
