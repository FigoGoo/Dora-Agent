...
func TestBusinessMarketplaceRepositoryWithActiveMigration(t *testing.T) {
	db := testdb.StartPostgres(t, "dora_business_skill_market")
	...
	for _, table := range []string{
		...
		if !testdb.TableExists(t, db.DB, table) {
			t.Fatalf("skill market active business migration table %s missing", table)
		}
	}
	...
	readMarketplaceBusinessFixture(t, "tests/fixtures/contracts/marketplace/creator_submit_approve_publish.json", &publishFixture)
	...
	readMarketplaceBusinessFixture(t, "tests/fixtures/contracts/marketplace/user_install_latest_personal.json", &personalInstallFixture)
	...
	readMarketplaceBusinessFixture(t, "tests/fixtures/contracts/marketplace/enterprise_install_pinned_upgrade.json", &enterpriseFixture)
	...
	if err := skillmarket.ValidateEnterprisePinnedUpgrade(initialEnterprise, enterpriseFixture.UpgradeRequest, upgradedEnterprise, enterpriseFixture.HistoricalRunRule); err != nil {
	...
	readMarketplaceBusinessFixture(t, "tests/fixtures/contracts/billing/skill_usage_precreate_confirm_charge.json", &chargeFixture)
	...
	readMarketplaceBusinessFixture(t, "tests/fixtures/contracts/billing/skill_usage_refund_reversal.json", &refundFixture)
	...
	if err := skillmarket.ValidateSkillUsageRefundReversal(usageBeforeRefund, usageAfterRefund, settlementAfterReverse); err != nil {
	...
	if err := db.DB.Model(&businesscore.SkillInstallationRecord{}).Count(&installationCount).Error; err != nil {
	...
	if err := db.DB.Model(&businesscore.SkillUsageRecord{}).Count(&usageCount).Error; err != nil {
...
func readMarketplaceBusinessFixture(t *testing.T, relativePath string, target any) {
	t.Helper()
