...
func TestPR3BusinessCreditAssetRepositoryWithActiveMigration(t *testing.T) {
	db := testdb.StartPostgres(t, "dora_business_pr3")
	...
	for _, table := range []string{"credit_holds", "credit_ledger_entries", "tool_pricing_snapshots", "generated_assets", "asset_commit_records"} {
		if !testdb.TableExists(t, db.DB, table) {
			t.Fatalf("PR-3 active business migration table %s missing", table)
		}
	}
	...
	readPR3BusinessFixture(t, "tests/fixtures/contracts/credit/freeze_commit_success.json", &creditCommitFixture)
	frozen, err := repo.FreezeToolCreditsV1(
		t.Context(),
		creditCommitFixture.FreezeRequest,
		creditCommitFixture.HoldAfterFreeze.CreditHoldID,
		"trace_pr3_credit_commit",
	...
	if err := pr3.ValidateFreezeCommitFlow(creditCommitFixture.FreezeRequest, frozen, committed); err != nil {
	...
	readPR3BusinessFixture(t, "tests/fixtures/contracts/credit/freeze_release_on_tool_failure.json", &creditReleaseFixture)
	...
	if err := pr3.ValidateFreezeReleaseOnFailure(frozenFailed, creditReleaseFixture.ToolTaskFailure, released); err != nil {
	...
	readPR3BusinessFixture(t, "tests/fixtures/contracts/asset/partial_commit_success.json", &assetFixture)
	commitResponse, err := repo.CommitGeneratedAssetsV1(
		...
		"trace_pr3_asset_commit",
		assetFixture.ToolResult.CreatedAt,
	)
	if err != nil {
		t.Fatalf("commit generated assets: %v", err)
	}
	if err := pr3.ValidatePartialAssetCommit(assetFixture.ToolResult, commitResponse, assetFixture.BillingRule); err != nil {
	...
	if err := db.DB.Model(&businesscore.PR3CreditLedgerEntryRecord{}).Count(&ledgerCount).Error; err != nil {
	...
	if err := db.DB.Model(&businesscore.PR3GeneratedAssetRecord{}).Count(&generatedAssetCount).Error; err != nil {
...
func readPR3BusinessFixture(t *testing.T, relativePath string, target any) {
	t.Helper()
