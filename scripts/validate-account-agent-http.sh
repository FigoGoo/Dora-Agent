...
echo "== engineering baseline baseline =="
scripts/validate-engineering-baseline.sh

echo "== account agent HTTP gofmt dry check =="
...
echo "== account agent HTTP semantic source checks =="
python3 - <<'PY'
...
for route in [
    ...
    if route not in business_router:
        fail(f"business account agent HTTP route missing: {route}")
...
for route in [
    ...
    if route not in agent_router:
        fail(f"agent account agent HTTP route missing: {route}")
...
for needle in [
    ...
    if needle not in business_migration:
        fail(f"account agent HTTP alignment migration missing {needle}")

if "FOREIGN KEY" in business_migration or "REFERENCES" in business_migration:
    fail("account agent HTTP migration contains database-level external constraint keyword")
...
print("account_agent_http semantic source checks ok")
PY

echo "== account agent HTTP targeted tests =="
...
echo "== account agent HTTP external constraint scan =="
...
echo "== account agent HTTP report truthfulness check =="
python3 - <<'PY'
from pathlib import Path
report = Path("tests/reports/account-agent-http-report.md")
if not report.exists():
    raise SystemExit("missing account agent HTTP report")
...
required = [
    ...
    "Skill、Tool、模型、积分和资产 RPC 不属于 account agent HTTP",
]
for needle in required:
    if needle not in text:
        raise SystemExit(f"account agent HTTP report missing executed command/result: {needle}")
if "未执行项通过" in text or "未执行但通过" in text:
    raise SystemExit("account agent HTTP report claims unexecuted items passed")
print("account_agent_http report check ok")
PY

echo "account agent HTTP validation passed"
