...
echo "== work marketplace baseline =="
scripts/validate-work-marketplace-flow.sh

echo "== service acceptance Go toolchain =="
go version

echo "== service acceptance gofmt dry check =="
...
echo "== service acceptance full Go tests =="
go test -count=1 ./...

echo "== service acceptance SQL up/down pair check =="
python3 - <<'PY'
...
echo "== service acceptance service acceptance semantic checks =="
python3 - <<'PY'
from pathlib import Path
import json
import re
...
for path, method in [
    ...
    if method not in paths.get(path, {}):
        fail(f"OpenAPI missing service acceptance route {method.upper()} {path}")

report = Path("tests/reports/service-acceptance-report.md")
if not report.exists():
    fail("missing service acceptance service acceptance report")
report_text = report.read_text()
for ref in re.findall(r"`(code-plan/[^`]+?\\.md)`", report_text):
    if not Path(ref).exists():
        fail(f"service acceptance report references missing fact-source file: {ref}")
for forbidden in ["阻塞问题：无", "未执行项：无", "smoke 通过", "mock-only"]:
    if forbidden in report_text:
        fail(f"service acceptance report contains over-optimistic phrase: {forbidden}")
for required in ["go test -count=1 ./...", "scripts/validate-service-acceptance.sh", "RPC", "HTTP", "AG-UI", "DB"]:
    if required not in report_text:
        fail(f"service acceptance report missing executed evidence section {required}")

print("service acceptance service acceptance semantic checks ok")
PY

echo "== service acceptance AG-UI fixtures =="
python3 tests/agent/agui/validate_fixtures.py

echo "== service acceptance contract fixtures =="
python3 tests/contract/validate_fixtures.py

echo "== service acceptance service e2e =="
test -d tests/e2e/service
python3 tests/e2e/service/validate_service_acceptance_e2e.py

echo "== service acceptance no database-level FK =="
...
echo "service acceptance service acceptance baseline passed"
