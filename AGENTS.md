...
主控 Codex = 协调者。负责拆解任务、选择 subagent、维护跨服务契约、合并结果并做最终验收。

subagent = 专项执行者。每个 subagent 只处理自己的 ownership 和职责范围，不跨边界替其他角色做决策。
...
1. 主控 Codex 只并发调度相互独立的任务。
2. subagent 不递归调度其他 subagent。
3. 涉及 RPC 或 AG-UI 的任务必须先明确契约 owner。
4. 产品体验设计师产出《产品系统设计》后，Go Eino 智能体微服务架构工程师、前端开发工程师、业务微服务后端工程师必须先按 `docs/standards/产品设计交接与开发约束规范.md` 输出需求映射矩阵，再进入契约或代码开发。
5. 产品文档未达到 `product_status: Done` 时，只能做澄清、评审、技术预研或文档修订，不能进入正式代码开发。
6. 开发按功能点拆分，并按功能点提交到 GitHub。
7. 跨边界冲突先停下并回报主控 Codex，不允许自行猜测。
8. 测试缺陷按 ownership 回派给对应 subagent 修复。
...
`karpathy-guidelines` Skill 只提供给以下 subagent 使用：业务微服务后端工程师、Go Eino 智能体微服务架构工程师、前端开发工程师。产品体验设计师和浏览器、RPC 与数据库测试工程师不使用该 Skill。
