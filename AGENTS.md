...
主控 Codex = 单一协作者。负责拆解任务、按领域 owner 边界执行或检查、维护跨服务契约并做最终验收。

领域 owner = 文档、契约和代码的责任边界，用于判断事实源维护者、语义确认者和评审责任；不是可调度配置，也不代表并发执行单元。
...
主控 Codex 默认在当前会话内单线程推进开发。根据任务类型参考以下领域 owner 边界：
...
1. 主控 Codex 直接执行任务，不启动并发协作者。
2. 领域 owner 只用于确认 ownership、契约语义和评审责任。
