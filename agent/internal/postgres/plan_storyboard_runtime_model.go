...
// planStoryboardPreviewRunModel 映射 Storyboard Preview 的稳定执行身份、Fence 与 Run 状态。
type planStoryboardPreviewRunModel struct {
	...
	// RequestDigest 是 canonical typed Intent 与 CreationSpec 精确引用的请求摘要。
...
// TableName 返回 Storyboard Preview 稳定 Run 的显式 Agent Schema 表名。
func (planStoryboardPreviewRunModel) TableName() string {
	return "agent.plan_storyboard_preview_run"
}

// planStoryboardPreviewContextModel 映射入队事务冻结且 append-only 的可信 Turn Context。
type planStoryboardPreviewContextModel struct {
	...
	RouterModelCallID string `gorm:"column:router_model_call_id;type:uuid"`
	// GraphModelCallID 是冻结的 Graph Planning Model Call UUIDv7。
	GraphModelCallID string `gorm:"column:graph_model_call_id;type:uuid"`
	...
	// CreationSpecID 是冻结的 Business CreationSpec Draft UUIDv7。
	CreationSpecID string `gorm:"column:creation_spec_id;type:uuid"`
	// CreationSpecVersion 是冻结的 CreationSpec Draft 精确版本。
	CreationSpecVersion int64 `gorm:"column:creation_spec_version"`
	// CreationSpecContentDigest 是冻结的 CreationSpec 内容 SHA-256 摘要。
	CreationSpecContentDigest string `gorm:"column:creation_spec_content_digest"`
	// AccessScopeRef 是冻结的 Owner、Project 与 CreationSpec 访问范围引用。
	...
	// DAGValidatorRef 是局部引用与依赖 DAG Validator 引用。
	DAGValidatorRef string `gorm:"column:dag_validator_ref"`
	// DAGValidatorDigest 是 DAG Validator canonical 摘要。
	DAGValidatorDigest string `gorm:"column:dag_validator_digest"`
	...
	// PlanningModelRouteRef 是本地 Graph Fake Planning Model Route 引用。
	PlanningModelRouteRef string `gorm:"column:planning_model_route_ref"`
	// PlanningModelRouteDigest 是 Graph Planning Model Route canonical 摘要。
	PlanningModelRouteDigest string `gorm:"column:planning_model_route_digest"`
...
// TableName 返回 Storyboard Preview 不可变 Turn Context 的显式 Agent Schema 表名。
func (planStoryboardPreviewContextModel) TableName() string {
	return "agent.plan_storyboard_preview_turn_context"
}

// planStoryboardPreviewModelReceiptModel 映射 Router 或 Graph Planning Fake Model 的冻结调用回执。
type planStoryboardPreviewModelReceiptModel struct {
	// ModelCallID 是 Router 或 Graph Model 稳定调用主键。
	ModelCallID string `gorm:"column:model_call_id;type:uuid;primaryKey"`
	// CallKind 区分 router 与 graph_planning 命名空间。
...
// TableName 返回 Storyboard Preview 分层 Model Receipt 的显式 Agent Schema 表名。
func (planStoryboardPreviewModelReceiptModel) TableName() string {
	return "agent.plan_storyboard_preview_model_receipt"
}

// planStoryboardPreviewToolReceiptModel 映射 receipt-first Tool Result 与 Business Unknown Recovery 工件。
type planStoryboardPreviewToolReceiptModel struct {
	...
	// ContentDigest 是 prepared Storyboard Draft 完整内容摘要。
...
// TableName 返回 Storyboard Preview receipt-first Tool Receipt 的显式 Agent Schema 表名。
func (planStoryboardPreviewToolReceiptModel) TableName() string {
	return "agent.plan_storyboard_preview_tool_receipt"
}
