...
var (
	...
	ErrInvalidClaim = errors.New("plan storyboard runtime claim is invalid")
	// ErrReceiptConflict 表示同稳定键出现异义摘要或损坏回执。
	ErrReceiptConflict = errors.New("plan storyboard runtime receipt conflict")
	// ErrOutputContract 表示 Eino 或 Tool 输出不满足严格结果契约。
	ErrOutputContract = errors.New("plan storyboard runtime output contract violation")
	// ErrModelReceiptReserved 表示同 Fence 已经占位但没有可重放终态。
	ErrModelReceiptReserved = errors.New("plan storyboard model receipt is reserved")
	// ErrFenceLost 表示 owner/fence 已失效；调用方不得继续写回执或终态。
	ErrFenceLost = errors.New("plan storyboard runtime fence lost")
	// ErrPersistence 表示持久化提交结果未知或读取暂时不可用。
	ErrPersistence = errors.New("plan storyboard runtime persistence unavailable")
	// ErrIdempotencyConflict 表示同 Session 与幂等键承载了不同 typed Intent 或 CreationSpecRef。
	ErrIdempotencyConflict = errors.New("plan storyboard runtime idempotency conflict")
	// ErrInvalidInput 表示 Handler 可稳定映射为 400 的严格请求失败。
	ErrInvalidInput = errors.New("plan storyboard runtime invalid input")
	// ErrNotFound 表示 Session、Project 或 CreationSpec 不存在或不可见。
	ErrNotFound = errors.New("plan storyboard runtime resource not found")
	// ErrSessionLaneBlocked 表示当前 Session Lane 不能接受本 Profile 输入。
	ErrSessionLaneBlocked = errors.New("plan storyboard runtime session lane blocked")
	// ErrRecoveryDeferred 表示 prepared/unknown 命令仍未形成权威终态，必须保持 HOL 未决。
	ErrRecoveryDeferred = errors.New("plan storyboard runtime recovery deferred")
	// ErrRecoveryExhausted 表示最后一次同键重发后 Business 仍权威 not_found，必须终结为 runtime failure。
	ErrRecoveryExhausted = errors.New("plan storyboard runtime recovery exhausted")
)

// EnqueueCommand 是显式 typed Intent 与可信 CreationSpecRef 的入队端口，不创建 session_message。
type EnqueueCommand struct {
	...
	// CreationSpecRef 是 BFF 绑定并由 Runtime 冻结的上游 Draft 引用。
...
type EnqueueResult struct {
	...
	ToolCallID string
	// BusinessCommandID 是 Storyboard 保存与恢复复用的命令 UUIDv7。
	BusinessCommandID string
	// RouterModelCallID 是外层 Router 模型回执 UUIDv7。
	RouterModelCallID string
	// GraphModelCallID 是 Graph Planning 模型回执 UUIDv7。
...
// ValidateClaim 逐值复核稳定身份、Profile pins、canonical Intent、CreationSpecRef 与 Context digest。
func ValidateClaim(claim Claim) error {
	ctx := claim.Context
	if claim.Owner == "" || claim.FenceToken < 1 || claim.Attempts < 1 || claim.EnqueueSeq < 1 ||
		ctx.SchemaVersion != turncontext.PlanStoryboardTurnContextSchemaVersion || ctx.Profile != Profile ||
		...
		ctx.IntentKeyVersion == "" || !canonicalUUIDv7.MatchString(ctx.CreationSpecID) ||
		ctx.CreationSpecVersion != 1 || !canonicalSHA256.MatchString(ctx.CreationSpecContentDigest) {
...
func validatePins(ctx turncontext.PlanStoryboardTurnContext) error {
	pins := ApprovedPins()
	if ctx.ToolRegistryRef != pins.ToolRegistryRef || ctx.ToolRegistryDigest != pins.ToolRegistryDigest ||
		...
		ctx.IntentSchemaRef != planstoryboard.IntentSchemaVersion || ctx.CandidateSchemaRef != planstoryboard.CandidateSchemaVersion ||
		ctx.ResultSchemaRef != planstoryboard.ResultSchemaVersion || ctx.PromptRef != pins.PromptRef ||
		ctx.PromptDigest != pins.PromptDigest || ctx.ValidatorRef != pins.ValidatorRef ||
		ctx.ValidatorDigest != pins.ValidatorDigest || ctx.DAGValidatorRef != pins.DAGValidatorRef ||
		ctx.DAGValidatorDigest != pins.DAGValidatorDigest || ctx.RouterModelRouteRef != pins.RouterModelRouteRef ||
		ctx.RouterModelRouteDigest != pins.RouterModelRouteDigest || ctx.PlanningModelRouteRef != pins.PlanningModelRouteRef ||
		ctx.PlanningModelRouteDigest != pins.PlanningModelRouteDigest || ctx.RuntimePolicyRef != pins.RuntimePolicyRef ||
...
func DigestTurnContext(value turncontext.PlanStoryboardTurnContext) (string, error) {
	value.ContextDigest = ""
	encoded, err := json.Marshal(value)
	if err != nil {
		return "", fmt.Errorf("digest plan storyboard turn context: %w", err)
	}
...
func RuntimeContextFromClaim(claim Claim) turncontext.PlanStoryboardRuntime {
	return turncontext.PlanStoryboardRuntime{
...
func PreviewContextFromClaim(claim Claim) turncontext.PlanStoryboardPreview {
	ctx := claim.Context
	return turncontext.PlanStoryboardPreview{
		...
		CreationSpecID: ctx.CreationSpecID, CreationSpecVersion: ctx.CreationSpecVersion,
		CreationSpecContentDigest: ctx.CreationSpecContentDigest, PromptVersion: planstoryboard.PromptVersion,
		ValidatorVersion: planstoryboard.ValidatorVersion, DAGValidatorVersion: planstoryboard.DAGValidatorVersion,
	}
}

// CoreContextFromRuntime 构造 Graph Tool Core 需要的最小可信上下文，模型参数无权覆盖任何字段。
func CoreContextFromRuntime(value turncontext.PlanStoryboardRuntime) planstoryboard.TrustedContext {
...
func CoreContextFromPreview(ctx turncontext.PlanStoryboardPreview) planstoryboard.TrustedContext {
	return planstoryboard.TrustedContext{
		...
		CreationSpecRef: planstoryboard.CreationSpecRef{ID: ctx.CreationSpecID, Version: ctx.CreationSpecVersion, ContentDigest: ctx.CreationSpecContentDigest},
		PromptVersion:   ctx.PromptVersion, ValidatorVersion: ctx.ValidatorVersion, DAGValidatorVersion: ctx.DAGValidatorVersion,
	}
}

// ResolveCoreContext 从 Runtime 私有 Context 提供 M1 Tool Core 需要的可信 Resolver。
func ResolveCoreContext(ctx context.Context) (planstoryboard.TrustedContext, bool) {
	value, ok := turncontext.PlanStoryboardPreviewFrom(ctx)
	if !ok {
		return planstoryboard.TrustedContext{}, false
	}
