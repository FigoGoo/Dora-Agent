...
const (
	...
	EnqueueResponseSchemaVersion = "plan_storyboard.preview.enqueue.v1"
	// EnqueuePendingStatus 表示 typed Input 已持久化，后台尚未形成 Tool Result。
	EnqueuePendingStatus = "pending"
)

// EnqueueRequest 是 Handler 完成身份与 CreationSpec 卡片绑定校验后交给 Service 的 DTO。
type EnqueueRequest struct {
	...
	// CreationSpecRef 是 Workspace 当前权威 Card 绑定的 Draft 引用。
...
// EnqueueResponse 是 Business BFF 可安全转发的 202 DTO，不含 Intent、CreationSpec 摘要或内部 pins。
...
// Service 负责 strict Intent、可信 CreationSpecRef、Access Scope pin 与成功入队唤醒。
type Service struct {
	store EnqueueStore
	clock Clock
	wake  func()
...
// NewService 创建 Storyboard typed Intent 入队 Service。
func NewService(store EnqueueStore, clock Clock, wake func()) (*Service, error) {
	if store == nil || clock == nil || wake == nil {
		return nil, fmt.Errorf("create plan storyboard runtime service: invalid dependency")
	}
...
func (s *Service) Enqueue(ctx context.Context, request EnqueueRequest) (EnqueueResponse, error) {
	if !canonicalUUIDv7.MatchString(request.RequestID) || !canonicalUUIDv7.MatchString(request.SessionID) ||
		...
		!canonicalUUIDv7.MatchString(request.IdempotencyKey) || !canonicalUUIDv7.MatchString(request.CreationSpecRef.ID) ||
		request.CreationSpecRef.Version != 1 || !canonicalSHA256.MatchString(request.CreationSpecRef.ContentDigest) ||
	...
	result, err := s.store.Enqueue(ctx, EnqueueCommand{
		...
		IdempotencyKey: request.IdempotencyKey, CreationSpecRef: request.CreationSpecRef, IntentJSON: intent.JSON,
