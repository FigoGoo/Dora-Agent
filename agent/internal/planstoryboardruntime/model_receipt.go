...
// ModelCallKind 区分外层 Router 和 Graph 内部 Storyboard 候选模型的独立回执命名空间。
type ModelCallKind string

const (
	// ModelCallRouter 是唯一主 Agent 的一次 ToolCall 生成。
	ModelCallRouter ModelCallKind = "router"
	// ModelCallGraphPlanning 是 Graph call_model 的一次候选生成。
	ModelCallGraphPlanning ModelCallKind = "graph_planning"
)
...
const (
	// ModelReceiptReserved 表示当前 Fence 已占位但尚无终态。
	ModelReceiptReserved ModelReceiptStage = "reserved"
	// ModelReceiptCompleted 表示完整经典 Message 已冻结。
	ModelReceiptCompleted ModelReceiptStage = "completed"
	// ModelReceiptFailed 表示安全错误码已冻结。
...
type ModelReceiptIdentity struct {
	...
	// RunID 是 Run UUIDv7。
	RunID string
	// ModelCallID 是 Router 或 Graph Model 调用 UUIDv7。
	ModelCallID string
	// CallKind 是独立回执命名空间。
...
// NewReceiptModel 创建 Router 或 Graph Planning 的独立回执包装器。
func NewReceiptModel(base model.BaseChatModel, store ModelReceiptStore, kind ModelCallKind) (*ReceiptModel, error) {
	if base == nil || store == nil || (kind != ModelCallRouter && kind != ModelCallGraphPlanning) {
		return nil, fmt.Errorf("create plan storyboard receipt model: invalid dependency or call kind")
	}
...
func (m *ReceiptModel) Generate(ctx context.Context, messages []*schema.Message, options ...model.Option) (*schema.Message, error) {
	...
	if !ok || trusted.Owner == "" || trusted.FenceToken < 1 || trusted.Context.Profile != Profile {
		return nil, fmt.Errorf("run plan storyboard receipt model: trusted runtime context is invalid")
	}
	...
	if !terminal || execute {
		return nil, fmt.Errorf("run plan storyboard receipt model: frozen response is unavailable")
	}
	return response, terminalErr
}
...
func (m *ReceiptModel) WithTools(tools []*schema.ToolInfo) (model.ToolCallingChatModel, error) {
	if m.kind != ModelCallRouter {
		return nil, fmt.Errorf("bind plan storyboard receipt model: graph planning model cannot bind tools")
	}
	base, ok := m.base.(model.ToolCallingChatModel)
	if !ok {
		return nil, fmt.Errorf("bind plan storyboard receipt model: router does not support tools")
	}
...
func (m *ReceiptModel) identity(value turncontext.PlanStoryboardRuntime) (ModelReceiptIdentity, string, string, error) {
	ctx := value.Context
	identity := ModelReceiptIdentity{
		Owner: value.Owner, FenceToken: value.FenceToken, SessionID: ctx.SessionID,
		InputID: ctx.InputID, TurnID: ctx.TurnID, RunID: ctx.RunID, CallKind: m.kind,
	}
	switch m.kind {
	case ModelCallRouter:
		identity.ModelCallID = ctx.RouterModelCallID
		return identity, ctx.RouterModelRouteRef, ctx.RouterModelRouteDigest, nil
	case ModelCallGraphPlanning:
		identity.ModelCallID = ctx.GraphModelCallID
		return identity, ctx.PlanningModelRouteRef, ctx.PlanningModelRouteDigest, nil
	default:
		return ModelReceiptIdentity{}, "", "", fmt.Errorf("run plan storyboard receipt model: unknown call kind")
	}
}
...
func terminalModelReceipt(snapshot ModelReceiptSnapshot) (*schema.Message, error, bool) {
	...
	case ModelReceiptFailed:
		...
		return nil, fmt.Errorf("plan storyboard local model failed: %s", snapshot.ErrorCode), true
...
func digestModelRequest(
	...
	if len(messages) == 0 || !canonicalUUIDv7.MatchString(callID) || len(contextDigest) != 64 || routeRef == "" || len(routeDigest) != 64 {
		return "", fmt.Errorf("digest plan storyboard model request: invalid identity or messages")
	}
	wire := struct {
	...
	}{"plan_storyboard.model_request.v2preview1", kind, callID, contextDigest, routeRef, routeDigest, messages}
	encoded, err := json.Marshal(wire)
	if err != nil {
		return "", fmt.Errorf("digest plan storyboard model request: %w", err)
	}
