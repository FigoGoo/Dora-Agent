...
type Runner interface {
	Run(context.Context, Claim) (analyzematerials.Result, error)
}
...
func NewEinoRunner(ctx context.Context, agent adk.Agent) (*EinoRunner, error) {
	if agent == nil || (agent.Name(ctx) != chatmodelagent.AnalyzeMaterialsName && agent.Name(ctx) != chatmodelagent.MVPAllToolsName) {
		return nil, fmt.Errorf("create analyze materials Eino runner: exact preview agent is required")
	}
	runner := adk.NewRunner(ctx, adk.RunnerConfig{Agent: agent, EnableStreaming: false})
	if runner == nil {
		return nil, fmt.Errorf("create analyze materials Eino runner: ADK runner is nil")
	}
	return &EinoRunner{runner: runner}, nil
}

// Run 注入 Runtime/Core 双层可信上下文，只接受 ReturnDirectly 的单个 Tool Result。
func (r *EinoRunner) Run(ctx context.Context, claim Claim) (analyzematerials.Result, error) {
	if r == nil || r.runner == nil || ValidateClaim(claim) != nil {
		return analyzematerials.Result{}, ErrInvalidClaim
	}
	ctx = turncontext.WithMaterialAnalysisRuntime(ctx, RuntimeContextFromClaim(claim))
	...
	if iterator == nil {
		return analyzematerials.Result{}, fmt.Errorf("run analyze materials Eino agent: nil iterator")
	}
	...
	for {
		...
		if variant.Role != schema.Tool || variant.ToolName != analyzematerials.ToolKey || seen {
			if runErr == nil {
	...
	if err := ctx.Err(); err != nil {
		return analyzematerials.Result{}, err
	}
	if runErr != nil {
		return analyzematerials.Result{}, runErr
	}
	if !seen {
		return analyzematerials.Result{}, fmt.Errorf("%w: Tool output is missing", ErrOutputContract)
	}
	return result, nil
}
