...
// ModelCallKind 区分外层 Router 和 Graph 内部 Prompt 候选模型的独立回执命名空间。
type ModelCallKind string

const (
	// ModelCallRouter 是唯一主 Agent 的一次 ToolCall 生成。
	ModelCallRouter ModelCallKind = "router"
	// ModelCallGraphPrompt 是 Graph call_model 的一次候选生成。
	ModelCallGraphPrompt ModelCallKind = "graph_prompt"
)
...
// NewReceiptModel 创建 Router 或 Graph Prompt 的独立回执包装器。
func NewReceiptModel(base model.BaseChatModel, store ModelReceiptStore, kind ModelCallKind) (*ReceiptModel, error) {
	if base == nil || store == nil || (kind != ModelCallRouter && kind != ModelCallGraphPrompt) {
		return nil, fmt.Errorf("create write prompts receipt model: invalid dependency or call kind")
	}
...
func (m *ReceiptModel) Generate(ctx context.Context, messages []*schema.Message, options ...model.Option) (*schema.Message, error) {
	...
	if !ok || trusted.Owner == "" || trusted.FenceToken < 1 || trusted.Context.Profile != Profile {
		return nil, fmt.Errorf("run write prompts receipt model: trusted runtime context is invalid")
	}
	...
	if !terminal || execute {
		return nil, fmt.Errorf("run write prompts receipt model: frozen response is unavailable")
	}
	return response, terminalErr
}
...
func (m *ReceiptModel) WithTools(tools []*schema.ToolInfo) (model.ToolCallingChatModel, error) {
	if m.kind != ModelCallRouter {
		return nil, fmt.Errorf("bind write prompts receipt model: graph prompt model cannot bind tools")
	}
	base, ok := m.base.(model.ToolCallingChatModel)
	if !ok {
		return nil, fmt.Errorf("bind write prompts receipt model: router does not support tools")
	}
...
func (m *ReceiptModel) identity(value turncontext.WritePromptsRuntime) (ModelReceiptIdentity, string, string, error) {
	ctx := value.Context
	...
	case ModelCallGraphPrompt:
		identity.ModelCallID = ctx.GraphModelCallID
		return identity, ctx.PromptModelRouteRef, ctx.PromptModelRouteDigest, nil
	default:
		return ModelReceiptIdentity{}, "", "", fmt.Errorf("run write prompts receipt model: unknown call kind")
	}
}
...
func terminalModelReceipt(snapshot ModelReceiptSnapshot) (*schema.Message, error, bool) {
	...
	case ModelReceiptFailed:
		...
		return nil, fmt.Errorf("write prompts local model failed: %s", snapshot.ErrorCode), true
...
func digestModelRequest(
	...
	if len(messages) == 0 || !canonicalUUIDv7.MatchString(callID) || len(contextDigest) != 64 || routeRef == "" || len(routeDigest) != 64 {
		return "", fmt.Errorf("digest write prompts model request: invalid identity or messages")
	}
	wire := struct {
	...
	}{"write_prompts.model_request.v2preview1", kind, callID, contextDigest, routeRef, routeDigest, messages}
	encoded, err := json.Marshal(wire)
	if err != nil {
		return "", fmt.Errorf("digest write prompts model request: %w", err)
	}
