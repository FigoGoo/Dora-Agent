...
type Repository interface {
	...
	CompleteToolResult(context.Context, Claim, writeprompts.Result, time.Time) error
...
func NewProcessor(
	...
	if repository == nil || runner == nil || recovery == nil || clock == nil || owner == "" ||
		...
		return nil, fmt.Errorf("create write prompts processor: invalid dependency or resource budget")
	}
	return &Processor{
...
func (p *Processor) Start() error {
	...
	defer p.mu.Unlock()
	if p.started {
		return fmt.Errorf("start write prompts processor: already started")
	}
...
func (p *Processor) Stop(ctx context.Context) error {
	...
	case <-ctx.Done():
		runStop()
		return fmt.Errorf("stop write prompts processor: %w", ctx.Err())
	}
}

// ProcessNext 同步执行一次全来源 HOL Claim 与既有 write_prompts 状态机；无工作时返回 false。
...
func validateClaimToolReceipt(claim Claim, snapshot ToolReceiptSnapshot) (writeprompts.Result, bool, error) {
	...
	case ToolReceiptOpen:
		if snapshot.PreparedCommand != nil || snapshot.PreparedCommandDigest != "" || snapshot.ContentDigest != "" ||
			...
			return writeprompts.Result{}, false, ErrReceiptConflict
		}
		return writeprompts.Result{}, false, nil
	case ToolReceiptBusinessPrepared, ToolReceiptBusinessUnknown:
		_, _, err := validateRecoverySnapshot(claim, snapshot)
		return writeprompts.Result{}, false, err
...
func validateCompletedCommand(claim Claim, snapshot ToolReceiptSnapshot, result writeprompts.Result) error {
	command := *snapshot.PreparedCommand
	if command.TrustedContext != CoreContextFromRuntime(RuntimeContextFromClaim(claim)) || result.PromptPreviewRef == nil || result.StoryboardPreviewRef == nil {
	...
	if err != nil || contentDigest != snapshot.ContentDigest || contentDigest != result.PromptPreviewRef.ContentDigest ||
		*result.StoryboardPreviewRef != command.TrustedContext.StoryboardPreviewRef ||
		result.TargetCount != len(command.Content.Prompts) {
		return ErrReceiptConflict
	}
	return nil
}

func (p *Processor) complete(ctx context.Context, claim Claim, snapshot ToolReceiptSnapshot, result writeprompts.Result) {
	if result.Status == "completed" {
		result.Card = cardFromPreparedResult(*snapshot.PreparedCommand, result, p.clock.Now().UTC())
		if writeprompts.ValidateTerminalResult(result, CoreContextFromRuntime(RuntimeContextFromClaim(claim))) != nil {
			p.runtimeFailure(ctx, claim)
			return
		}
	}
...
func cardFromPreparedResult(command writeprompts.DraftCommand, result writeprompts.Result, now time.Time) *writeprompts.Card {
	ref := result.PromptPreviewRef
	if ref == nil || result.StoryboardPreviewRef == nil {
		return nil
	}
	return &writeprompts.Card{
		...
		PromptPreviewID: ref.ID, ProjectID: command.TrustedContext.ProjectID,
		StoryboardPreviewRef: result.StoryboardPreviewRef, Version: ref.Version, ContentDigest: ref.ContentDigest,
		TargetCount: len(command.Content.Prompts), Prompts: clonePromptEntries(command.Content.Prompts), UpdatedAt: now.UTC(),
	}
}

func (p *Processor) runtimeFailure(ctx context.Context, claim Claim) {
	failure := RuntimeFailure{
		SchemaVersion: "write_prompts.preview.runtime_failure.v1", InputID: claim.Context.InputID,
		TurnID: claim.Context.TurnID, RunID: claim.Context.RunID, Code: "WRITE_PROMPTS_RUNTIME_FAILED",
		Summary: "Prompt 写作运行时暂时无法完成", Retryable: false,
	}
