...
const (
	...
	approvalConsumptionVectorCountV1  = 102
...
func evaluateApprovalConsumptionRecordV1(fixture approvalConsumptionFixtureV1) approvalConsumptionEvaluationV1 {
	...
	if exact := findApprovalConsumptionIndexV1(fixture.StoredConsumptionIndex, intent.ApprovalID, intent.ConsumptionKey); exact != nil {
		if reason := validateApprovalConsumptionStoredIndexV1(*exact); reason != "" {
			return invalidApprovalConsumptionV1(reason)
		}
		stored := findApprovalConsumptionCoreV1(fixture.StoredCores, exact.CoreDigest)
		if stored == nil || validateApprovalConsumptionStoredCoreV1(*exact, *stored) != "" {
	...
	if backstop := findApprovalConsumptionApprovalIndexV1(fixture.StoredConsumptionIndex, intent.ApprovalID); backstop != nil {
...
func evaluateApprovalConsumptionQueryV1(fixture approvalConsumptionFixtureV1) approvalConsumptionEvaluationV1 {
	...
	index := findApprovalConsumptionIndexV1(fixture.StoredConsumptionIndex, command.ApprovalID, command.ConsumptionKey)
	if index == nil {
	...
	stored := findApprovalConsumptionCoreV1(fixture.StoredCores, index.CoreDigest)
	if stored == nil || validateApprovalConsumptionStoredCoreV1(*index, *stored) != "" {
		return invalidApprovalConsumptionV1("STORED_CONSUMPTION_CORE_INVALID")
	}
...
func validateApprovalConsumptionFreshRecordMaterialV1(fixture approvalConsumptionFixtureV1) string {
	...
	for _, evidence := range material.RevalidationObservation.Evidence {
...
func findApprovalConsumptionIndexV1(indexes []approvalConsumptionStoredIndexV1, approvalID, key string) *approvalConsumptionStoredIndexV1 {
	for index := range indexes {
		if indexes[index].ApprovalID == approvalID && indexes[index].ConsumptionKey == key {
...
func findApprovalConsumptionApprovalIndexV1(indexes []approvalConsumptionStoredIndexV1, approvalID string) *approvalConsumptionStoredIndexV1 {
	for index := range indexes {
...
func findApprovalConsumptionCoreV1(cores []approvalConsumptionStoredCoreV1, digest string) *approvalConsumptionStoredCoreV1 {
...
func approvalConsumptionVectorIDsV1() []string {
	return []string{
		...
		"ACR-N95-stored-source-formula", "ACR-N96-stored-resulting-version",
