...
func ValidateToolPlan(plan ToolPlan) error {
	...
	if !foundation.IsValidState(foundation.StateToolPlanStatus, plan.Status) {
	...
	if err := foundation.ValidateDigest(plan.ToolPlanDigest); err != nil {
...
func ValidateToolPlanItem(item ToolPlanItem) error {
	...
	if err := foundation.ValidateDigest(item.InputDigest); err != nil {
...
func ValidateToolTask(task ToolTask) error {
	...
	if !foundation.IsValidState(foundation.StateToolTaskStatus, task.Status) {
	...
	if err := foundation.ValidateDigest(task.InputDigest); err != nil {
		return fmt.Errorf("input_digest: %w", err)
	}
	if task.OutputDigest != nil {
		if err := foundation.ValidateDigest(*task.OutputDigest); err != nil {
...
func ValidateToolResult(result ToolResult) error {
	...
	if err := foundation.ValidateDigest(result.ResultDigest); err != nil {
