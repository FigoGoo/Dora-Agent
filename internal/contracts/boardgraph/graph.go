...
func ValidateGenericCreationGraph(graph GenericCreationGraph) error {
	...
	if err := foundation.ValidateDigest(graph.GraphTemplateDigest); err != nil {
...
func ValidateGraphTemplate(template GraphTemplate) error {
	...
	for index, node := range template.Nodes {
		...
		if node.ConfigDigest != nil {
			if err := foundation.ValidateDigest(*node.ConfigDigest); err != nil {
	...
	if err := foundation.ValidateDigest(template.TemplateDigest); err != nil {
...
func ValidateGraphPlan(plan GraphPlan) error {
	...
	if !foundation.IsValidState(foundation.StateGraphPlanStatus, plan.Status) {
	...
	for index, node := range plan.Nodes {
		...
		if node.InputDigest != nil {
			if err := foundation.ValidateDigest(*node.InputDigest); err != nil {
				return fmt.Errorf("node %d input_digest: %w", index+1, err)
			}
		}
		if node.OutputDigest != nil {
			if err := foundation.ValidateDigest(*node.OutputDigest); err != nil {
	...
	if err := foundation.ValidateDigest(plan.GraphPlanDigest); err != nil {
...
func ValidateGraphCheckpoint(checkpoint GraphCheckpoint) error {
	...
	if err := foundation.ValidateDigest(checkpoint.StateDigest); err != nil {
