...
func ValidateCreativeBoard(board CreativeBoard) error {
	...
	if !foundation.IsValidState(foundation.StateBoardStatus, board.Status) {
	...
	if err := foundation.ValidateDigest(board.BoardDigest); err != nil {
...
func ValidateCreativeElement(element CreativeElement) error {
	...
	if err := foundation.ValidateDigest(element.ContentDigest); err != nil {
...
func ValidateBoardPatch(patch BoardPatch) error {
	...
	if err := foundation.ValidateDigest(patch.PatchDigest); err != nil {
...
func ValidateBoardSnapshot(snapshot BoardSnapshot) error {
	...
	if !foundation.IsValidState(foundation.StateBoardStatus, snapshot.Status) {
	...
	if err := foundation.ValidateDigest(snapshot.BoardDigest); err != nil {
		return fmt.Errorf("board_digest: %w", err)
	}
