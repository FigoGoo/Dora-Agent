...
type E2EFixture struct {
	...
	Status                 string              `json:"status"`
	Owner                  string              `json:"owner"`
	DependsOnContracts     []string            `json:"depends_on_contracts"`
...
func ValidateE2EFixture(relativePath string, fixture E2EFixture) error {
	...
	for _, requiredContract := range expected.DependsOnContracts {
		if !contains(fixture.DependsOnContracts, requiredContract) {
			return fmt.Errorf("%s missing dependency %s", relativePath, requiredContract)
		}
	}
