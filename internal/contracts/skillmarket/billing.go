...
func ValidateSkillUsageRecord(record SkillUsageRecord) error {
	...
	if err := foundation.ValidateDigest(record.PricingPolicyDigest); err != nil {
		return fmt.Errorf("pricing_policy_digest: %w", err)
	}
	if err := foundation.ValidateDigest(record.SkillUsageDigest); err != nil {
		return fmt.Errorf("skill_usage_digest: %w", err)
	}
	if !foundation.IsValidState(foundation.StateSkillUsageStatus, record.UsageStatus) {
		return fmt.Errorf("invalid usage_status %q", record.UsageStatus)
	}
	if !foundation.IsValidState(foundation.StateSkillUsageChargeStatus, record.ChargeStatus) {
		return fmt.Errorf("invalid charge_status %q", record.ChargeStatus)
	}
	if !foundation.IsValidState(foundation.StateSkillUsageRefundStatus, record.RefundStatus) {
		return fmt.Errorf("invalid refund_status %q", record.RefundStatus)
	}
	if !foundation.IsValidState(foundation.StateSettlementStatus, record.SettlementStatus) {
...
func ValidateSkillSettlement(settlement SkillSettlement) error {
	...
	if !foundation.IsValidState(foundation.StateSettlementStatus, settlement.Status) {
