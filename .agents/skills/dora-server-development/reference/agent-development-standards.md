...
| 数据库 | PostgreSQL 16 | 本地镜像固定为 `postgres:16.4-alpine`，与 `deploy/local/compose.yaml` 一致 |
...
实施 Agent 或 AIGC Graph Tool 前，除本规范外还必须按改动范围读取：

- `docs/README.md`：唯一文档入口和功能路由；
- `docs/requirements/delivery-status.md`：唯一阶段状态源；
- `docs/design/system-architecture.md`：Module、Owner、跨边界和一致性基线；
- `docs/design/functions/<function>.md`：对应用户功能的当前实现设计；
- `docs/design/agent/graphtool/<tool_key>-design.md`：每个 Agent-facing Graph Tool 的强制独立设计。

`make trial-basic` 的固定验收范围包括统一六 Tool、Worker Job Claim/Lease、Business Finalize、确定性 PNG、固定参数 MP4、同源读取与 Workspace V5 恢复。它只验证基本功能 happy path，不替代五条 isolated smoke、进程重启、Fence takeover、故障注入、三 Module 全量门禁或生产发布。当前结果、下一步和门禁分层只以 `docs/requirements/delivery-status.md` 为准；各 Tool 的 Node、Branch、State、状态机与批准范围只以固定路径独立设计为准。

文档按功能维护，不再按 W0/W1/W2、Rxx、历史 `main` 或单次试跑建立新的叙述性真源。冻结 Approval Manifest 及其引用文档只用于审计，不维护当前状态；Git 历史和已删除文档不能作为实现依据。任何“已实现”声明都必须同时有当前代码、Migration 和可重复测试支撑。

设计文档与本规范冲突时，按以下边界处理：

- 删除或替换无业务含义的 Tenant/TenantID；
- 文档中的外键关系均为逻辑外键，不创建数据库物理外键；
- PostgreSQL 是 Session、Job、Receipt 和 Event 权威来源，Redis 只缓存和唤醒；
- 用户、鉴权、Skill、Storyboard、Binding、Asset、积分和费用归 Business；
- Worker 只执行已持久化 Job、生成/校验产物并请求 Business Finalize；
- 当前实现、Production P1 和产品 P2 必须明确分层，禁止把目标能力写成当前事实。
