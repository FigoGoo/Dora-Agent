...
8. PostgreSQL 是 Session、Input、Run、Turn、Checkpoint、Model/Tool 回执、Approval、Operation、Batch、Job、Outbox、Skill 版本和事件日志的权威来源；Redis 只用于缓存、短期协调和唤醒。
...
- `skill` 负责系统 Skill 和用户 Skill 的发布、加载、版本冻结及权限；不得注册 Tool，也不得依赖具体 Graph Tool。
...
- 新场景优先新增或版本化 Skill；只有需要模型可选择的高层能力时才新增 Graph Tool。
...
5. 动态 Skill Middleware 每个新 Turn 加载一次并冻结有效版本；同一 Turn 的技术重试和恢复不得重新选择版本。
...
- Skill 内容和元数据存放在 Agent 自有 PostgreSQL，通过 Eino Skill Middleware 动态加载。
- Eino Skill Middleware 使用固定的数据库 Backend 和内部 Loader；Loader 只把审核通过的 Skill 内容以内联方式注入当前主 Agent，不属于业务 Graph Tool 白名单，不产生用户可见 ToolRun，也不能执行业务副作用。
- Skill 的上下文模式只允许 inline，必须拒绝 `fork` 和 `fork_with_context`，防止通过 Skill 变相创建子 Agent。
- 同一 Skill 版本在一个 Turn 内最多成功加载一次；Loader 调用仍消耗该 Turn 的迭代、Tool、时间和 Token 预算，不得成为绕过硬预算的内部通道。
- 新 Turn 按确定性规则加载当前有效的系统 Skill 与当前用户 Skill；同一 Turn 冻结 Skill ID、版本、内容摘要、优先级和加载顺序。
- 同一 Turn 的重试、Resume、回执重放必须使用冻结快照；发布或停用的变化从下一个 Turn 生效。
- 没有有效 Skill 时不注入空 Skill 指令、占位 Loader 或虚构能力。

### 9.2 发布和权限

- 已发布 Skill 版本不可变；任何编辑必须创建新版本，历史 Session/Turn 保留原版本引用。
...
结合现有 AIGC 设计，v1 主 Agent 只允许注册以下五个高层 Graph Tool：

| Tool Key | 主要职责 | 是否创建异步生成任务 |
| --- | --- | --- |
| `analyze_materials` | 分析用户素材并形成受控分析结果 | 否 |
| `plan_creation_spec` | 规划创作规格和候选方案 | 否 |
| `plan_storyboard` | 规划故事板、元素、槽位和可审核 Prompt | 否 |
| `generate_media` | 为当前有效故事板创建媒体生成 Operation/Batch/Job | 是 |
| `assemble_output` | 创建预览或最终成片装配 Operation/Batch/Job | 是 |
...
- 每个新 Turn 冻结消息边界、Skill 快照、Prompt 版本、Tool Registry 版本、预算和可信 Command Context。
...
- Business 用户、鉴权、积分、费用、资产和管理配置由 Business Service 负责；Agent 通过版本化 RPC DTO 访问。
...
- Agent 创建并拥有的 Session、Skill、Checkpoint、Receipt、Approval、Event、Operation、Batch、Job 或 Outbox 表由 `agent/migrations` 管理；跨 Module 消费不转移所有权。
...
- Agent 不自行决定价格或最终扣费；费用事实通过 Business RPC 或受信 Worker 结果获得。
...
- Graph Tool 只能创建有界 Operation、Batch、Job 和 Outbox，然后返回 `accepted`；不得轮询或阻塞等待 Worker。
...
- Worker 不调用主 Agent、不加载 Skill、不决定 Prompt、不重新规划故事板。
- Worker 终态通过版本化、持久化 `BatchContinuationResult` 进入 Session Lane；它不是原 Graph 函数返回。
- 确定性的成功投影、状态收尾和费用快照处理不依赖模型；只有确需用户解释或下一步语义选择时才创建 Agent Turn。
- Agent 不重试 Worker、Provider、计费或补偿；这些层各自按权威状态和幂等策略处理。
...
- 当前五个 AIGC Capability 默认使用持久化 Approval 加新的可信 Continuation Turn，不在审核点保留原 Graph 调用栈；若某个独立 Graph Tool 设计确需 Interrupt/Checkpoint/Resume，必须先完成 outer/inner Checkpoint、Mapping、Version/Fence、Fallback 和恢复测试评审。
...
- 配置热更新不得改变正在执行 Turn 的 Skill、Prompt、Tool Registry 和预算快照；新版本从后续 Turn 生效。
...
- Agent/Business RPC、Worker Continuation、Tool Schema、Event/A2UI 和 Skill 版本使用契约测试。
...
现有设计文档同时描述“当前实现”和“目标形态”，实施和评审必须明确区分，禁止把目标项表述为已实现。特别是 Redis Streams、独立 Stage Ledger、持久化 ToolOperationResult、确定性 PostBatchContinuation Graph、统一 Projector/Inbox、统一最终事务等目标能力，在代码和 Migration 实际存在前不得作为当前能力验收。

旧设计中的 `internal/aigc/**`、根 Module 等路径属于历史结构描述，不能覆盖本规范确认的 `agent/internal/**`、`business/**`、`worker/**` 三 Module 布局；修改相关能力时必须把涉及的代码路径和契约映射到新的功能包结构，并同步修正文档。设计文档中的“已实现”声明必须用当前分支的代码、Migration 和测试核验，不能仅凭文字继承。
...
- 用户、鉴权、积分、费用、资产和管理配置留在 Business，通过 RPC 交互；
...
- [ ] Runtime Skill 只从数据库动态加载并以内联模式注入；系统/用户 Skill 来源、审核、不可变版本和 Turn 冻结规则符合规范，未使用 `fork`/`fork_with_context`。
...
- [ ] Skill、Prompt、Tool Registry、消息边界和预算在 Turn 内冻结，恢复不混入后续消息。
...
- [ ] Worker 结果通过持久化 Continuation 进入 Session Lane，Agent 不等待或重试 Worker/Provider。
