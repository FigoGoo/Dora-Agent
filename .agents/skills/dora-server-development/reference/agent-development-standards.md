...
当前工作树已经建立 `business/`、`agent/`、`worker/` 三个独立 Go Module、各自生产 Runtime 入口和版本化 Migration 目录。local-only Development Preview 已用 `make trial-basic` 跑通统一六 Tool、Worker Job Claim/Lease、Business Finalize、确定性 PNG、固定参数 MP4、同源读取与浏览器恢复的快速 MVP happy path；这只证明基本功能主链可运行，不等于五条 isolated canonical smoke、进程重启、故障注入、三 Module 全量门禁或生产发布已通过。精确的阶段状态、Evidence 和下一步只以 `docs/requirements/full-function-smoke-development-plan.md` 为准，各 Tool 获批范围以 `docs/design/agent/graphtool/README.md` 与对应独立设计为准，不得仅凭目录、代码或单条 Trial 存在宣称完整功能已验收。现有三份顶层 AIGC 设计文档是历史 `main` 单体实现迁移快照与目标契约参考，实施和评审必须明确区分，禁止把历史能力或目标项表述为当前工作树已实现。特别是 Redis Streams、独立 Stage Ledger、持久化 ToolOperationResult、确定性 PostBatchContinuation Graph、统一 Projector/Inbox、统一最终事务等目标能力，在当前工作树的代码、Migration 和测试 Evidence 实际存在前不得作为当前能力验收。
